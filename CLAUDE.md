...
- **D0–D7 products** — distinct surfaces, each owning its Render service + UX/speed testing (D0 terminal + orders-dashboard · D1 store · D3 broadway · D4 Exos/Bridge *(SPA source in `JulianS4K/EXP` since 2026-09-24, edge functions since 2026-09-26; its migrations + SQL harnesses stay here while the DB is shared)* · D5 fantasy · D7 N2S obligation-covering). ⚠ **D7 is a NAMED SUBSET of D0's surface**, not a separate tree — `subs.{html,js}` and `n2s_*` only; see `PROJECT_BIBLE §2.3`.

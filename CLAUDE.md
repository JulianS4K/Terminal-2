...
- **Enforcement is two hard CI gates** (server-side; `--no-verify` cannot skip). `docs-registry-check` (`bin/check-docs.sh`) fails a PR that adds a root `*.md` not in the README registry, or a dated/working-note file in `docs/` outside `docs/archive/`. **`bible-freshness` (`bin/check-bible-freshness.sh`) fails a PR that changes a doc-bearing path — migration · `*_client.py` · edge fn · deploy IaC — without updating a canonical doc** (RULE 0, same-PR discipline); escape hatch `BIBLE-OK: <reason>` in a commit message.

...
- **D0–D7 products** — distinct surfaces, each owning its Render service + UX/speed testing (D0 terminal + orders-dashboard · D1 store · D3 broadway · D4 Exos/Bridge *(source in `JulianS4K/EXP`: SPA since 2026-09-24; edge functions, `*exos*` migrations + `tests/exos` since 2026-09-26. The DB is still shared, so they apply/deploy to this project; only the built `static/bridge/` bundle lives here)* · D5 fantasy · D7 N2S obligation-covering). ⚠ **D7 is a NAMED SUBSET of D0's surface**, not a separate tree — `subs.{html,js}` and `n2s_*` only; see `PROJECT_BIBLE §2.3`.

...
| **Flights** (`flights.html` / `flights.js`) | A | `T.api('/api/broker/flights/search?q=&city=&date_from=&date_to=&max_price=&min_sources=&sort=')` (search results + the per-date price graph in one round-trip); `T.api('/api/broker/flights/event/{id}?qty=')` on row expand (per-marketplace booking options + cheapest live listings + price history); `T.api('/api/broker/flights/suggest?q=')` typeahead. Path A only — the TD tables are RLS-locked, so no direct RPC | `events`, `event_listing_snapshot_daily`, `aq_event_map` → `ticketsdata_event_xref` → `ticketsdata_listings_snapshots`, `latest_event_metrics` |

...
|---|---|---|---|
| **Home** (`index.html` / `home.js`) | A + C | `T.api('/api/broker/movers?window_hours=…')` (`home.js:31`); rpc `get_blind_spots_sg_selling({p_min_score:2})` (`home.js:122`); rpc `get_sg_market_chart(offset,limit)` (Top-50 non-owned chart); rpc `get_sg_owned_stall_chart(offset,limit)` + `get_sg_owned_stall_sections(event_id)` (owned-and-stalled cross-source companion panel + click-open section drill, mig 20260709190000) | `event_movers_index`, SG blind-spot views, `sg_market_chart`, `sg_owned_stall_chart(_sections)` |

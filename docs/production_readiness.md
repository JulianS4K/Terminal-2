...
| 2 | **No Supabase connection pooling** | App uses one global service-role client, no pooling. A traffic spike or a 2–5 min Supabase blip can pile up requests → dyno OOM → outage. | A1 / Operator | Enable Supabase's **Transaction/Session pooler**, repoint the app at the pooled connection string (env). Verify under load. | S–M |
| 3 | **`seatgeek_seller_listings` NULL-key upsert dup** | `ON CONFLICT(sg_listing_id,content_hash)` never matches NULL `sg_listing_id` (Postgres `NULL≠NULL`) → unbounded duplicate rows on every pull. Found in the 2026-06-25 bug-hunt; code-only fix would lose inventory. | A1 / Operator | DB migration: partial/expression unique index or a non-null surrogate (`COALESCE(sg_listing_id, seller_listing_id)`) + repoint the upsert. Branch-test first. | S — see **KANBAN `BUGHUNT-1`** |

## P1 — resilience (before an SLA or running >1 instance)

| # | Item | Why | Owner | Concrete action | Effort |
|---|---|---|---|---|---|
| 4 | **No circuit breaker / retry on the DB client** | A Supabase hiccup cascades instead of failing fast + shedding load. | B1 / A1 | Wrap the Supabase calls with retry+backoff and a fast-fail circuit breaker; add a fallback read path (cached/stale snapshot) for the storefront. | M |
| 5 | **In-process rate limiter + per-process session secret** | Both are single-instance only. At >1 dyno the rate limit is per-instance (bypassable) and signed cookies break across instances. | Operator (provision) + B1 (code) | Provision Redis (or similar); swap `_IPRateLimiter` for a shared store and pin a stable session-signing secret in env. **Do this before scaling past 1 instance.** | M |
...
| 7 | **No tested migration rollback** | 631 migrations, no reverse/PITR path exercised; a bad apply has no fast recovery. | A1 / Operator | Document + dry-run a rollback runbook (reverse migration + Supabase point-in-time restore) against a branch. | M |

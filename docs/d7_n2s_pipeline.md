...
### 1. Ingest — two paths into the same table
...
A second path lands rows the CRM feed never carries: `n2s_items_ingest_from_apps_script()`
(mig `20260915130000`), called by `scripts/apps_script/n2s_email_sync.gs` from a Gmail
label the operator configures per marketplace. It upserts on `email_thread_id` instead of
`n2s_id` (a synthetic id from `n2s_email_items_id_seq`, namespaced at 900,000,000,000+ so
it can never collide with a real CRM item id) and sets `item_source = 'email_apps_script'`
so the two populations stay distinguishable. Every stage from §2 on reads `n2s_items`
without caring which path a row came from.

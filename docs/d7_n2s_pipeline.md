...
| cron 598 | every minute | `n2s_order_identity_pull(); n2s_map_events(true); n2s_gt_map_by_name(); n2s_pull_all_sources();` — drain by-id order responses, cross-map, fire new probes (20260911090000); then map; then the D7 GoTickets tiered second pass (20260911020000); then poll the newly-mapped |

...
### 4b. What counts as a live obligation

Every stage asks "is this obligation still worth working" before spending a poll or a
match on it. Until `20260914223000` the test was `event_dt::date >= current_date`, and
`current_date` is the **UTC** date while `event_dt` is venue-**local**. At 00:00 UTC —
8pm Eastern — every evening event on the calendar day that had just ended in UTC fell out
of the book: no on-arrival pull, no candidates, hidden from `v_n2s_orders`. On NFL Sunday
09-13 that was 88 orders; Cowboys at Giants kicked off 8:20pm ET, 64 orders arrived from
7:57pm ET and all were invisible.

The test is now `n2s_event_live(event_dt, tevo_event_id)`: live until the event started
more than 4 hours ago, by local time, resolved in order — TEvo's offset-carrying
`occurs_at_local` when it has one, else `venue_assets.nws_time_zone`, else `event_dt` read
as `America/Los_Angeles` (the westernmost mainland zone, so nothing is ever dropped early).
A midnight `event_dt` is TEvo's "time TBD" and is live through the whole local day.

> ⚠ The predicate lived in **nine** places (eight `n2s_*` functions + the view). The
> migration rewrites the live bodies mechanically and raises if any still carry the old
> form — add new call sites through `n2s_event_live()`, never by hand-rolling a date test.

### 4c. Landed cost, not the bare ask

The three sources quote on three bases: TEvo `retail_price` is a **bare ask** (the buyer
fee lands at checkout), GoTickets `all_in_price` and SeatGeek `retail_price_all_in` are
what checkout charges. Ranking them as one number let a TEvo listing win every near-tie.
Since `20260914223500` every money expression in `n2s_cover_candidates` — tier-3
admission, the profitable / within-cap gates, the per-order ranking, `sub_total`,
`cover_cost` — goes through `n2s_landed_total(src, ea, qty)`, which adds
`order_fee_schedule.buyer_fee_pct` + `fixed_fee_per_order` for `source='tevo'` and passes
the others through. **`sub_ea` is unchanged** — it is the ask `n2s_buy_intent_create`
puts in the TEvo order payload and the number `n2s_cover_verify` compares to the live ask.

> ⚠ The schedule row is a placeholder (`buyer_fee_pct = 0`, "verify"). Nothing changes until
> the operator sets the rate — `UPDATE order_fee_schedule SET buyer_fee_pct = <fraction>
> WHERE source = 'tevo'` — and it takes effect on the next tick, no redeploy.

...
|---|---|---|
| `n2s_crm_direct_20s` | every 20 s | `n2s_crm_fetch_direct()` — **new and changed orders.** Synchronous GET of CRM page 0 (sorted `updated_at` DESC) through the `http` extension, upserted in the same statement. Logged per fetch in `n2s_crm_direct_log` (2-day window). (20260925231959) |
| cron 640 `n2s_pipeline_tick_1min` | every minute, back-to-back | `n2s_pipeline_tick()` — one budgeted chain: drain the pg_net CRM replies → order-identity pull → mapper → GoTickets name pass (5-min cadence) → `n2s_pull_all_sources` → queue refresh / history / feed sync → sub ping → push outbox → **fire CRM pages 150/300 via pg_net** for the next tick |
...
> ⚠ **Why page 0 is not on pg_net (20260925231959).** pg_net is one FIFO queue shared with every
> edge-function invocation and GoTickets poll, and a batch finishes with its slowest request.
> Measured 2026-09-25: the tick's CRM pages waited a typical 62 s in that queue (never under
> 34 s; all three pages "answered" at the same instant — the batch, not the CRM), 19% died on
> the 60 s timeout, and the reply then sat a typical 78 s until the next tick's drain. Net: a
> typical 2 min 5 s from CRM creation to `n2s_items`. pg_net has no priority, so page 0 left
> the queue rather than trying to jump it.
>

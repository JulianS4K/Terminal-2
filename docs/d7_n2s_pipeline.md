...
| cron 640 `n2s_pipeline_tick_1min` | every minute, back-to-back | `n2s_pipeline_tick()` — one budgeted chain: drain the pg_net CRM replies → order-identity pull → mapper → GoTickets name pass (5-min cadence) → `n2s_pull_all_sources` → queue refresh / history / feed sync → sub ping → push outbox → **fire CRM pages 0/150/300 via pg_net** for the next tick |

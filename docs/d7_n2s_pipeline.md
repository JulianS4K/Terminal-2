...
| 0d | `n2s_vivid_order_identity` | `vivid_orders.tevo_event_id` on the Vivid order id, else the hub on `raw.productionId` (= `aq_event_map.vivid_event_id`) | Our own Vivid broker-order pull, filled by id on demand (§3 probe, mig `20260911060000`) |
| 0e | `n2s_sg_order_identity` | `seatgeek_orders` on the SG order id → tevo id from the row, else `sg_event_id` via xref, else the hub | Our SellerDirect order pull; **dormant until the pull reaches current pages** (mig `20260911050000`) |
...
- **SeatGeek** — the SellerDirect pull fetched page 1 of each status, ascending by creation,
  ...
  `confirmed` 6,633, `fulfilled` 582,238). Mig `20260911050000` adds the last two pages per
...
`n2s_order_identity_pull()`, mig `20260911060000`): one GET per open unmapped Vivid/GoTickets
...
| cron 598 | every minute | `n2s_order_identity_pull(); n2s_map_events(true); n2s_gt_map_by_name(); n2s_pull_all_sources();` — drain by-id order responses, cross-map, fire new probes (20260911060000); then map; then the D7 GoTickets tiered second pass (20260911020000); then poll the newly-mapped |

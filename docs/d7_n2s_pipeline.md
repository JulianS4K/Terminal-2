...
| 0c | `n2s_gt_order_identity` | `gotickets_sales.gt_event_id` → `gotickets_event.tevo_event_id` on the sale id | GT's **own** event id, already resolved by the GT mappers (mig `20260911040000`) |
| 0d | `n2s_vivid_order_identity` | `vivid_orders.tevo_event_id` on the Vivid order id | Our own Vivid broker-order pull; **dormant until the pull is widened** (see below) |
...
**Vivid and SeatGeek: the road exists, the books are not reaching the orders.** Rules 0d/0e
are wired but mapped 0 of 117 / 0 of 16 when they shipped, for two ingest reasons that are
A1's to fix, not the mapper's:
- **Vivid** — cron 207 pulls `getOrders?status=PENDING_SHIPMENT` only. 867 of our rows sit
  inside the N2S id range and the N2S ids' neighbours are there, but the N2S ids themselves
  are absent even from raw: the failed orders carry some other Vivid status. Widen the
  pull (or confirm the status with one `getOrder?orderId=` read) and rule 0d starts mapping.
- **SeatGeek** — the SellerDirect pull fetched page 1 of each status, ascending by creation,
  ...
  status; its first cycle proves whether offset paging still works on `/orders`.
...
| cron 598 | every minute | `n2s_map_events(true); n2s_gt_map_by_name(); n2s_pull_all_sources();` — map, then the D7 GoTickets tiered second pass (20260911020000), then poll the newly-mapped |

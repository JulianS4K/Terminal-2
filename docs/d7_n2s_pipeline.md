...
| cron 640 `n2s_pipeline_tick_1min` | every minute, back-to-back | `n2s_pipeline_tick()` — one budgeted chain: drain the pg_net CRM replies → order-identity pull → mapper → `n2s_pull_all_sources` → queue refresh / history / feed sync → sub ping → push outbox → **fire CRM pages 0/150/300 via pg_net** for the next tick |
| `n2s_gt_map_by_name_1min` | every minute, only while a live order has no TEvo event; skips while the previous run is going | `n2s_gt_map_by_name()` — GoTickets name matcher, **out of the tick** (20260926014415): it added ~80 s to every 5th-minute tick (those ran a typical 2 min 1 s vs 41 s) while creating ~1 link / 3 days. Writes only `gotickets_event`, which the tick only reads. |

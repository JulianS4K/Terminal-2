...
| D4-OPS-1 | D4 | **`/bridge` service-worker scope — CONFIRMED BROKEN (D4 comprehensive test 2026-05-23).** `registerSW.ts:31` registers `/sw.js` (origin root) but the build serves it at `/bridge/sw.js` → registration 404s under the prefix → SW never installs (no offline shell, no PWA install; or it binds the hub's root `/sw.js` over `/bridge/`). Even if it registered, `sw.js:20` `SHELL` precaches root paths (`/`,`/index.html`,`/manifest.json`) = the **hub** doc, not the Bridge SPA, so the offline nav fallback serves the wrong app. NB: the localStorage offline check-in registry is SW-independent and works — only cold-boot-offline + installability are affected. | Register `${import.meta.env.BASE_URL}sw.js` with `{ scope: import.meta.env.BASE_URL }`; serve `Service-Worker-Allowed: /bridge/`; make `SHELL` paths relative to `BASE_URL` (Railway/header side may need D0/A1). |
| D4-OPS-3 | D4 | **SW never-cache list + comments are stale (Firebase, not Supabase).** `static/bridge/sw.js:24-33` `NEVER_CACHE_HOSTS` lists firestore/firebase/identitytoolkit/securetoken/firebaseio + Stripe; Firebase is fully removed (now Supabase) and the Supabase host isn't listed. No live bug today (Supabase reads are cross-origin GETs → fall through to case-4 uncached; auth/RPC are POST and skip the GET-only handler), but it's dead/misleading config + a footgun if a same-origin `/api` proxy or cross-origin cache branch is ever added. | Refresh `NEVER_CACHE_HOSTS` to the Supabase host (`hzrizjeaxlqcxfrtczpq.supabase.co`); drop the Firebase/Firestore entries + Firestore-era comments. |
| D4-OPS-4 | D4 | **Offline CSV export mislabels voided tickets as `active`.** `OrganizerCheckIn.tsx:279` builds the status column as `e.used ? 'used' : 'active'` and ignores the `voided` flag carried in the same offline registry entry → refunded/voided tickets export as `active` in the attendee CSV backup. | One-line: `e.voided ? 'voided' : e.used ? 'used' : 'active'`. |
| D4-OPS-5 | D4 | **Event-level oversell not enforced — only tier capacity is.** `exos_mint_tickets` (mig 20260520130000) checks `tier.capacity` but never `exos_events.total_tickets`; verified live that `tickets_sold` can exceed `total_tickets`, and `p_tier_id = NULL` mints unlimited. Scanner UI surfaces `total_tickets` as "Total Authorized" (`OrganizerCheckIn.tsx:929`), so combined tier capacity > total_tickets oversells the advertised house. | Decide whether `total_tickets` is a hard house cap; if so add the check in the RPC (operator-gated migration). Else document that per-tier capacity is the only cap. |
| D4-OPS-6 | D4 (B1 aware) | **Check-in is not server-verified (by-design, documented).** `exos_check_in_ticket` only does the org-role gate + atomic status flip; HMAC/secret verification lives ONLY client-side (`barcode.ts:29-33`). A compromised organizer client / direct authenticated RPC call can admit any ticket id for an org it staffs without presenting a valid barcode. Accepted tradeoff for primary-market (organizer owns its own door). Logged for visibility, not as a defect. | If the threat model tightens: pass the scanned payload to the RPC and verify HMAC server-side there. No action unless prioritized. |
| D4-OPS-7 | D4 + Op (Stripe) | **🔴 No paid purchase flow — comp/test mint only (prod-blocker for any paid event).** `lib/tickets.ts:13` + bible: real buyer fulfillment (Stripe Checkout + webhook-driven mint) is unbuilt. `exos_mint_tickets` is the org-staff/admin comp path. `exos_org_secrets.payments` has Stripe Connect fields but nothing is wired. Today you cannot sell a ticket for money. | Build Stripe Connect onboarding (per-org connected account) + Checkout session creation + a `stripe-webhook` edge fn that mints on `checkout.session.completed` under service_role (idempotent on session id → `order_ref`). Op: provide Stripe keys/vault. Largest single workstream. |
| D4-OPS-8 | D4 + A1 deploy | **🔴 `exos_mail` drainer unbuilt — no ticket delivery / receipts / transfer notices (blocker).** Queue + `exos_queue_mail` SECDEF RPC land rows as `status='pending'` (mig 20260520160000) but nothing sends them. Buyers get no email; access is in-app only. | Build a `exos-mail-drain` edge fn (cron-gated, `requireCronSecret`) that pulls `pending` rows, sends via an email provider (Resend/Postmark/SES), marks `sent`/`failed` with retry. Op: provider API key in vault. Templates already defined in the RPC. |
...
| D4-OPS-10 | D4 | **🟠 Multi-lane offline double-admit risk.** Offline scanners admit on the local localStorage registry + queue a replay; lanes don't share lock state, so the same ticket can walk in at two offline doors (the server flip only catches it on later sync). `barcode.ts:34-37` acknowledges cross-station replay. Real gate-crashing exposure at a multi-lane 1000-person door on flaky WiFi. | Decide policy: (a) require online check-in at the door (simplest — depends on solid venue WiFi + D4-OPS-1), (b) single designated offline-master lane, or (c) a lightweight shared-lock channel (Supabase realtime broadcast of used-ids between lanes). Document the chosen door SOP. |
| D4-OPS-11 | D4 / Op | **🟠 No load / door-throughput rehearsal.** Lifecycle verified at 13 rows only. Unproven: 1000-ticket registry pull + concurrent multi-scanner check-in throughput + Auth sign-in spike. Per-ticket check-in is atomic (no global contention) and a 1000-row registry is small, but nothing has been driven at scale. | Seed a 1000-ticket test event on a Supabase branch; run concurrent simulated scans (k6/Artillery against the RPC) + measure registry pull + a physical door rehearsal with 2-3 lanes. Gate go-live on the rehearsal. |
...
| D1-OPS-2 | D1 | **No `Cache-Control` on `/static/store/*`.** Verified live: `store.js` returns `cf-cache-status: DYNAMIC` with NO app `Cache-Control` → browsers revalidate every asset every load despite the `?v=<sha>` cache-bust already in place. Sprint 5 perf item. | Mount static assets with `Cache-Control: public, max-age=31536000, immutable` (the `?v=<sha>` query already handles invalidation). ~5 lines on the `/static/store` mount in `app.py`. |
...
| D1-OPS-4 | D1 | **Legal pages lack favicon/OG/description.** `/store/about`,`/store/privacy`,`/store/terms` (PR #164) shipped before the SEO scaffold (PR #166) and never got the `<head>` parity. Verified live: `/store/about` has only `<title>` — no favicon link, no OG, no meta description. | Add the same favicon `<link>` + `<meta name="description">` (+ optional OG) block from `index.html` to the 3 legal shells. `robots` can stay default (these are indexable). Small. |
...
4. **D4-OPS-4** CSV voided-label fix (**S**) — one-line in-lane quick win.

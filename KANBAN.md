...
- **[B1-NEXT-32] [SEC-CRIT]** Add `requireCronSecret` body-gate to `supabase/functions/match-sg-performers-to-tevo/index.ts:126`. Currently relies on Supabase platform `verify_jwt=true` only — anon JWT (publishable via `/api/public/config`) suffices to authenticate. Caller-controlled `limit` (1-200) drives paid TEvo `/v9/performers/search` calls. Cost-exploitation vector. Cross-lane to A1 (edge-fn owner). Surfaced by B1 code audit 2026-05-16 (`docs/security-audit-2026-05-16-code-review.md` §1).
- **[B1-NEXT-33] [SEC-HIGH]** Add `requireCronSecret` body-gate to `supabase/functions/why-noaa-weather-alerts/index.ts:30`. Same anon-JWT-suffices problem; POST handler unboundedly INSERTs into `why_signals` table via service-role client. DB integrity / DoS risk. NOAA API is free so no credential leak — risk is DB write amplification. Cross-lane to A1. Surfaced by code audit §2.
- **[B1-NEXT-34] [SEC-MED]** Replace wildcard CORS in `supabase/functions/espn/index.ts:27` (`"access-control-allow-origin": "*"`) with `ESPN_ALLOWED_ORIGINS` env-var allowlist. Mirror the `chat` edge function's `CHAT_ALLOWED_ORIGINS` pattern. ESPN API is free/public so no credential exposure — defense-in-depth gap. Cross-lane to A1. Surfaced by code audit §3.

...
| ID | Lane | Task | Action |
|---|---|---|---|
| BUGHUNT-1 | A1 / Op | **`seatgeek_seller_listings` dedup is defeated for NULL `sg_listing_id`** (found in the 2026-06-25 bug-hunt; code-fix deliberately NOT applied). `store_seller_listings` upserts `on_conflict="sg_listing_id,content_hash"`, but in Postgres `NULL ≠ NULL` in a unique key, so any seller listing returned without an `sg_listing_id` re-inserts on every pull → unbounded duplicate growth. A code-only "skip null rows" would *lose* that inventory, so the correct fix is DB-side. | **A1 (operator-gated, RULE 1):** add a partial/expression unique index or a non-null surrogate to the conflict key (e.g. `COALESCE(sg_listing_id, seller_listing_id)`), then point the upsert at it. Verify against a branch first; needs an `apply_migration`. |

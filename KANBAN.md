...
| Item | Action |
|---|---|
| **Recharge TwitterAPI.io credits** — starter balance exhausted 2026-07-07 18:14Z after 660 tweets (≈$0.10 @ ~$0.15/1k); X wire is in 402-backoff (one probe/~30 min, mig `20260707200000`) | Recharge at the twitterapi.io dashboard → wire auto-resumes ≤30 min, no other action. Spend is now **double-capped** (operator directive 2026-07-08, mig `20260708160000`): incremental `since:` polling bills only NEW tweets + a hard daily budget (`x_news_budget`: 1,200 req/day · 3,000 tweets/day ≈ $0.45/day ceiling; queue holds until UTC midnight on cap; post-recharge roster sweep spreads ~3 days, insiders first). Watch `v_x_news_usage` + `v_x_news_budget_state`; tune `UPDATE x_news_budget SET max_tweets_per_day = <n>;`. |

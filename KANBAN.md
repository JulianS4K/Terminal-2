...
| D4-OPS-5 | D4 ✅ built · A1 apply | **✅ FIXED IN SOURCE (PR #323) — apply pending.** Mig `20260523150000` CREATE OR REPLACEs `exos_mint_tickets` to enforce `exos_events.total_tickets` atomically in the same UPDATE that bumps `tickets_sold` (`total_tickets = 0` = uncapped, mirrors tier `capacity = 0`); a NULL-tier mint is now also bounded. RAISE rolls back the tier increment (single txn). Same signature → no overload. | **A1:** apply mig `20260523150000`. Validate on a branch with the exos schema (preview replay is broken repo-wide). |
| D4-OPS-6 | D4 ✅ built · A1 apply · B1 review | **✅ FIXED IN SOURCE (PR #323) — apply pending.** Server-side barcode verification added. Mig `20260523160000` DROPs the 3-arg `exos_check_in_ticket` + CREATEs a 4-arg with `p_barcode_payload`: for a signed payload the server re-checks ticket id + owner + ±2-bucket window + HMAC (`extensions.hmac`, base64url — same scheme as `lib/barcode.ts`) and rejects forgeries (`barcode-rejected` / `barcode-expired`). NULL payload = manual/attested (allowed, recorded as such); legacy 3-seg skipped (rollout); a non-3/4-segment `T-` payload is now rejected as malformed (tightened 2026-05-23, branch-verified). Client (`tickets.ts` + `OrganizerCheckIn`) passes the scanned payload + honors server rejection (incl. the offline-registry online flip, where a transfer-rotated secret now denies instead of optimistically admitting). tsc + build green; HMAC expr matches the client (proven in the 2026-05-23 crypto test). | **A1:** apply mig `20260523160000` (it DROP+CREATEs — verify no other caller relies on the 3-arg). **B1:** review the new server auth surface. |
| D4-OPS-7 | D4 ✅ scaffold · Op decisions+keys · A1 deploy · B1 review | **🟡 SCAFFOLD BUILT IN SOURCE (PR #323) — Stripe decisions + keys + deploy pending.** Full server path: mig `20260523170000` (`exos_checkout_sessions` ledger + **idempotent** `exos_fulfill_checkout()` service_role mint, keyed on session id, honors tier + `total_tickets` caps; + `exos_record_org_stripe()`); edge fns `exos-checkout` (auth'd buyer → destination-charge Checkout Session + pending ledger row), `stripe-webhook` (signature-verified → fulfill on `checkout.session.completed`, Connect status on `account.updated`), `exos-connect-onboard` (owner → Connect account + onboarding link); client seam `lib/checkout.ts`. tsc green; no Deno locally (edge fns unchecked). | **Operator (decide, marked TODO in code):** Connect account type (Standard?), charge model (destination + app-fee assumed), fee % (`EXOS_PLATFORM_FEE_BPS`, default 5%), success/cancel URLs; provide `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET`. **A1:** apply mig + deploy 3 fns (webhook `--no-verify-jwt`). **B1:** review. **Gate:** D1-OPS-5 counsel'd Terms/Privacy. **Open:** refund flow when `exos_fulfill_checkout` hits a sold-out race (charged-but-no-ticket); UI buttons not wired. |
| D4-OPS-8 | D4 ✅ built · A1 apply+deploy · Op secrets | **✅ DRAINER BUILT IN SOURCE (PR #323) — apply + deploy + secrets pending.** `supabase/functions/exos-mail-drain/index.ts` (cron-gated via `requireCronSecret`, service_role) claims pending rows, sends via Resend, records the outcome. Mig `20260523130000` adds the concurrency-safe claim primitives: interim `'sending'` status + `attempts`/`claimed_at`/`last_attempt_at` + `exos_mail_claim_batch()` (FOR UPDATE SKIP LOCKED, re-claims stuck rows) + `exos_mail_mark()` (sent / failed-at-cap / pending-retry). Provider call isolated for easy swap; couldn't typecheck locally (no Deno), mirrors `sg-seller-webhook`. | **A1:** apply mig `20260523130000` + deploy `exos-mail-drain`. **Op:** set edge-fn secrets `RESEND_API_KEY` + `EXOS_MAIL_FROM` (verified sender) + schedule the 2-min cron (snippet in the fn header; `cron.*` operator-gated). |
...
| D4-OPS-13 | D4 ✅ scaffold · Op creds + Hard-Rule-2 sign-off · A1 deploy | **🟡 DISTRIBUTION SCAFFOLD BUILT (PR #323) — "Toast for tickets" outbound layer.** Insight (operator): D1's storefront reads TEvo/EVO "owned" inventory and Automatiq distributes INTO EVO, so ONE path (Exos primary → Automatiq → EVO) surfaces inventory in D1 **and** the wider secondary ecosystem — distribution *is* discovery. Mig `20260523190000`: `exos_distribution_listings` (per-event-per-channel state, staff-read RLS) + `exos_request_distribution(event, channels[], qty, price)` (owner/manager, records intent). Edge fn `exos-distribute` (cron-gated, service_role) drains pending intents → would call Automatiq → records result. **GATED + INERT** until `AUTOMATIQ_API_KEY` + per-org `distribution` creds + Hard-Rule-2 operator sign-off (live = upstream listing WRITE). | **A1:** apply mig `20260523190000`, deploy `exos-distribute`. **Op:** Automatiq/Lysted creds + Rule-2 authorization. **Follow-up (D1):** once live, EVO listings appear in D1 automatically (it reads EVO); add D1 dedupe/route via `bridge_event_xref.tevo_event_id` so the buy goes to the in-house Exos checkout, not the secondary path. Not branch-tested (inert scaffold). |
...
6. **D4-OPS-8** `exos_mail` drainer (**M**) — ✅ built in source (PR #323: edge fn + claim/mark migration `20260523130000`). Delivery / receipts / transfer notices (NOT payments — useful on free events too; shares the provider with D4-OPS-9). Pending: A1 apply mig + deploy fn; Op set `RESEND_API_KEY`/`EXOS_MAIL_FROM` + schedule the 2-min cron.
7. **D4-OPS-5** event-level oversell enforcement (**S**) — ✅ built in source (PR #323: mig `20260523150000` enforces `total_tickets` in `exos_mint_tickets`). Pending A1 apply.

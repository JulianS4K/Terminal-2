...
| Item | Action |
|---|---|
| **X news wire — LAST STEP: create the API key** (PR #816; migs `20260706113000`+`20260706114500`+`20260707190000` **✅ APPLIED 2026-07-07** via MCP under operator direction, fn bodies md5-verified vs repo; **FULL 427-handle roster ✅ IMPORTED + ENABLED** from the Ticket_news_ticker sheet with per-category poll tiers (insiders 15m · beats 2h · leagues 3h · orgs/promoters/reporters 4h · teams/venues/theaters/shows 6h · artists 12h) + keyword tagging at ingest (`match_news_keywords` → `x_news.matched_keywords`, hits surface as ticker summary); queue is overdue-ratio prioritized; crons live + gated, zero spend until the key exists) | Create the vault secret: `SELECT vault.create_secret('<twitterapi.io key>', 'TWITTERAPI_IO_KEY');` → wire goes live within ~1 min (no deploy; insiders lead the initial sweep). Spend ceiling = the 1-call/min queue tick (≈1,440 calls/day flat, roster size only spreads cadence) — trim via `x_news_sources.enabled`/`poll_interval_minutes`, or raise `x_news_queue(N)` in the cron to trade spend for freshness. |

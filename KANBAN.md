...
| D4-OPS-16 | D4 / open | **⚠️ `event-cancelled` / `event-updated` templates UNWIRED.** Both exist in `exos_queue_mail` (mig 20260520160000) but **no UI/flow triggers them** — ticket-holders are NOT notified on event cancel/update. They derive recipient from a single ticket; notifying ALL holders needs a per-holder loop (like `exos_announce_to_followers`). | Build `exos_notify_event_holders(event_id, template)` (org staff; loops holders → queues `event-cancelled`/`-updated`) + wire EditEvent's cancel/update actions. Not built (flagged in the 2026-05-23 mail audit). |

...
| D4-OPS-33 | D4 ✅ built · **apply pending** (2 migs) · deploy pending (Python + bridge rebuild) | **✅ STAGE 5 (ORGANIZER GROWTH) BUILT IN SOURCE 2026-09-11 — PR #977 draft, branch `claude/d4-organizer-stage5` stacked on #976.** (1) **Email campaigns** mig `20260911140000`: `exos_campaigns` + recipients + per-org `exos_marketing_optouts`; audiences holders / no-shows / waitlist / followers / past attendees / all buyers resolved server-side; send now or scheduled (cron `exos_send_scheduled_campaigns`); tokenised unsubscribe link → `/unsubscribe/:token` (anon RPC); `CampaignsPanel` on the event report; 'campaign' added to the mail allowlist; **SMS modeled, send refuses until a provider is configured**. Security review fix folded in (base_url charset CHECK + escaped href). (2) **Google distribution** mig `20260911141000`: `google_place_id` / `venue_lat` / `venue_lng` (+ `updated_at`) on events and `exos_public_events`; Create/Edit inputs; `buildEventJsonLd` now emits geo / hasMap / identifier / performer / eventStatus / url / validFrom; **`routers/pages.py` D4 block pre-renders `/bridge/event/{uuid}` with the JSON-LD + OG server-side and serves `/bridge/sitemap-events.xml`** (7 pytest cases). (3) **Ads boost** on PromoteEvent: per-platform tracked link + pixel status + Ads Manager hand-off; Google Search/Maps readiness checklist + sitemap URL. Validation: vitest 86/86, tsc, vite build, exos harness Parts A–J, pytest full suite green, check-docs clean. | **Operator/Applier:** apply `20260911140000` → `141000` after the Stage 3 set; deploy = Python service redeploy (pages router) + `static/bridge/` rebuild; submit `/bridge/sitemap-events.xml` in Search Console. **Decisions:** SMS provider (Twilio? consent/STOP handling to follow) or leave deferred · authorize Meta CAPI / Google conversion uploads (upstream writes, Hard Rule 2) · apply to Google Things-to-do Center · Google Places API key in Vault for venue autocomplete (today the organizer pastes the Place ID). |
...
- **Stage 5 — organizer growth (✅ in source 2026-09-11, PR #977 draft on `claude/d4-organizer-stage5`, stacked on #976; ⏳ migrations `20260911140000`/`141000` NOT applied — see D4-OPS-33):** email campaigns ✅ (segments · opt-out · schedule · panel; SMS modeled, send gated on a provider) · Google distribution ✅ (Place ID + geo, full Event JSON-LD, `/bridge/event/{id}` crawler pre-render, `/bridge/sitemap-events.xml`) · ads boost panel ✅ (tracked links + pixel status; conversion uploads / Business Profile / Things-to-do **gated**, Hard Rule 2). Operator brief: "selling via Google Maps, promotion on FB/Google Ads, text and email campaigns".

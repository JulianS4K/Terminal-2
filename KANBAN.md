...
| D4-OPS-2 | D4 | **Data cut-over — run `scripts/migrate-to-orgs.ts`.** All 5 exos migration phases applied to prod (PRs #305 + #308). `lib/storage.ts` + `lib/mail.ts` migrated. 0 rows in `exos_*`; Firestore still source-of-truth. **Unblocked** — validate app against Supabase backend on a test org, then run the script. | D4 runs `scripts/migrate-to-orgs.ts` after smoke-testing the Supabase backend. Remove `firebase-admin` SDK dep post-cut-over. |

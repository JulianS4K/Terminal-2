...
- **Stage 2 — attendee side (customer session; 2a ✅ alerts feed · 2b ✅ attendee names · 2c ✅ Spanish — PR #975 merged 2026-09-11 · 2d ✅ Release-my-seat on the pass + series grouping on browse/storefront + "more dates" on the event page + My Tickets archive tab + navbar unread badge · 2e ✅ offline ticket pass (device-cached passes so a dead-signal door still scans; `lib/offlinePass.ts`) + venue directions deep link (`lib/directions.ts`, Apple/Google) + add-to-calendar on the My Tickets cards — PR #978):** D4-OPS-23 attendee info capture (per-ticket name ✅; organizer-defined questions still need the organizer half) · ~~in-app notification backend~~ ✅ already shipped (mig `20260709120000_exos_notification_reads`, wired in `Notifications.tsx` + `useUnreadAlerts`) · D4-OPS-20 wallet passes (PR #870 carries the pass builders; needs signing certs — land the optical-pass part first) · ~~second i18n locale~~ ✅ (es).

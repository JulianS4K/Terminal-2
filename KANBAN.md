...
| Item | Action |
|---|---|
| **X news wire — LAST STEP: create the API key** (PR #816; migs `20260706113000`+`20260706114500` **✅ APPLIED 2026-07-07** via MCP under operator direction, post-apply smoke green; **28 insider handles ✅ ENABLED** (Woj/Shams/Schefter/+25, `bot_chat` #3283); crons live + gated, zero spend until the key exists) | (1) create the vault secret: `SELECT vault.create_secret('<twitterapi.io key>', 'TWITTERAPI_IO_KEY');` → wire goes live within ~1 min (no deploy); (2) hand over the full source list — handles and/or `kind='query'` advanced-search rows get INSERTed/enabled per direction. Spend = ~1 call/source per `poll_interval_minutes` (default 15m; 28 enabled sources ≈ 1 call/min steady ≈ 1.4k/day) — tune per row or disable rows to trim. |

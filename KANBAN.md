...
- **[B1-NEXT-38] [SEC-MED]** `security_invoker=true` missing on 2 new views from [PR #177](https://github.com/JulianS4K/Terminal-2/pull/177) — `v_sg_broker_sales_by_event` (mig 20260516220100) + `v_sg_broker_sales_by_section` (mig 20260516220000). Both correctly REVOKE from PUBLIC/anon/authenticated + GRANT to service_role, but `reloptions` is empty so default SECDEF semantics apply. Underlying `seatgeek_sales_snapshots` has RLS enabled — SECDEF view could bypass RLS if anon grant is ever added. `release_health_check.views.security_definer_views` currently WARN=2. Fix is a two-line `ALTER VIEW ... SET (security_invoker = true)` migration. Cross-lane to A1 (PR comment on #177 filed via bot_chat 275). Filed 2026-05-17 ~02:30 UTC.

...
|---|---|---|---|---|
| **P0** | → `A1-OPS-3` | A1 / Op | **SG broker token is the #1 product ceiling** — one token shared with an invisible external prod program; 63–77% 429, 1,235 starved events, 10/30 MLB games 2–3d stale. Physics limit, not code. | See `A1-OPS-3` (OPS table). **Operator action:** coordinate SG token isolation from the external app; interim approve the `p_max 8→4` cron-body cut. |
| **P0** | → `A1-OPS-16..21` | A1 / Op | **Snapshot firehose storage + bloat** — `listings_snapshots` 3.6GB/8.96M, SG listings 3.2GB, SG sales 1.18GB (11–23× overcount), `espn_injuries` 175MB/16.5k rows. Retention Waves 2–4 (dedup · change-detection write-gate · partition+VACUUM) still pending. | See `A1-OPS-16/17/18/21`. Finish Wave 3 change-detection (in progress), then Wave 4 partitioning. |
| **P1** | **BR-CODE-1** | D0 / A1 / Op | **`app.py` monolith — 8,753 LOC, 112 routes, 236 fns, 302 inline SQL calls, ALL surfaces in one file** (5 fns >220 lines). #1 velocity drag; every route re-writes the same filter chains, no DB-access layer. | Carve out a `routers/` package by domain (broker · store · seatgeek · seatdata · axs · admin) + a thin shared `db`/query helper; move routes incrementally behind unchanged paths. Land in small PRs, tests green each step. |
...
| **P2** | **BR-SPLIT-2** | B1 / Op | **Mechanical lane-boundary CI check (the missing enforcement).** Asset→lane assignment is now complete + orphan-free (`BOT_HIERARCHY §5.5/§5.5b`, 2026-06-19) and labeled (`.github/labeler.yml`), but enforcement is still **doc-only between D-lanes**: `CODEOWNERS` + `forbidden-paths-check.yml` only separate *owner vs coworker* (all bots commit as `@JulianS4K`), so nothing mechanically stops a D1 PR from editing `static/terminal/*` (D0) etc. | Add a CI check (extend `forbidden-paths-check.yml` or a sibling) that, from the PR's branch-prefix lane (`d0/*`→terminal, `d1/*`→store, …), **fails/warns when a PR touches another D-surface's exclusive paths** — using the §5.5 map as the source of truth; cross-surface seams (§5.5b: shared-fe, retail-chat, trip-planner) trigger a "needs-coordination" label, not a hard fail. B1 owns; operator confirms branch-prefix→lane convention. |

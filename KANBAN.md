...
| D4-OPS-7 | D4 ⏸️ DEFERRED | **⏸️ DEFERRED — free-first go-live excludes payments (operator 2026-05-23).** Scaffold built (mig `20260523170000` + `exos-checkout`/`stripe-webhook`/`exos-connect-onboard` + `lib/checkout.ts`) and stays DORMANT/unapplied — not deployed, no keys. **UI now wired (PR #375), gated on `VITE_STRIPE_PUBLISHABLE_KEY`** so it stays dormant: EventDetails paid tier → `startCheckout` (redirect to Stripe); OrgSettings "Payments" → `startStripeOnboarding`. No key → paid tiers show "Coming soon". D4 runs free-first (comp/issue + transfer + scan). | When resumed — **Operator:** Connect type, fee % (`EXOS_PLATFORM_FEE_BPS`), URLs, `STRIPE_SECRET_KEY`+`STRIPE_WEBHOOK_SECRET`. **A1:** apply mig `170000` + deploy 3 fns (webhook `--no-verify-jwt`). **B1:** review. **Gate:** D1-OPS-5 legal. **Open:** refund-on-sold-out-race. |
...
**Capacity estimate (2026-05-26, modeled — `docs/d4_scaling_rfc.md`):** measured `max_connections=60` (shared w/ broker), PostgREST 8s cap, in-DB ops µs-scale. Comfortable now ≈ few-hundred concurrent users / low-hundreds of claims-per-sec-per-event / hundreds of check-ins/sec; **risk zone = a single-event onsale >~1–2k simultaneous buyers** (hot-counter serialization + 8s cap + shared pool). Confidence LOW–MED until the D4-OPS-34 load test.
...
- **D4-OPS-35 — dedicated Supabase project for D4 (L).** ⏸️ **DEFERRED (operator 2026-05-26): shared SQL for now, own surface eventually.** De-couple from the broker 8M-row firehose + ~75 crons (noisy-neighbor; documented to tip RPCs past 8s). Revisit trigger: before a high-demand paid onsale. Provision → replay `exos_*` → repoint env/secrets → cutover window.
- **D4-OPS-36 — per-surface Render service (S, planned).** Un-mount `/bridge` from the unified app at the beta DNS split.

**Open for operator:** ✅ dedicated DB = shared now / own later (decided 2026-05-26); peak onsale concurrency target (queue sizing for D4-OPS-32); Stripe timeline (sequence with D4-OPS-32/33).

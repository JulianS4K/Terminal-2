...
| **P0** | → `A1-OPS-16..21` | A1 / Op | **Snapshot firehose storage + bloat** — **LIVE 2026-06-19: `listings_snapshots` 48GB/123M, SG listings 13GB/16M, SG sales 7.5GB/12.6M, `espn_injuries` 631MB/36.7k live + 593k DEAD (16:1)** (bible figures 10–13× stale). ~76GB total. | **Fix play: `docs/archive/2026-06-19-snapshot-bloat-play.md`** — (1) pg_repack `espn_injuries` (lossless ~590MB now) → (2) drain via existing sweeps (pollers paused) → Wave 3 change-detection → Wave 4 partition → Wave 2 sales dedup. Data-quality gate: durable daily layer carries history; trim raw beyond ~30d only after verifying no RPC reads it. Apply = A1/operator. |

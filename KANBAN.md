...
| D4-OPS-5 | D4 | **✅ FIXED IN SOURCE (PR #323) — B1 review + A1 apply pending.** Was: `exos_mint_tickets` checked `tier.capacity` but never `exos_events.total_tickets`; `p_tier_id=NULL` minted unlimited. Fix: migration in PR #323 adds hard `total_tickets` cap check + rejects null-tier mints. | **A1:** merge PR #323 → apply per `docs/d4_prod_apply_handoff.md` (free-first batch). |
...
| D4-OPS-7 | D4 + Op (Stripe) | **🟡 SCAFFOLD IN PR #323 — gated on operator creds + Hard-Rule-2 sign-off.** `exos_checkout_sessions` ledger + idempotent `exos_fulfill_checkout` + `exos-checkout`/`stripe-webhook`/`exos-connect-onboard` edge fns all authored. NOT deployed — inert until Op provides Stripe keys (vault) + authorizes Hard-Rule-2 write path. Today you still cannot sell a ticket for money. | **Op:** provide Stripe Connect + `stripe-webhook` secrets. A1 deploys edge fns after B1 sign-off + op auth. D1-OPS-5 (counsel-reviewed Terms/Privacy) is the go-live gate. |
| D4-OPS-8 | D4 + A1 deploy | **✅ FIXED IN SOURCE (PR #323) — B1 review + A1 deploy + Op creds pending.** `exos-mail-drain` edge fn authored (cron-gated, `requireCronSecret`): pulls `pending` rows, sends via Resend, marks `sent`/`failed` with retry. Migration adds drainer infrastructure. Inert until Op provides `RESEND_API_KEY` + `EXOS_MAIL_FROM` in vault. | **A1:** merge PR #323 → apply mig (free-first batch) → deploy `exos-mail-drain`. **Op:** `RESEND_API_KEY` + `EXOS_MAIL_FROM` + cron snippet — all in `docs/d4_prod_apply_handoff.md`. |
...
> **🔑 A1 next action — PR #323 ready to merge + apply.** Phase 2 hardening (scan/caps/buy-limits) + Stripe/distribution scaffolds all authored on `claude/d4-comprehensive-testing-wPECJ`. Executable go-live runbook: **`docs/d4_prod_apply_handoff.md`** (on branch; becomes main after merge). Read it before applying — it covers the free-first apply order, the apply-mechanism gotcha (MCP not CLI), B1 review flag on the check-in DROP+CREATE, operator secrets, and the door-rehearsal gate. B1 review of the scan hardening migration is also requested before apply.
...
6. **D4-OPS-8** `exos_mail` drainer (**M**) — ✅ source-fixed (PR #323). Needs Op email creds + A1 edge-fn deploy.
7. **D4-OPS-5** event-level oversell enforcement (**S**) — ✅ source-fixed (PR #323). A1 applies mig (free-first batch).
8. **D4-OPS-6** server-side check-in HMAC verification (**M**, optional) — scaffold in PR #323; only activate if threat model tightens.
...
9. **D4-OPS-7** Stripe Connect checkout + webhook-driven mint (**L**) — 🟡 scaffold in PR #323 (gated). Op provides Stripe keys; A1 deploys edge fns; D1-OPS-5 (counsel-reviewed Terms/Privacy) is the launch gate. Deliberately last: pre-go-live flip, gates nothing upstream.

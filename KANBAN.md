...
> **Doc version:** v1.2.0 · baseline 2026-05-28 (A1); v1.1.0 2026-05-31 (A1) — added collector-cadence overhaul follow-ups A1-OPS-16..20 (retention Wave 2/3/4, Part D post-event report, evo_discover Phase-2); v1.2.0 2026-06-02 (D0) — Seat Map follow-ups D0-PROD-5..7 (realtime CSP, D1 port, CDN coverage probe); v1.3.0 2026-06-03 (D0) — D0-OPS-1: apply mig 20260603120000 (all-source listing metrics RPC); v1.4.0 2026-06-03 (D0) — D0-PROD-7 solved by seatmap_manifest cache (mig 20260603160000 + edge fn, applied); v1.5.0 2026-06-03 (A1) — added A1-OPS-21 (SG non-owned poller OFF pending re-enable + listings retention halve drain); removed obsolete A1-OPS-7 (floor-sweep ramp — floor sweep retired 05-31); v1.6.0 2026-06-03 (A1) — data-health audit: +2 known-gaps rows (dormant Reddit/SeatData/TickPick; release_health_check false-positives); v1.7.0 2026-06-08 — added A1-OPS-24 (POTENTIAL: last30days social-pulse → why_signals → alerts + scheduled-task bot; exploring, operator-gated); v1.8.0 2026-06-08 (D0) — added D0-FANTASY-1 (Fantasy League feature shipped as files — migrations 20260608210000..210400 + ingest + FE — pending operator apply + branch verification). Append-only board; the section-level version + bot-ref convention → [`README.md`](README.md) *Doc-writing rules*.
...
| `D0` | Terminal FE (consolidated frontend lead) | `static/terminal/*`, terminal-side `app.py` routes |
...
| B1-NEXT-59 | D0/D1 (B1) ✅ RESOLVED 2026-06-08 | **Tier-2** — 4 event-metadata definer views still anon-readable after PR #350: `espn_injury_snapshot_latest`, `seatgeek_event_latest`, `v_event_full_sports`, `v_event_full_v2`. Not revoked in #350 to avoid breaking an unaudited public/SEO event page. | ✅ **RESOLVED** — verified live 2026-06-08: all 4 have **`anon` AND `authenticated` SELECT revoked** (`has_table_privilege` = false for both); 3 are now `security_invoker=on`. No longer anon/auth-readable, consistent with the B1-NEXT-57 detector returning 0. No action needed. |
| B1-NEXT-60 | D2/D0 (B1) | **Tier-3** — post-#350, any *authenticated* user (not only `@s4kent`) still reads the order/intel views (definer, no row filter). Proper fix: `security_invoker=true` + RLS, or `auth.jwt()->>'email'` gate. | Owning-lane migration on the order/intel views. |
...
| A1-OPS-22 | A1 | **`aq_event_map` duplicate-row consolidation** — ~211 `tevo_event_id`s carry ≥2 map rows (PK is `aq_short_event_id`); TBD/`aq_curated` placeholder + later `system_seed` `SYS-*` rows resolve to the same game and never merge, scattering platform IDs so the TD daily snapshot reads only one cluster (e.g. Knicks G3 `3346011`). **16 of these carry _conflicting_ `sg_event_id`s** (same tevo, ≥2 distinct SG ids — often main + parking pseudo-event or split-doubleheaders). `td_reconcile_knicks_finals_aq` already consolidates the Knicks subset; generalize it. | 🔍 **AUDITED 2026-06-08 — NOT superseded; still open + grown.** Live: **235** `tevo_event_id`s with ≥2 map rows (was ~211), **19** with conflicting `sg_event_id`s (was 16); 8,080 total map rows. Only `td_reconcile_knicks_finals_aq` exists (Knicks-only) — no general sweep yet. (A1-OPS-23's matcher fix touched `sg_events_canonical`, a different table, so it didn't reduce these.) **Build plan + RISK:** the safe class (TBD/`system_seed` placeholder dups where sg ids agree or one is NULL) is a mechanical COALESCE-onto-richest-survivor; but the **19 conflicting-sg cases are NOT safe to blind-merge** — they're often main-vs-parking pseudo-events or split doubleheaders (genuinely distinct events). Recommend: general sweep for the safe class + a parking-pseudo filter, and handle the 19 per-case (do NOT COALESCE distinct doubleheader games). Model on `td_reconcile_knicks_finals_aq`. Ready to build on request. |
| A1-OPS-23 | A1 ✅ APPLIED 2026-06-08 | **Upstream: `sg_events_canonical.tevo_event_id` mislinks (SG→TEvo matcher).** Comprehensive audit (2026-06-01) found **11** SG canonical rows whose own `tevo_event_id` is name-inconsistent — the SG matcher (`sg_attempt_event_xref_v3` / `auto_match_sg_canonical_v3`) bound venue+date to the wrong opponent (e.g. SG "Atlanta Braves at San Francisco Giants" → tevo "Athletics at San Francisco Giants"; SG "TBD at Dallas Stars" → "Demi Lovato"). `aq_event_map` is now shielded (mig `20260601184500` name-guards the sg→tevo copy in `backfill_aq_maps`), but the SG layer itself is still wrong → any SG-keyed consumer is affected. | ⏳ **MIGRATION AUTHORED** (`supabase/migrations/20260608194500_sg_matcher_name_league_guard.sql`) — `CREATE OR REPLACE sg_attempt_event_xref_v3` (live body verbatim + a name/league accept-guard mirroring `backfill_aq_maps`) + a self-selecting cleanup that **deletes the 11 wrong `seatgeek_event_xref` rows** (the matcher early-returns on an existing xref, so these must go first) then nulls the 11 `sgc.tevo_event_id`. **Build-audited live: guard rejects exactly the 11 mislinks, 0/3,116 correct matches over-rejected.** ✅ **APPLIED to prod 2026-06-08 (via MCP); post-apply verified mislinked_remaining = 0 + guard live.** Re-match of the unbound 11 left to the hourly `auto_match_sg_canonical_v3` cron. Residual (still tracked): same-city *same-league* "TBD at X" placeholder collisions, which neither guard catches. |
...
| D0-FANTASY-1 | D0 ✅ authored · A1 apply · Op render | **Fantasy League feature (NEW surface) — authored as files, PENDING operator apply + branch verification.** Full H2H weekly fantasy for NFL/NBA/MLB/NHL/WNBA, drafted from `espn_athletes` rosters, scored from REAL player box scores. **Phase 1 (data foundation):** `player_game_stats` + `athlete_external_ids` tables (mig `20260608210000`) + `fantasy_ingest.py` (pulls per-player box scores from open-source `sportsdataverse`; reuses canonical team mappers `espn_teams_canonical`/`performer_espn_team_xref` for the crosswalk; MLB resolves by name+team) + `requirements-fantasy.txt` + Render cron `render-fantasy-ingest.yaml`. **Phase 2 (product):** `fantasy_*` tables + RLS + `is_league_member()` + `v_fantasy_standings` (mig `210100`); create/join/snake-draft/pick/lineup + H2H schedule-gen RPCs (`210200`); pluggable scoring engine `player_box_v1` + `team_result_v1` fallback (`210300`); in-DB scoring cron `fantasy-score-hourly` @ `:48` (`210400`). `/api/fantasy/*` read routes in `app.py`; `static/terminal/fantasy.html`+`fantasy.js` + nav entry. Read-only lockdown clean (`check_readonly.py` green; all fantasy data is internal Postgres). | **A1:** apply migs `20260608210000`→`210400` on a branch first; `get_advisors` clean; seed a 2-mgr league, run draft + `fantasy_score_period` against real `player_game_stats`, verify idempotency + standings; then prod-apply. **Op:** provision the `vibepass-fantasy-ingest` Render cron (set `SUPABASE_URL`+`SERVICE_ROLE_KEY`), run `fantasy_ingest --mode backfill` once per league. **Known v1 limit:** MLB athlete crosswalk is name+team (unresolved players keep MLBAM id, espn_athlete_id NULL) — Chadwick `playerid_lookup` is a follow-up. **Verification caveats (do FIRST on the branch):** (1) the SQL has NOT been executed anywhere (no local Postgres in the authoring env) — the Supabase-branch apply is the real syntax/logic check, especially the scoring engine (`fantasy_score_period`/`fantasy_compute_player_box`) + the circle-method schedule generator (`fantasy_generate_schedule`); (2) `fantasy_ingest.py`'s per-league `COLUMN_MAP` (sportsdataverse loader fn + DataFrame column names) is best-effort vs the documented API — confirm against the installed package on the first `--mode backfill` run (mappings are deliberately isolated for a localized fix). PR #495 (draft). |
...
| A1-OPS-5 | A1 ✅ FIXED | **`/health` endpoint doesn't exist** — the live health check is `/healthz`, not `/health`. | ✅ **FIXED** — the only live stale reference was the rate-limit middleware exemption in `app.py` (`path in ("/", "/health")`), which left Render's real `/healthz` poll subject to the per-IP cap. Changed to `/healthz`. (`/api/d2/health` is a separate real D2 route — left as-is; render.yaml already uses `/healthz`.) |
...
| A1-OPS-9 | A1 ✅ VERIFIED RESOLVED 2026-06-08 | **`td_pull_drain` re-fires soft-cancelled rows.** Drain selects `WHERE fired_at IS NULL` only — a row with `resolved_at` set but `fired_at` NULL stays fireable + gets re-fetched (normalize skips it = wasted upstream call). The 23 existing stale rows were neutralized 2026-05-29; the function gap remains. | ✅ **ALREADY FIXED** — the live `td_pull_drain` SELECT already reads `WHERE q.fired_at IS NULL AND q.resolved_at IS NULL` (verified via `pg_get_functiondef` 2026-06-08). The function gap described here no longer exists; no migration needed. |
...
| A1-OPS-11 | D0 ✅ FIXED | **TM verified-resale split not in `event.js` TD Listings tab.** Market Carpet (performer + venue) now surfaces TM PRIMARY **and** RESALE (`td_tm_resale_*`) + a TM-R÷TM premium spread — A1, commit `52d57ff` ✓. But D0's merged `event.js` predates mig 220000, so its TD Listings tab shows TM primary only, not the verified-resale dataset. | ✅ **FIXED** — `get_event_td_listings` already returns `inventory_type` (`raw->'offer'->>'inventoryType'`), so no RPC change needed; `loadTdPlatformListings()` now splits the TM summary into FACE (primary) vs RESALE get-in/median and adds an "Inv" column (Face/Resale) — shown only for platforms carrying the discriminator (TM). SH/GT/VD/TP unchanged (single stat). |
...
| A1-OPS-13 | A1 ✅ APPLIED 2026-06-08 | **TD 402 quota-exhaustion handler doesn't disable the TP/TM enqueue crons.** `td_normalize_drain`'s 402 branch disables `td_enqueue_peak_sh/_vd/_gt` only — `td_enqueue_peak_tp` + `td_enqueue_peak_tm` keep enqueuing after quota exhaustion (wasteful + inconsistent). | ✅ **APPLIED to prod 2026-06-08** (mig `20260608193000`, via MCP). `td_normalize_drain` 402-branch now also disables `td_enqueue_peak_tp`/`_tm` — verified live (`prosrc` contains both). |
...
| A1-OPS-21 | A1 / Op | **SG non-owned listings poller is OFF + raw-listings retention drain in flight** (migs `20260603180000` + `…190000`, PR #423, applied 2026-06-03). (a) `sg_listings_poll_nonowned_5min` is wired but `cron_policy.enabled=false` per operator ("only poll owned for now"); owned (`sg_listings_poll_owned_1min`) is ON. Non-owned has a ~1,700-event poll backlog parked. (b) `sweep_old_listings(15)` is draining the EVO `listings_snapshots` 15–30d backlog (~9 GB / half the 18 GB table) over ~1–3 weeks via the 80s-bounded daily run. | (a) Decide when to re-enable non-owned: `UPDATE cron_policy SET enabled=true WHERE jobname='sg_listings_poll_nonowned_5min'` — first day draws harder on the shared SG token to clear the backlog (consider a temporary `p_max` bump). (b) Retention drain self-completes; no action unless faster reclaim wanted (temp hourly sweep or batched purge + autovacuum). Mirroring the SG owned/non cadence split onto EVO is an open option (EVO has no rate limit, so storage-only benefit). |
...
|---|---|---|---|
| EVAL-1 | [`Egonex-AI/Understand-Anything`](https://github.com/Egonex-AI/Understand-Anything) | Claude Code plugin: turns a codebase into an interactive knowledge graph (tree-sitter static analysis + LLM agents). MIT. Tagline "Graphs that teach > graphs that impress." | **Plausibly useful** as a dev/onboarding aid for *this* repo (533 migrations, 132 tables, 5k-line `event.js`/`app.py`) — could shorten per-session codebase comprehension. Low risk (read-only analysis tool). Verdict TBD: try on a worktree, judge signal vs token cost. |

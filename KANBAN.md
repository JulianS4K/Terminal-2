...
- **Metric discipline (2026-08-31)**: the daily table is NOT one fee basis — its populate fn computes every TD column from `list_price` except TickPick (`price_with_fees`, mig `20260606160000`), while the live listing layer is `price_with_fees` throughout. Prices now carry a `basis` and are ranked only **within** one (an all-in $43 beats a pre-fee $38 that costs $46 at the door); the UI tags every number ALL-IN / PRE-FEE. The "vs usual" verdict is measured on a single **anchor marketplace** (most days of coverage in the window) on both sides, instead of a per-day cheapest-of-whatever-existed: with ~98.5% of daily rows carrying one source, coverage flickers were moving the baseline and firing bogus "great price" verdicts. Two regression tests lock both behaviours.
- **Wired in**: event page hero → `COMPARE ON FLIGHTS →` badge, new `flights.html?event=<id>` focus mode (single event, expanded, date grid stood down), and a FLIGHTS card on the `/home/` hub (gated with the other operator surfaces).
- **No schema change** — reads existing tables only; no migration, cron, or upstream fetch. Price tracking is client-side; a durable alerting watchlist would be a separate data-plane change. The DB's own mixed-basis `td_*_getin` columns are left alone deliberately: mig `20260606160000` kept SH/GT/VD/TM on `list_price` so their historical series stay continuous, so this labels the basis rather than rewriting history.
- **Tests**: `tests/test_flights_routes.py` (39 cases, 100% line+branch on the new module) + the page added to the Playwright smoke inventory.

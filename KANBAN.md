...
| A1-OPS-11 | D0 | **Ticketmaster FE not wired.** TD now pulls TM **PRIMARY** (box-office) listings for focus events (`platform='TM'`, 21 events live 2026-05-29 — 20 NYY home + 1 Knicks Finals) into `ticketsdata_listings_snapshots`; per-event metrics in `event_listing_snapshot_daily.td_tm_listings/td_tm_median`. `event.js` TD panels / Market Carpet don't surface `TM`. | D0: add `TM`/Ticketmaster to the TD Listings tab + Market Carpet source list (same pattern as A1-OPS-8/TP). **TM is TWO datasets** — surface PRIMARY (`td_tm_listings/td_tm_median`) and RESALE (`td_tm_resale_listings/td_tm_resale_median`) separately (resale appears on sold-out/NBA games, e.g. Knicks Finals once on-sale; Yankees are primary-only). Data live; FE-only. |

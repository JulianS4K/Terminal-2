...
> **Doc version:** v1.67.0 (2026-06-28) — latest: hub (`/` landing) verify + rewire, 22 frontend tests green. Board is append-only; full prior doc-version history → [`docs/archive/2026-07-02-doc-version-history.md`](docs/archive/2026-07-02-doc-version-history.md). Convention → [`README.md`](README.md) *Doc-writing rules*.
...
| C1-OPS-3 | C1 / Op | **Tier-2 lane consolidation (governance model — follow-up to the 2026-07-02 §2 convergence, PR #729).** §2.3 now models ownership as a **platform-substrate (A1/B1/C1) × products (D0–D4)** grid and already annotates the two vestigial cells: **D2** (orders dashboard, writes no tables, broker-facing) folds toward **D0**, and **C1** is a surface tag whose work is the **Auditor** principal, not a distinct actor. The lane *codes* were deliberately NOT renamed — a rename would hit the `bot_chat_bot_level_check` DB constraint, every `A1-OPS-*`/`D0-PROD-*` KANBAN ID, branch prefixes, `labeler.yml`, and pr-title-lint (huge blast radius, ~zero benefit). **Open (needs operator sign-off — it touches CI + conventions, not just docs):** (1) fold `d2_dashboard/*` ownership D2→D0 in `labeler.yml` + `surface-boundary-check.yml` (keep the path, re-own it); (2) formally retire C1 as a first-class lane → Auditor-principal-only, reconciling the `docs/*` self-contracts + `docs/templates/LANE_DISCIPLINE.template.md` (overlaps C1-OPS-2's drift sweep). Net: first-class lanes 8→~6. | **C1:** draft as its own PR, reviewed independently of the model reframe. **Op:** sign off on the 8→~6 reduction before the CI/labeler edits land. |

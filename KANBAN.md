...
- **[B1-NEXT-25] [SEC-LOW]** Periodic full-pass drift audit of `RESOURCES_BIBLE.md` §1 (external services) — verify Render service IDs match `mcp__render__list_services`, vault secret names match `get_app_secret` allowlist, lane ownership rows match `PROJECT_BIBLE.md §2`. *(Objective table/view/fn/cron/edge counts now auto-generated — `RESOURCES_INVENTORY.generated.md` via `scripts/gen_inventory.py`; wire `--check` into CI to kill count-drift permanently.)* Rotating quarterly.

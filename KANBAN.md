...
| ID | Lane | Task | Action |
|---|---|---|---|
| A1-OPS-30 | A1 / Op | **Our SG SellerDirect SALES are ingest-dark — the "SG selling, we're not" stall chart can't see genuine SG-channel sales (found 2026-07-09, PR for `sg_owned_stall_chart`).** Three stacked gaps: (1) `sg-seller-webhook` edge fn exists in-repo but is **NOT deployed** (absent from the 42 live fns); (2) its idempotency table `sg_seller_webhook_notifications_seen` (mig `20260516210000`) was **never applied to prod** — deploying the fn without it would error every notification; (3) `sg_seller_orders_queue_30min` cron is inactive (deliberately not resumed in mig `20260620120000`) and `seatgeek_orders` has nothing newer than 2025-06. Net: an event genuinely selling via our SG seller account shows as "stalled" on the new home panel (its EVO/Vivid/TickPick channels are the only own-sales the chart can observe — stated in the panel tooltip + mig header). Related known-state: the whole SG LISTINGS path stays deliberately dark since 06-26 (`RESOURCES_BIBLE §5` SG state), so SG-side owned listings render as last-known info only. | **Operator decision (Applier actions):** either (a) accept SG-channel sales blindness for now (chart stays conservative — may over-flag), or (b) relight our-SG-sales ingest: apply mig `20260516210000`, deploy `sg-seller-webhook` (+ secret check), and/or `cron_resume_with_gate('sg_seller_orders_queue_30min')`. The stall chart heals automatically once `seatgeek_orders` flows — its union already includes the source. |

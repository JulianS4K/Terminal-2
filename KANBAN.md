...
> **Doc version:** v1.1.0 · baseline 2026-05-28 (A1); v1.1.0 2026-05-31 (A1) — added collector-cadence overhaul follow-ups A1-OPS-16..20 (retention Wave 2/3/4, Part D post-event report, evo_discover Phase-2). Append-only board; the section-level version + bot-ref convention → [`README.md`](README.md) *Doc-writing rules*.
...
| A1-OPS-21 | A1 | **`aq_event_map` false-match residual** — mig `20260601173000` added `aq_name_consistent()` guard to `resolve_aq_tevo_from_sources` + `backfill_aq_maps` step 2 and unbound 12 upcoming false matches (incl. Forrest Frank→Knicks G4). Residual NOT caught: **same-city placeholder collisions** where away side is `TBD` and the home *city* token matches across leagues (verified: `W44YJ9D4` "TBD at Minnesota Timberwolves (If Necessary)" still bound to a Minnesota **Lynx** WNBA game `3286122`). The token guard passes it (shared "minnesota", away=TBD skipped). | Add league/team-level validation (e.g. require `event_xref.espn_league` agreement, or block cross-category venue+date binds) — or explicitly unbind the handful of cross-league `TBD at <team>` placeholders. Low harm (placeholders, mostly `(If Necessary)`). |
| A1-OPS-22 | A1 | **`aq_event_map` duplicate-row consolidation** — 221 `tevo_event_id`s carry ≥2 map rows (PK is `aq_short_event_id`); TBD/`aq_curated` placeholder + later `system_seed` `SYS-*` rows resolve to the same game and never merge, scattering platform IDs so the TD daily snapshot reads only one cluster (e.g. Knicks G3 `3346011`). `td_reconcile_knicks_finals_aq` already consolidates the Knicks subset; generalize it. | Author a general dedupe sweep: per `tevo_event_id`, pick the richest-platform survivor, `COALESCE` the other rows' sg/sh/vivid/tm/tp ids + TD xref `aq_short_event_id` onto it, retire the loser. Model on `td_reconcile_knicks_finals_aq`. |

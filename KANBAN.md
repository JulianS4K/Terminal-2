...
| A1-OPS-23 | A1 | **Upstream: `sg_events_canonical.tevo_event_id` mislinks (SG→TEvo matcher).** Comprehensive audit (2026-06-01) found **11** SG canonical rows whose own `tevo_event_id` is name-inconsistent — the SG matcher (`sg_attempt_event_xref_v3` / `auto_match_sg_canonical_v3`) bound venue+date to the wrong opponent (e.g. SG "Atlanta Braves at San Francisco Giants" → tevo "Athletics at San Francisco Giants"; SG "TBD at Dallas Stars" → "Demi Lovato"). `aq_event_map` is now shielded (mig `20260601184500` name-guards the sg→tevo copy in `backfill_aq_maps`), but the SG layer itself is still wrong → any SG-keyed consumer is affected. | Port `aq_name_consistent()` + `aq_league_consistent()` into the SG matcher's accept criteria, and one-time-null the 11 mislinked `sg_events_canonical.tevo_event_id`s. Detector: `sg_events_canonical sgc JOIN events e ON e.id=sgc.tevo_event_id WHERE NOT public.aq_name_consistent(e.name, sgc.sg_event_name)`. |
...
| A1-OPS-13 | A1 ⏳ AUTHORED, pending apply | **TD 402 quota-exhaustion handler doesn't disable the TP/TM enqueue crons.** `td_normalize_drain`'s 402 branch disables `td_enqueue_peak_sh/_vd/_gt` only — `td_enqueue_peak_tp` + `td_enqueue_peak_tm` keep enqueuing after quota exhaustion (wasteful + inconsistent). | ⏳ **MIGRATION AUTHORED** (`supabase/migrations/20260608193000_td_402_disable_tp_tm_enqueue.sql`) — `CREATE OR REPLACE td_normalize_drain` reproduces the live body verbatim + adds `td_enqueue_peak_tp`/`_tm` to the 402-branch list (both confirmed live + active 2026-06-08). **Awaiting operator `apply_migration`** — do NOT merge before apply (repo↔prod drift, per A1-OPS-12). |

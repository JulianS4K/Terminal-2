...
| **P2** | A1-OPS-SRC1 | A1 / Op | **Sourcing-cron redesign — TD credit control.** Phase 1 SHIPPED (AXS credit instrumentation, mig 20260630160000): AXS now logs to `ticketsdata_credit_usage` at ingest, true cost confirmed **≈1/fetch** (the prior "~46/fetch / ~10k-day AXS" was a quota-counter contention artifact; 3 counter groups `{SH}`/`{VD,GT,TM}`/`{AXS}`). New views `v_td_credit_usage_by_platform`/`_mtd`/`v_td_credit_cost_per_call`. | **Phase 2 (next):** per-platform `collector_cadence` breadth ladder (SH/VD all bands · GT ≤30d · TP ≤14d · SG listings ≤7d) + peak/off-peak two-schedule crons + retire dead `td_poll_policy` tier crons + TM→MSG-only (venue 896) primary track + SG broker = discovery-first (all sales, listings ≤7d, off TD budget). **Phase 3 (data-gated ~3–7d):** per-platform budget split (`td_credits_by_platform`/`td_budget_split_ok`). Full plan: `.claude/plans/solutions-buzzing-riddle.md`. |

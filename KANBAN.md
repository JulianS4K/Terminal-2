...
> **Doc version:** v1.2.0 · baseline 2026-05-28 (A1); v1.1.0 2026-05-31 (A1) — added collector-cadence overhaul follow-ups A1-OPS-16..20 (retention Wave 2/3/4, Part D post-event report, evo_discover Phase-2); v1.2.0 2026-06-02 (D0) — Seat Map follow-ups D0-PROD-5..7 (realtime CSP, D1 port, CDN coverage probe); v1.3.0 2026-06-03 (D0) — D0-OPS-1: apply mig 20260603120000 (all-source listing metrics RPC). Append-only board; the section-level version + bot-ref convention → [`README.md`](README.md) *Doc-writing rules*.
...
| D0-PROD-7 | D0 | **Seat Map real CDN coverage unknown.** Only `venue_id`+`configuration_id` presence (≈100% upcoming) and `v_broker_configurations.fanvenues_key` (≈79 configs) are known in-DB; the true count of venue/config pairs `maps.ticketevolution.com` actually serves needs a browser probe (CDN egress-blocked from CI/agent). | Run the manifest-probe console snippet (logged 2026-06-02) across distinct upcoming pairs; record the real hit count + by-venue-tier breakdown. Then optionally hide the Seat Map tab for events with no map. Low effort. |

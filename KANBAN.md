...
| A1-OPS-33 | A1 | **Four crons disabled 2026-08-31 at a 100% failure rate — each needs a real fix before it goes back on (mig `20260831100000`).** All four burned their full 900s budget every run and returned nothing: `compute_alerts_tick_15min` (96/96 failed, dies in the `INSERT INTO event_alerts` / `_alert_dedupe_ok` path), `sweep_expired_pg_net_pending_hourly` (24/24, its `NOT EXISTS (… net._http_response WHERE h.id = p.request_id)` is a correlated scan of an unindexed 61 GB table — see A1-OPS-32), `venue_section_rollup_hourly` (24/24, `DISTINCT ON` over `section_metrics`, 43 GB/176 M rows), `listings_deltas_backfill_overnight` (15/15, `listings_deltas_backfill_chunked`). | Root-cause each one (most are the A1-OPS-32 scan or an unbounded window), then `cron.alter_job(<id>, active := true)`. IDs: 149, 191, 479, 229. Do NOT bulk re-enable — they saturated the cron pool. |
| A1-OPS-32 | A1 / Op | **`net._http_response` is ~300x bloated and pg_net leaves it unindexed on `id` — the root cause of the 2026-08-31 memory saturation (mig `20260831100000`).** The table hit 61 GB (heap 9.9 GB + 51 GB TOAST) for ~173k rows <6 h old; autovacuum had run **twice in its lifetime** (workers permanently busy on the 100 GB+ snapshot tables). pg_net ships only a `created` index, so all **55** of our `*_process()`/`*_drain()` consumers seq-scan it via `JOIN … ON h.id = <pending>.request_id`: 204,764 seq scans / **14.2 B tuples**; a single `WHERE id = <lit>` EXPLAIN did not return in 60 s. That is why ~10 unrelated drains all died at the 900 s cap in a `FOR over SELECT rows` loop. **We cannot add the index** — `net` + the table are owned by `supabase_admin`; `postgres` has SELECT/UPDATE/DELETE + MAINTAIN but no ownership or schema CREATE. Mitigated by `pg_net_reap()` (2 h retention, `*/10`) + cron overlap guards. | **(a) ✅ DONE 2026-08-31 18:55Z** — `VACUUM (FULL, ANALYZE)` reclaimed **62 GB → 300 MB** (heap 9,922 MB → 74 MB, TOAST 51 GB → 223 MB). The unindexed `WHERE id = <lit>` lookup went from *not returning in 60 s* to **87 ms** (9,442 buffers vs 1.27 M pages). pg_cron backends 20.3 avg → **1**; stalled >5 min 16 → **0**. It had to be run after the throttled TOAST autovacuum finished (1 h 33 m), and its 4 blocking drains cancelled so it did not burn its 900 s budget waiting. **(b) Operator:** ask Supabase support to add `net._http_response (id)` — it is the permanent fix and only they can. **(c)** Consider raising `autovacuum_max_workers` above 3. |

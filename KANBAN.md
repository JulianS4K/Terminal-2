...
> **Doc version:** v1.15.0 · baseline 2026-05-28 (A1); v1.1.0 2026-05-31 (A1) — added collector-cadence overhaul follow-ups A1-OPS-16..20 (retention Wave 2/3/4, Part D post-event report, evo_discover Phase-2); v1.2.0 2026-06-02 (D0) — Seat Map follow-ups D0-PROD-5..7 (realtime CSP, D1 port, CDN coverage probe); v1.3.0 2026-06-03 (D0) — D0-OPS-1: apply mig 20260603120000 (all-source listing metrics RPC); v1.4.0 2026-06-03 (D0) — D0-PROD-7 solved by seatmap_manifest cache (mig 20260603160000 + edge fn, applied); v1.5.0 2026-06-03 (A1) — added A1-OPS-21 (SG non-owned poller OFF pending re-enable + listings retention halve drain); removed obsolete A1-OPS-7 (floor-sweep ramp — floor sweep retired 05-31); v1.6.0 2026-06-03 (A1) — data-health audit: +2 known-gaps rows (dormant Reddit/SeatData/TickPick; release_health_check false-positives); v1.7.0 2026-06-08 — added A1-OPS-24 (POTENTIAL: last30days social-pulse → why_signals → alerts + scheduled-task bot; exploring, operator-gated); v1.8.0 2026-06-08 (D0) — added D0-PROD-8 (per-performer Trip Planner shipped via #463/#475/#477/#478/#483; refinement backlog: unpriced-event pricing, real tile map, D1 parity, multi-performer mode, value weighting); v1.9.0 2026-06-08 — **cleanup: removed 6 fully-resolved OPEN-WORK rows** per the closure protocol — `A1-OPS-5` (/healthz), `A1-OPS-9` (td_pull_drain already-guarded), `A1-OPS-11` (TM resale split), `A1-OPS-13` (TD 402 tp/tm, applied), `A1-OPS-23` (SG matcher guard, applied), `B1-NEXT-59` (4 definer views revoked); detail in their merged PRs (#465-489) + applied migs (MIGRATION_CONVENTIONS §14). `B1-NEXT-55` kept (regression fixed, batch-REVOKE still open); v1.10.0 2026-06-10 (A1) — added A1-OPS-25 (AXS primary box-office: data collection LIVE via #511/#533; checkout/order-substitution automation PARKED, operator-gated §2 security-CRIT); v1.11.0 2026-06-17 (C1) — added C1-OPS-1 (workflow-skills rollout backlog from the git audit; ship-a-migration + router landed PR #570); v1.12.0 2026-06-17 (operator) — added C1-OPS-2 (4-domain reorg follow-ups: own-bible CI wiring, peripheral drift sweep, create per-bot bibles); v1.13.0 2026-06-17 (operator) — added the **🌿 BRANCHES / PRs IN FLIGHT** git tracker (19 open PRs by lane + orphan-branch note) so the board tracks both work items and the live git surface; v1.14.0 2026-06-19 (A1) — added A1-OPS-26 (port the `sg-to-tevo-search-bridge` name-overlap accept-guard from PR #616 into the sibling `aq-to-tevo-search-bridge` / `match-sg-performers-to-tevo` binders; flags the #613/#616 "A1-OPS-24" ID mislabel); v1.15.0 2026-06-19 (A1) — **closed A1-OPS-26**: `aq-to-tevo-search-bridge` name-overlap guard shipped + deployed v3 (mig 20260619230000 unbound 4 venue+date-coincidence mis-binds — Forrest Frank→NBA Finals, Salsa→LA Phil, Corridos→Lupillo Rivera, Bucs Season Tix→Bruno Mars); `match-sg-performers-to-tevo` audited = SAFE (performer matcher, name-confidence-gated, no venue+date hole); detector now 0 across all binders; v1.16.0 2026-06-22 (A1) — corrected A1-OPS-21(b): the `sweep_old_listings` retention drain was silently broken (`column "id" does not exist`) so `listings_snapshots` grew to 45 GB/121 M rows; fixed (delete by `captured_at` via `ctid`) + branch-tested + applied to prod (PR #568); v1.17.0 2026-06-25 (B1) — **closed BR-CODE-3**: product code driven to 100% line+branch coverage, coverage tooling + CI ratchet gate added, 2 latent bugs fixed (PR #685); v1.18.0 2026-06-25 (B1) — bug-hunt sweep over the now-covered surface: fixed 6 correctness bugs (reserve sold-out 0-falsy, tours/multi 500-on-bad-id, `_last5` snapshot double-count, seatdata gzip + epoch-string, seatgeek cursor no-progress) + filed BUGHUNT-1 (NULL-key upsert, needs DB migration) / BUGHUNT-2 (holiday-window product call); v1.19.0 2026-06-25 (D0) — BR-CODE-1 slice 13: `routers/shares.py` extracted (4 share routes), app.py → ~6489 LOC, 100% coverage held; v1.20.0 2026-06-25 (D0) — BR-CODE-1 slice 14: `routers/seatmap.py` extracted (3 seatmap routes + fetch helper), app.py → ~6373 LOC, 100% held; v1.21.0 2026-06-26 (B1) — app.py renamed → server.py (entrypoint uvicorn server:app); README + all bibles + current KANBAN refs updated (historical archive rows left intact); v1.22.0 2026-06-26 (D0) — BR-CODE-1 slice 15: routers/retail_chat.py extracted (2 routes + helpers), server.py → ~6284 LOC, 100% held; v1.23.0 2026-06-26 (D0) — BR-CODE-1 slice 16: core/store_events.py (2 leaf SQL-only helpers extracted), 100% held; v1.24.0 2026-06-26 (D0) — BR-CODE-1 slice 17: +2 leaf helpers to core/store_events.py (build_zone_resolver, fetch_owned_ticket_groups), 100% held; v1.25.0 2026-06-26 (D0) — BR-CODE-1 slice 18 (KEYSTONE): _resolve_event_with_filters → core/store_events.py via dependency injection, server.py → ~5779 LOC, 100% held; v1.26.0 2026-06-26 (D0) — BR-CODE-1 slice 19: routers/store.py (event-detail + zones lifted, first post-keystone), 100% held; v1.27.0 2026-06-26 (D0) — BR-CODE-1 slice 20: /api/store/concierge → routers/store.py, 100% held; v1.29.0 2026-06-26 (D0) — BR-CODE-1 slice 21: 4 store trip-planner delegate routes → routers/store.py, 100% held; v1.28.0 2026-06-26 (B1) — added docs/production_readiness.md (operator launch-blocker punch-list) + pointer in the Operator-only section; v1.30.0 2026-06-26 (B1) — BR-CODE-1 slice 22: the 4 storefront discovery routes (`/api/store/search` · `/movers` · `/home` · `/events/near`) lifted into routers/store.py behind unchanged paths — discovery getters bind the live caches (`_search_cache`/`_movers_cache`/`_MOVERS_CACHE_REFRESHING`) + heavy helpers (`_search_*`/`_attach_owned_metadata`/`_refresh_movers`/`_compute_movers`) that stay in server.py; `/events/near` declared before the `{event_id}` catch-all (Starlette match-order — else "near" 422s as an int path-param); server.py → ~5083 LOC, 100% line+branch held; v1.31.0 2026-06-26 (B1) — BR-CODE-1 slice 23: the 4 broker trip-planner delegate routes (`/api/broker/performers/{id}/trip-plan|tour-package`, `/tours/multi|near`) lifted into routers/broker.py — the D0 authed twins of the slice-21 D1 store routes; payload builders (`_trip_plan_payload`/`_tour_package_payload`/`_multi_tour_payload`/`_discover_payload`) stay in server.py, shared with the store routes + passed to both routers via getters; server.py → ~5015 LOC, 100% held; v1.32.0 2026-06-26 (B1) — BR-CODE-1 slice 24: 3 standalone DB-read broker routes (`/api/broker/news`, `/performers/by-league/{league}`, `/event/{id}/raw-tevo`) lifted into routers/broker.py — no in-server helper coupling (raw-tevo adds a `get_client` getter for the live read-through); server.py → ~4871 LOC (under 5k), 100% held; v1.33.0 2026-06-26 (B1) — BR-CODE-1 slice 25: 3 more standalone DB-read broker routes (`/event/{id}/section-metrics`, `/watchlist-movers`, `/event/{id}/orders`) lifted into routers/broker.py — require_sb only + the pure core delta/cadence helpers; server.py → ~4710 LOC, 100% held; v1.34.0 2026-06-26 (B1) — production-readiness resilience pass: P0 #2 Supabase per-request timeout (`core/db.py`), P1 #4 circuit breaker + retry + storefront stale-read (`core/resilience.py`, `/healthz db_circuit`, store_home via `safe_sb_read`), P1 #5 Redis-or-in-process rate limiter (`core/ratelimit.py`); **BUGHUNT-1 fix authored + tested** (mig `20260626120000` → non-null dedup key `UNIQUE (sg_event_id, content_hash)` + upsert repoint, dry-run-verified 0 dups / already-unique — **prod apply HELD pending operator go**); 100% line+branch held throughout; v1.35.0 2026-06-26 (B1) — BR-CODE-1 slice 26: `/api/broker/event/{id}/overview` lifted into routers/broker.py (cleaner than expected — its big helper `_bulk_event_context` was already in core; route only needs require_sb + pure core delta/cadence helpers + `_bulk_performer_assets` via a new getter); server.py → ~4637 LOC, 100% held; v1.36.0 2026-06-26 (B1) — BR-CODE-1 slice 27: `/api/broker/event/{id}/section-zones` + `/zones` lifted into routers/broker.py (require_sb + the `_PARKING_*` regexes, both moved into the router); server.py → ~4466 LOC, 100% held; v1.37.0 2026-06-26 (B1) — BR-CODE-1 slice 28: `/api/broker/performer/{id}/espn` lifted into routers/broker.py (require_sb + stdlib datetime only — ESPN team/injury/news/recent reads); server.py → ~4384 LOC, 100% held; v1.38.0 2026-06-26 (B1) — BR-CODE-1 slice 29: `/api/broker/event/{id}/chart-data` (the ~485-line self-contained route, all `_series`/`_stand_series`/`_team_index_series`/etc. nested inside) lifted into routers/broker.py — require_sb the only external dep; server.py → ~3902 LOC, 100% held; v1.39.0 2026-06-26 (B1) — BR-CODE-1 slice 30 (FINAL broker route): `/api/broker/movers` + its `_broker_movers_v2` helper lifted into routers/broker.py — **the entire `/api/broker/*` surface now lives in routers/broker.py**; server.py → ~3655 LOC (from 6,705 at session start, −45%), 100% held; v1.40.0 2026-06-26 (B1) — BR-CODE-1 slice 31: `/api/performers` + `/api/venues` (search) lifted into routers/catalog.py (TEvo client passthroughs, joining their `/{id}` siblings already there; `get_client` typed `Any` → cleaned the router's pre-existing mypy noise too); server.py → ~3612 LOC, 100% held; v1.41.0 2026-06-26 (B1) — BR-CODE-1 slice 32: the 3 SeatGeek POSTs (`/event/{id}/link`, `/sync-listings`, `/sync-sales`) lifted into routers/seatgeek.py (completes that router — reads already moved in slice 9; `get_sg_client` getter resolves the live `_get_seatgeek_client`; RULE-2 audit stays clean — GET-from-SG, write-our-DB only); server.py → ~3538 LOC, 100% held; v1.42.0 2026-06-26 (B1) — BR-CODE-1 slice 33: the 3 SeatData POSTs (`/event/{id}/link`, `/auto-search`, `/sync-sales`) lifted into routers/seatdata.py (completes that router; `require_cron_or_auth` passed in for the pg_cron-driven sync routes; RULE-2 clean — GET-from-SD, write-our-DB only); server.py → ~3455 LOC, 100% held. **All SG + SD ingest routers now complete.**; v1.43.0 2026-06-26 (B1) — BR-CODE-1 slice 34: `/api/store/events` (the ~230-line TEvo catalog GET — the last orphaned store route) lifted into routers/store.py via scripted reindent; deps all already-wired getters (get_client/get_bulk_performer_assets/get_require_sb) + a new `get_tevo_office_id` getter (caught: the route used the `TEVO_OFFICE_ID` server constant). **The entire `/api/store/*` surface now lives in routers/store.py.** server.py → ~3226 LOC, 100% held; v1.44.0 2026-06-26 (B1) — BR-CODE-1 slice 35: `/api/watchlist` POST + DELETE (mutations) lifted into routers/lists.py (joins the watchlist GET there — require_sb + require_auth only); the whole `/api/watchlist` + `/api/runs` + `/api/snapshots/*` surface now lives in lists.py; server.py → ~3198 LOC, 100% held; v1.45.0 2026-06-26 (B1) — BR-CODE-1 slice 36: the 3 event-detail routes (`/api/events/{id}` + `/series` + `/sections/series`) lifted into routers/catalog.py (get_client + a new `get_require_sb` getter for the time-series DB reads + core `is_event_seat`); the whole `/api/events*` + performer/venue/configuration catalog surface now lives in catalog.py; server.py → ~3049 LOC, 100% held; v1.46.0 2026-06-26 (B1) — BR-CODE-1 slice 37: `/api/portfolio` (the ~214-line owned-inventory aggregation, pure require_sb — no client/helper coupling) lifted into routers/broker.py; server.py → ~2834 LOC (under 3k, from 6,705 = −58%), 100% held; v1.52.0 2026-06-26 (B1) — BR-CODE-1 slice 38: the 2 bot-gated store POSTs (`/api/store/verify-human` + `/reserve`) lifted into routers/store.py — getters resolve the patched server symbols (RECAPTCHA_ENABLED/_verify_recaptcha/_require_human/STOREFRONT_RESERVE_REQUIRES_AUTH/_fetch_owned_ticket_groups_from_db + require_auth-via-getter for reserve's direct call); cookie helpers import from core.auth; reserve stays a MOCK (no /v9/orders). **The entire `/api/store/*` surface now lives in routers/store.py.** server.py → ~2733 LOC, 100% held; v1.47.0 2026-06-26 (B1) — **BR-CODE-2 slice 1** (client dedup begins): `core/readonly_guard.py` single-sources the RULE-2 `_assert_readonly_method` guard across the 8 GET-only `*_client.py` (was 8 hand-rolled copies); all audit tokens + per-client error classes + test imports preserved; 50 guard tests + static audit green, 100% held; v1.48.0 2026-06-26 (B1) — **BR-CODE-2 slice 2** (retry dedup): `core/http_retry.py::fetch_with_retry` centralizes the 429/5xx Retry-After+backoff loop; evo/tickpick/vivid/gotickets migrated (each keeps its transport/auth thunk + bespoke parse), 100% held + RULE-2 clean; v1.49.0 2026-06-26 (B1) — BR-CODE-2 slice 2 cont.: seatgeek (both loops) + seatdata migrated to `fetch_with_retry` (jitter + session + tuple/gzip returns preserved; test fakes gained `.ok`); **6/8 clients on the shared loop**; axs (fixed-delay) + broadway (scraper) left as documented outliers; 100% held + RULE-2 clean; v1.50.0 2026-06-26 (B1) — **BR-CODE-2 slice 3** (vault resolver): `core/vault.py::vault_secret` single-sources the Vault `get_app_secret` lookup + 3-shape unwrap across seatgeek/seatdata/ticketsdata/axs; **BR-CODE-2 effectively complete** (guard 8/8 + retry 6/8 + vault 4/4); 100% held + RULE-2 clean; v1.51.0 2026-06-26 (B1) — BR-CODE-4 slice 1 (CSS token dedup): terminal/style.css's `:root` (byte-identical to `_shared/design-tokens.css`) hoisted to the shared file via an index.html link, dup block removed; verified via TestClient (link served, shared CSS resolves 200, values unchanged); store deferred; v1.53.0 2026-06-26 (B1) — BR-CODE-1 slice 39: the 11 static page-server routes (`/`, `/home[/]`, `/terminal[/]` + `/terminal/{page:path}` proxy, `/undelivered`, `/bridge[/]` + `/bridge/{page:path}` proxy, `/version.json`) lifted into `routers/pages.py` behind unchanged paths — getters resolve the three test-patched server symbols (`STOREFRONT_AS_LANDING`, `_STOREFRONT_VERSION`, `_BRIDGE_DIR`) at request time so the route tests' monkeypatch keeps binding; path-traversal guards + extension whitelists preserved verbatim; server.py → ~2583 LOC (6,705 → 2,583 this session, −61%), 100% line+branch held + RULE-2 clean; v1.54.0 2026-06-26 (B1) — BR-CODE-1 slice 40: the 9 `/store/test/*` wiring-harness HTML page routes (index/search/performer/venue/event/share/media_test/trip/tour) + their `_require_non_prod` gate lifted into `routers/store_test.py` — `get_is_production` getter resolves the test-patched `_is_production` server symbol at request time so the non-prod-gate test (`app._is_production`) keeps binding; server.py → ~2528 LOC, 100% line+branch held + RULE-2 clean; v1.55.0 2026-06-26 (B1) — BR-CODE-1 slice 41 (admin/ingest tail): the 6 admin/ingest routes (`/api/collect/run` + `/api/admin/{seed-home-venues,wire-sg-to-tevo,wire-sd-to-tevo,collect-orders,collect-sg-seller}`) lifted into `routers/admin.py` — A1's data-plane ingest surface (GET-from-TEvo/SG, write-our-DB only, RULE-2 clean); all live collaborators injected via getters that resolve the patched server symbols at request time (`client`/`require_sb`/`_flatten_order`/`_flatten_order_items`/`_venue_overlap`/`_upsert_tevo_event_into_events`/`_fire_collect`/`_collect_run_last_call`/`CRON_SECRET`/`SUPABASE_URL`/`_get_seatgeek_client`), so the route tests' monkeypatches (incl. the direct `app._upsert_tevo_event_into_events`/`app._fire_collect` unit calls) keep binding; those patchable helpers stay in server.py; unused `Header` import dropped; server.py → ~2205 LOC (6,705 → 2,205 this session, −67%), 100% line+branch held + RULE-2 clean; v1.56.0 2026-06-26 (B1) — production-readiness P1 #4 follow-up (incremental `safe_sb_read` adoption): routed the remaining `store_home` first-paint read (the events query — previously a bare try/except that didn't feed the breaker) + the `store_concierge` premium-rail read through the shared DB circuit breaker, so a Supabase blip on either fast-fails to a conservative empty grid (sentinel None) AND records the failure on `_db_breaker` (opens + sheds load after repeated failures) instead of silently swallowing; 100% line+branch held; doc note updated (remaining: search/near/movers); v1.57.0 2026-06-26 (B1) — production-readiness P1 #4 cont.: routed `store_movers` cold-compute path through the shared DB breaker too (empty-strip fallback matching `_compute_movers`'s own empty shape, not cached so it retries on recovery); rewrote `test_movers_cache_failed_compute_doesnt_poison` → `…degrades_gracefully` to assert the new 200-empty contract (was 5xx); home/concierge/movers homepage strips now all breaker-protected. Remaining: `store_events/near` (multi-read entanglement) + `store_search` (active-query UX is a product call); v1.58.0 2026-06-26 (B1) — production-readiness P1 #4 cont.: routed `store_events/near` through the breaker at both DB-read sites (hybrid owned-metadata merge + supabase candidate pull), each degrading to an empty geo-strip (200) on a Supabase blip instead of 500; +2 failure-path tests (`…hybrid_owned_metadata_read_failure_degrades`, `…supabase_read_failure_degrades`). **Every passive homepage strip (home/concierge/movers/near) is now breaker-protected.** Only remaining read site is `store_search` — deliberately left pending a product call on silent-empty vs surface-error for an active query; v1.59.0 2026-06-26 (B1) — production-readiness P1 #4 COMPLETE (code side): `store_search` now runs its whole compute (sql/live + players layer) through a new re-raising breaker helper (`sb_read_or_raise` → `core.resilience.resilient_call(breaker=_db_breaker)`, wired via `get_resilient_read` getter) and surfaces an explicit **502** "search temporarily unavailable" on DB failure instead of a misleading empty result — the right UX for an ACTIVE query (vs the passive strips' silent-empty), while still feeding the breaker for load-shedding; +1 test (`test_search_db_failure_returns_502`). **Every customer-facing storefront read site is now breaker-protected.**; v1.60.0 2026-06-28 (B1) — **BR-CODE-4 slice 2 (frontend smoke-test harness — the BR-CODE-4 prerequisite)**: `tests/frontend/` Playwright harness boots the real FastAPI app in a background thread (demo mode, no creds) and drives the pre-installed Chromium over all 16 shipped pages (home, terminal + 6 subpages incl. the event.js dedup target, store catalog/discover/tour/event, legal shells, undelivered), asserting each loads <400, renders its `body[data-page]` shell, has no same-origin JS/CSS 404, and fires **zero uncaught pageerrors** (console errors are signature-filtered for API/network noise). **Teeth validated**: 2 negative controls (a `function(){}` syntax error → "Function statements require a function name"; an injected bad call) both fail the harness; clean baseline is pageerror-free across every page. Isolated from the backend suite (`pytest.importorskip` + own dir + CI `--ignore=tests/frontend`; Playwright's sync event loop conflicts with the suite's `anyio.run` handler tests, so it runs as its own `frontend-smoke` CI job). Browser resolves the dev container's `/opt/pw-browsers` build via executable_path, CI installs its own. **This unblocks the event.js (~5,672 LOC) JS dedup — there is now a regression net under it.**; v1.65.0 2026-06-28 (B1) — **BR-CODE-1 core/ helper pass (decomp pushed to ~100%)**: the remaining business-logic bodies lifted out of server.py into `core/` — `core/search.py` (+`search_live`/`search_players`/`search_cache_get|put`), `core/discovery.py` (`attach_owned_metadata`), `core/trip_payloads.py` (4 trip/tour/discover builders), `core/ingest.py` (`fire_collect`/`upsert_tevo_event_into_events`/`flatten_order`), `core/storefront_html.py` (`read_storefront_html`), `core/canonical_refresh.py` (`fire_canonical_refresh`). Each is injected back via a thin server wrapper that passes the live symbols (db/client/caches/config) so every monkeypatch (`app._search_*`/`app._bulk_performer_assets`/`app._tour_package_payload`/`app._fire_collect`/`app._upsert_*`/`app._flatten_order`/`app._read_storefront_html`/`app._fire_canonical_refresh` + the `app.threading.Thread`/`app.requests.post` singletons) keeps binding; `_render_storefront_page` builds its HTMLResponse from the server `_read_storefront_html` wrapper (re-export trap caught by test_render_storefront_page_sets_no_cache). **server.py 2,152 → 1,599 LOC — now bootstrap + middleware + router-wiring + thin injection shims + the 2 infra routes (`/healthz`,`/webhooks/render`); no large business-logic bodies remain.** Total session: server.py 6,705 → 1,599 (−76%). 100% backend coverage + 18 frontend tests + RULE-2 + docs all green on each slice. PROJECT_BIBLE §2.5 updated; v1.66.0 2026-06-28 (B1) — **P0-critical fix caught by the pre-merge full smoke**: `core/db.make_supabase_client` imported `ClientOptions` from the deprecated `supabase.lib.client_options` shim, which on supabase ≥2.31 lacks the `.storage` attribute that `create_client → _init_supabase_auth_client` reads → **AttributeError at client construction → `sb=None` → every DB-backed route 500s in prod**. Hidden because the unit tests inject a fake `_create_client` and the frontend smoke only exercises static pages. Fixed: import `ClientOptions` from the canonical top-level `supabase` export; added `test_make_supabase_client_real_constructor` (builds via the REAL lazy `create_client`, asserts `.storage` present) so the mock-bypass gap can't regress. App boot now initializes `sb` (was None). **This is exactly why the operator asked for a full smoke before merge.**; v1.67.0 2026-06-28 (B1) — hub (`/` landing) verify + rewire: live Playwright test confirmed the VibePass hub serves at `/` with all 5 surface cards (Terminal/Undelivered/Orders/Store/Bridge) + correct hrefs + wired auth control + zero JS errors. Fixes: stale "4-surface" → "5-surface" subtitle (Orders/D2 was added); removed the ineffective `frame-ancestors 'none'` from the hub's `<meta>` CSP (browsers ignore it in meta — it only logged a console warning; framing is enforced by `_SecurityHeadersMiddleware`'s HTTP `X-Frame-Options: DENY` + CSP header). Confirmed the Render landing is already wired to the hub: `vibepass-storefront-test` web service runs `uvicorn server:app` with `STOREFRONT_AS_LANDING=false` (→ `/` = hub) and the static CDN's `static/index.html` redirects `/` → `/home/`. Added `tests/frontend/test_hub.py` (4 tests: card/href set, subtitle, auth control, no frame-ancestors-meta-warning regression). 22 frontend tests green. v1.61.0 2026-06-28 (B1) — BR-CODE-1 slice 42: the 11 storefront HTML page-shell routes (`/store`, `/store/chat`, `/store/event/{id}`, `/store/discover`, `/store/tour`, `/store/about|privacy|terms`, `/store-sw.js`, `/s/{share_id}`, `/store/shares`) lifted into `routers/storefront_pages.py` behind unchanged paths — the version/base-rewrite + cache helper (`_render_storefront_page`/`_read_storefront_html`/`_STOREFRONT_HTML_CACHE`) stays in server.py (tests call them directly + mutate the cache) and is injected via `get_render_storefront_page` getter; server.py → ~2149 LOC (6,705 → 2,149 this session, −68%); 100% backend coverage held + the new frontend smoke harness green over all /store* pages + RULE-2 clean; v1.62.0 2026-06-28 (B1) — **BR-CODE-1 slice 43 (route decomposition COMPLETE)**: `/api/public/config` lifted into `routers/misc.py` (payload builder `_public_config_payload` stays in server.py reading the live test-patched config constants, injected via `get_public_config` getter). **server.py now has ZERO domain routes — only the two bootstrap-infra endpoints `/healthz` + `/webhooks/render` remain**; every API/page/domain surface lives in `routers/*`. server.py ~2152 LOC of pure app-assembly (bootstrap/creds/middleware/router-wiring + shared helpers consumed via getters). 100% backend coverage + RULE-2 clean. (Remaining decomp is optional: extracting the shared helpers — payload builders, discovery helpers, _attach_owned_metadata, the storefront cache — into `core/`, a larger lower-value pass.); v1.63.0 2026-06-28 (B1) — **BR-CODE-4 JS-dedup investigated → deferred as low-value (evidence-based); the row's premise was inaccurate.** Findings on `static/terminal/event.js` (5,672 LOC IIFE): (1) **cross-file** JS dup does NOT exist — `app.js` already exposes the shared `window.Terminal` helper namespace and there are ZERO duplicated function names across the terminal JS; (2) the row's **"130+ loose globals manually localStorage-synced"** is inaccurate — it's an IIFE with ~37 let/var + ~25 const module-scoped vars and only **7** localStorage sites; (3) the **"5+ near-duplicate render functions"** are actually DISTINCT (e.g. `buildTevoSplitsTable` = 7 cols owned+market dual-data vs `buildSgSplitsTable` = 4 cols single-data; different empty-states + data shapes) — not safely mergeable into one `renderMarketplaceTable` without an abstraction that adds complexity; (4) the only genuine repetition is ~20 LOC of trivial `full-list-host`/`full-list-tbl` DOM-scaffold boilerplate across 8 distinct render fns + a 3× SG empty-state guard — marginal, clearer inline, and a *behavioral* change the crash-smoke-harness can't fully guard. **Conclusion: the high-value BR-CODE-4 work shipped (CSS token dedup slice 1 + the frontend smoke harness slice 2); the JS micro-dedup is not worth the risk/reward. BR-CODE-4 effectively complete.**; v1.64.0 2026-06-28 (B1) — BR-CODE-4 slice 3 (behavioral frontend net): `tests/frontend/test_store_render.py` upgrades the frontend net from crash-level to **behavioral** for the storefront catalog — stubs `/api/store/home` via Playwright route interception with a fixture and asserts store.js renders one `a.card` per returned event (names + `/store/event/{id}` hrefs), plus the empty-state path renders `#empty`. **Teeth validated**: breaking the card render (className change) fails the test. 18 frontend tests now green (16 smoke + 2 behavioral); establishes the fixture-stub pattern for any future storefront render work. Append-only board; the section-level version + bot-ref convention → [`README.md`](README.md) *Doc-writing rules*.
...
| C1-OPS-2 | C1 / B1 / Op | **4-domain reorg follow-ups (governance docs reframed 2026-06-17, PR #570).** `BOT_HIERARCHY.md` reframed from a top-down hierarchy to **peer-domain lane guidance** (A1 data plane · B1 git/code · C1 docs/coord · D0–D4 FE surfaces); push to `main` is **per-task** (A1 + B1 maintain, supersedes "A1 sole pusher"); DB security → A1, git security → B1; own-bible promotion model added. Authoritative docs updated (BOT_HIERARCHY §1–§6, CLAUDE.md §3/§5/§6/Push, PROJECT_BIBLE §1.5/§2, MIGRATION_CONVENTIONS, README). **Still open:** (1) **own-bible CI wiring** — the `<BOT>_BIBLE.md` registered-doc class isn't yet allowlisted in `bin/check-docs.sh` / README registry; do it in the same PR that creates the first new per-bot bible (`A1_BIBLE.md` / `B1_BIBLE.md`), not before (gate is registry↔disk bidirectional — a registry row with no file fails). (2) **peripheral drift sweep** — ~6 `docs/*` self-contracts + `.github/CONTRIBUTING.md` + `docs/templates/LANE_DISCIPLINE.template.md` still say "A1 sole pusher"/"reports to D0"; C1 to reconcile to the per-task + peer-domain model (CONTRIBUTING's "human operator = sole approver" is a *different* statement and stays). (3) **create the per-bot bibles** — A1/B1 author their own when ready; C1 arbitrates promotions to the main set. (4) **prune orphan branches** — ~19 `claude/*` + 2 `a1/*` remote branches have no open PR (see the 🌿 BRANCHES / PRs tracker); confirm-merged-then-delete. | **C1:** drive the drift sweep + own the promotion gate. **B1:** wire the `<BOT>_BIBLE.md` allowlist into `bin/check-docs.sh` when the first one lands. **Op:** confirm the per-task push model is reflected in any GitHub branch-protection settings (docs say per-task; branch protection is operator-only). |
...
| Reddit / SeatData / TickPick-orders ingest **dormant** | Intentional (operator 2026-06-03 data-health audit): no active crons — reddit ~22d stale, `seatdata_listings` empty + `seatdata_sales` ~25d, `tickpick_orders_queue/_process_30min` `active=false` (~3d) | **Don't re-flag in freshness audits.** Re-enable only on operator request. |

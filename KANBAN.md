...
| ID | Lane | Finding/Task | Action |
|---|---|---|---|
| B1-NEXT-63 | B1 | **SECDEF RPC `occurs_at_local` parsing audit.** `public.events.occurs_at_local` is a TEXT column (PROJECT_BIBLE §3 landmine) — any SECDEF RPC that casts/parses it as a timestamp (`::timestamptz`, `to_timestamp`, date math) can throw or silently mis-sort on non-ISO rows. Defense-in-depth; no known live break. | Sweep `pg_proc.prosrc` for `occurs_at_local` co-occurring with `::timestamptz` / `to_timestamp` / `::date`; guard unsafe casts (prefer a real timestamptz column where one exists, else a safe-parse wrapper). B1 files any findings as fix rows. |
...
| A1-OPS-11 | D0 | **TM verified-resale split not in `event.js` TD Listings tab.** Market Carpet (performer + venue) now surfaces TM PRIMARY **and** RESALE (`td_tm_resale_*`) + a TM-R÷TM premium spread — A1, commit `52d57ff` ✓. But D0's merged `event.js` predates mig 220000, so its TD Listings tab shows TM primary only, not the verified-resale dataset. | D0: add the `td_tm_resale_*` split to `event.js` `loadTdPlatformListings()` / TD Listings tab (Market Carpet already covers it). Low priority — resale only populates on sold-out / NBA games. |
...
| D0-PROD-5 | D0 | **Realtime websocket blocked by CSP** (found during Seat Map testing 2026-06-02). `event.html` CSP `connect-src` lists `https://*.supabase.co` but not `wss://*.supabase.co`, so the live-refresh socket (`supabase.js` realtime) is blocked in console. Pre-existing; unrelated to the seat map. Likely affects other terminal pages too. | Add `wss://*.supabase.co` to `connect-src` in `event.html` (and audit the other terminal HTML `<meta>` CSPs). One-line each. Low effort. |
...
| D0-PROD-7 | D0 ✅ infra shipped | **Seat Map real CDN coverage — SOLVED by `seatmap_manifest` cache** (mig `20260603160000` + edge fn `seatmap-manifest-sync` + `*/30` cron, applied 2026-06-03). `public.seatmap_manifest(venue_id,configuration_id,has_map,section_count,section_keys[])` now holds the actual manifests (Supabase egress reaches the CDN; the agent/SQL sandbox can't). `has_map` replaces the `fanvenues_key` proxy. Early sweep: ~89% has_map. | Remaining (low effort): (a) once the cron finishes the full ~1,197-pair sweep, record the real coverage count; (b) FE reads `has_map` to hide the Seat Map tab on events with no map; (c) optional `section_alias` precompute so the matcher is server-cached. |

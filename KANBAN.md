...
| **P2** | **BR-CODE-3** | B1 / D0 [IN PROGRESS, PR #584] | **Test coverage ~12%** (5,951 test LOC vs ~46.5k); **60+ broker/admin routes untested** (`/api/broker/*/chart-data`, `/movers`, all `/api/admin/*`, `/api/seatdata/*`, `/api/axs/*`). | Add route-level tests for the broker/admin surface (mock the SECDEF RPCs); prioritize the event-page + movers paths. Target 30% ratio. **Increment 1 (done):** `tests/test_public_infra_routes.py` — 10 tests over the public non-DB infra routes. **Increment 2 (done):** `tests/test_broker_routes.py` — 5 tests over `/api/broker/leagues`, `/performer/{id}/assets`, `/event/{id}/overview` + a reusable `FakeSupabase`. **Increment 3 (done):** `tests/test_broker_movers.py` — 4 tests over `/api/broker/movers` v2 + `/watchlist-movers`. **Increment 4 (done):** `tests/test_watchlist_runs_snapshots.py` — 6 tests over `/api/watchlist`, `/api/runs`, `/api/snapshots/latest` (event-join enrichment + sort), `/api/snapshots/velocity`. **Increment 5 (done):** `tests/test_catalog_routes.py` — 6 tests over the TEvo-catalog passthrough GETs (`/api/performers/{id}`, `/api/venues/{id}`, `/api/configurations` + `/{id}`) asserting arg-passthrough + response relay. **Total: 31 route tests.** **Next:** `/event/{id}/chart-data` (469-line fn) + the `/api/admin/*` POST orchestrators (mock the upstream fire). |
...
| A1-OPS-26 | A1 / Op | **Port the venue+date-coincidence name-guard into the OTHER tevo-search binders.** PR #616 (mig `20260619210000` + edge fn `sg-to-tevo-search-bridge` **v6**) added a "≥1 shared meaningful name token" accept-guard — without it, a venue (+50) + same-date (+24) score (~74) bound a *different show* at the same venue/date with **zero** name overlap (Salsa Spectacular→LA Philharmonic @ Hollywood Bowl; TBD Avalanche/NHL→Nuggets/NBA @ Ball Arena). The sibling **`aq-to-tevo-search-bridge`** has the SAME hole — its accept gate `bestScore < minScore \|\| bestVenue < 25` requires venue but **not** name overlap; **`match-sg-performers-to-tevo`** should be checked too. **(NB: the merged #613/#616 commit/migration/edge-fn text mislabels this matcher work "A1-OPS-24" — an id already owned by the social-pulse item below; track the matcher work by PR #, not that id.)** | Mirror the `NAME_STOP` token-overlap guard from `sg-to-tevo-search-bridge` into `aq-to-tevo-search-bridge` accept logic + deploy (operator-gated edge fn); audit `match-sg-performers-to-tevo`. Then re-run `NOT aq_name_consistent(e.name, sgc.sg_event_name)` across `sg_events_canonical` → confirm 0 wrong-name binds from any binder. |

...
> **Doc version:** v1.91.0 (2026-09-11) — **view / WC exclusions on both scanner legs (`deal_listing_excluded`); EVO notes captured (collect-listings v11 → `listings_snapshots.public_notes/view_type`); mig 162200 applied.** · v1.90.0 (2026-09-11) — **event-day PRICE MODEL (bucket v1, trained on every played curated-zone event) wired into the scanner + RESULTS ledger/RPC/strip so predictions are checked against outcomes; mig 162100 applied.** · v1.89.0 (2026-09-11) — **feed floor set to 7+ days (scanner + retire tick) and `deal_score_v1` re-tuned for that window (resale velocity first), mig 162000 applied.** · v1.88.0 (2026-09-11) — **deal scanner rebuilt: EVO source + GoTickets in one feed, 14-day floor removed (scanner + retire tick), scan-on-arrival every minute nearest-first, GT poller back on (161800/161900 applied).** · v1.87.0 (2026-09-11) — **ESPN collectors redeployed on `site.web.api.espn.com` (operator-approved); purchase pollers moved to the GoTickets 15-min cadence with rate-limit caps (mig 161500)**. · v1.86.0 (2026-09-11) — **D0-DEALS-1 APPLIED end-to-end (label · scorer · training set · SeatGeek 72h windows; 5,163-row snapshot backfill; first fit = report only) + A1-OPS-SOCIAL-DEAD ROOT-CAUSED (ESPN Akamai 403 on `site.api.espn.com` → host swap authored; reddit/X paused since the 2026-08-31 pause-all)**. · v1.85.0 (2026-09-11) — **purchase pollers APPLIED + unmapped mapped (579/633) + WINNER SCORER authored (D0-DEALS-1); A1-OPS-SOCIAL-DEAD filed (reddit/ESPN-news/transactions/X all 0 rows in 7d)** · **Deal-feed OUTCOME LABEL + OUR PURCHASES books authored (apply-pending), branch `claude/underpriced-listing-detector-8hdzzb`**: filed **D0-DEALS-1** (label + calibration + purchase pollers, and the prod-vs-repo scanner drift this session found: the live `scan_gotickets_deals` cites an uncommitted mig 20260812120000 + the 2026-08-19 zone-level rewrite; `deal_tracking`/`deal_model_shadow`/`gt_deals_retire_tick` exist only in prod). · v1.84.0 (2026-09-11) — **A1-MAP-1: venue ids mapped between every source** (operator: "for venues check for a venue mapping table, if none create, and map the venue ids between each source" · "add cross venue mapper to our current function to strengthen it"): the table exists (`cross_source_venue_map`); mig 20260911220000 adds `gotickets_venue_id` (154 filled by event majority from the purchase + sales books) and the AQ `venue_short_id` (343), fills 288 marketplace ids back onto `aq_venue_map`, and the resolver now takes the source's own venue id (looked up by ID before the name; rules 1/2 accept a known id with no venue string) while every strong live hit teaches the xref that id. · v1.83.0 (2026-09-11) — **A1-MAP-1: LIVE on every cron surface** (operator: "apply and merge into prod and then begin mapping and crons"): s4kcs / N2S / GoTickets catalogue / SG canonical flipped to `live`; migs 20260911217000 (live mode keeps the legacy mapper as a per-surface fill-only fallback — s4kcs venue-less names, N2S parking convention), 218000 (rule-4 gate drops its identity bypass: catalogue rows cost 500 → ~30 ms), 219000 (gotickets_event selector = venue-known rows, soonest first; per-surface `live_cap`, 1500 for the catalogue); cold path `event_mapper_deep_enqueue()` fired by hand; 6 wrong/stale `gotickets_event` bindings corrected. · v1.82.0 (2026-09-11) — **A1-MAP-1: shadow dry run RUNNING on every cron surface** after the first s4kcs cron run under the new command timed out at 170 s inside the shadow replay and rolled the legacy writes back; fixed by migs 20260911215000 (mirror indexes, bounded `event_mapper_shadow_tick` in its own transaction, live cap 400) + 216000 (rule 4 asks the hub through an index before calling the 163 ms `match_to_aq_event_id`) — resolve 388 → ~99 ms; first s4kcs tick 296/300 agree, 0 disagree. · v1.81.0 (2026-09-11) — **A1-MAP-1: APPLIED to prod** (200000/200100/210000 + 213000 rule-4 name guard after the first live run bound one purchase to a same-venue neighbour session; reverted). · v1.80.0 (2026-09-11) — **A1-MAP-1: switch-over authored (mig 20260911210000)** — every mapper cron calls `event_mapper_run(surface)`; per-surface legacy/shadow/live modes with the shadow dry run logged; cross-map of event, venue and performer ids on every hit. · v1.79.0 (2026-09-11) — **ONE event mapper authored (A1-MAP-1, apply-pending)**: `event_mapper_resolve()` + the shared TEvo cold path + `event_mapper_parity()`; `our_purchases_map()` re-pointed as the first caller (migs 20260911200000 / 200100 on `claude/happy-planck-eri09u`, superseding the unapplied 161100). Phased switch-over of s4kcs / N2S / GoTickets / SeatGeek / AXS mappers filed as the card's next steps. · v1.78.0 (2026-09-11) — **D4 Stage 3 (organizer side) shipped in source, PR #976 (stacked on #975), 4 migrations apply-pending**: filed **D4-OPS-32** (the stage ledger row: analytics/CSV · RSVP release · comp batch + budget · scan-reject audit · series schema + create flow) and annotated the staged-build Stage 3 bullet + backlog rows D4-OPS-22/24/27. · v1.77.0 (2026-09-11) — **D4 readiness sweep + staged build plan (payments excluded)**: added `### 🧱 D4 staged build` under NEXT (D4); refreshed the stale D4-OPS-1…16 ledger rows (they still read "deploy/apply pending" from May — prod's migration list shows the whole free-first batch landed 2026-05-24/25); stage 2a/2b customer-facing shipped (event-side alerts feed; attendee names on tickets); filed **D4-OPS-30** (six exos migrations in the repo are NOT in prod, incl. the media anon-listing fix, plus the 0703 template reconcile silently dropped `waitlist-open`) and **D4-OPS-31** (auto pre-event reminders shipped in source — apply pending). · v1.76.0 (2026-09-10) — added **D7-DATA-1**: listing view quality now ships (`sub_view`/`sub_notes`), but TEvo — the source behind most covers — stores no seller notes at all, so the majority of rows read `unknown` and an obstructed seat can still present as gate 1. The fix is an A1 change to `listings_snapshots` + the TEvo collector. · v1.75.0 (2026-09-10) — closed A1-OPS-32 (paciolan crons off) and D7-PROD-2 (allocator replaced with an assignment pass; the bug was worse than filed — 19 of 72 obligations were losing their cover outright); parked D7-OPS-1/2 as moot for D7 after the TicketsData contract lapsed; added A1-OPS-33 (~22 TD crons still firing at a dead vendor). · v1.74.0 (2026-09-10) — added A1-OPS-32 (Paciolan is an engine without ignition — zero data everywhere, yet crons 532/533 fire 312×/day against an empty queue; surfaced by folding the D6 lane into A1) and retired the `D6` lane-shorthand row. · v1.73.0 (2026-09-10) — added the `D5`/`D6`/`D7` lane-shorthand rows and the six **D7** N2S cards (D7-PROD-1/2, D7-OPS-1/2/3, D7-SEC-1). · v1.72.0 (2026-07-09) — added A1-OPS-30 (our SG SellerDirect sales ingest-dark: webhook undeployed + its idempotency-table mig unapplied + orders cron off) + A1-OPS-31 (SeatGeek primary inventory is unobservable via the resale-only broker feed — a new SG consumer/v2 pull would be needed to surface SG face-value availability on the discovery chart, which today uses AXS+TM only). · v1.71.0 (2026-07-08) — added A1-OPS-29 (movers-agg windows outgrew per-statement budgets; 2700s cron stopgap applied mig 20260708164500, fast-by-construction rewrite queued). · v1.70.0 (2026-07-03) — added the UNMERGED BRANCH INVENTORY section: a full 2026-07-03 sweep of all 60 remote branches (ahead-of-main + last commit) for triage/reconcile. · v1.69.0 (2026-07-02) — latest: added A1-OPS-28 (engines-without-ignition + doc-drift audit sweep parked for later analysis); v1.68.0 — added A1-OPS-27 (Bands in Town tour-tracking client shipped PR #693; unwired backlog + `core/instagram_share.py` removal candidate). Board is append-only; full prior doc-version history → [`docs/archive/2026-07-02-doc-version-history.md`](docs/archive/2026-07-02-doc-version-history.md). Convention → [`README.md`](README.md) *Doc-writing rules*.
...
| **P1** | **D0-DEALS-1** | D0 / A1 / Op [APPLIED 2026-09-11 (all but 161100 → mapper session); fit = report only] | **The DEALS feed had no outcome label and prod had drifted from the repo.** 5,131 feed rows since 2026-08-11 and nothing graded them (shadow `realized_eol` never filled; `deal_tracking` = one 10-row cohort). Ad-hoc grading 2026-09-11: predicted-≥0.70 bucket won ≥15% net only **42%** of the time (84% predicted); DUMPING regime won 9–12%; since the 2026-08-19 directive the live scanner writes every zone outlier ungated (91% of rows carry no anchor) — those won ~11% vs S4K's own later sales. **Drift:** prod `scan_gotickets_deals` cites mig 20260812120000 (not in tree) and writes 10 feed columns no migration added; `deal_tracking`, `deal_model_shadow`, `snapshot_deal_models`, `evaluate_tracked_deals`, `gt_deals_retire_tick` are prod-only. | **Authored (3 migs, this branch):** `20260911160300` records the 10 prod-only feed columns (idempotent) · `20260911160400` **outcome label** — `gotickets_deal_outcome` + `grade_deal_outcomes()` (event-day SG+SeatData+CRM sales at curated-zone > section > event, net of fee → WIN/FLAT/LOSS/NO_COMPS, prediction snapshot) + `v_deal_calibration` + `get_deal_calibration()` + daily cron · `20260911160500` **our purchases** — `seatgeek_purchases` (brokerdata `/purchases`) + `gotickets_purchases` (`/rest/purchases`) pollers (pg_net sync/drain, REDACTED payloads, 30-min crons), `our_purchases_map()` via hub, `v_our_purchases` / `v_our_purchase_flips` (buy→our CRM sale) / `v_our_purchase_deals` (did we buy a flagged deal + its label). Clients: `gotickets_client.get_purchases()` added (GET-only). **Next:** operator apply + verify on the 104 played deals; then capture the prod scanner body into the tree (separate reconciliation mig); then curve-relative 15-day trend + scorer over the label. **APPLIED 2026-09-11 (operator: 160500 only):** GoTickets purchases live — first pull 400 "Date range cannot exceed 30 days" → mig `20260911160700` chunks the window (applied); first drain 633 purchases, 338 hub-mapped, 295 unmapped (mapping-agent queue). SeatGeek `/purchases` → **401 code 421004 "token does not allow access to this endpoint"** — scope still not granted; operator action with SeatGeek, nothing to fix in code. 160300 + 160400 (label) still apply-pending. **+ 2026-09-11 later:** `20260911160700` (30-day chunks, applied) · `20260911161000` **map the unmapped** (applied; hub → unique event match venue+day+name-overlap≥0.6 → AQ 4-tier: 238+3 mapped, **579/633 GoTickets purchases now carry a tevo id**, 54 are events we don't carry — World Cup, Broadway, club shows, college FB) · `20260911160600` `v_our_purchase_deals` (needs label) · `20260911160800` **WINNER SCORER** — `deal_signal_snapshot` (flag-time features: moneyness / sigma_14d / ma7-over-ma14 / theta_7d / liquidity / ESPN team+opponent / Kalshi-Polymarket futures / social / venue ratio / sentiment) + `deal_score_v0` (hand-weighted logistic seeded from the grading) + `deal_model_coef`/`deal_score()` plug for a fitted version + `get_deal_signals()` RPC + 10-min cron · `20260911160900` `v_deal_training_set` (features ⋈ label, y_win market + y_flip own) + `scripts/fit_deal_scorer.py` (numpy IRLS, emits coef SQL; `tests/test_fit_deal_scorer.py`). **Own-flip label already live:** 58 purchases matched to a later CRM sale, median +30.6%, 55% ≥15%, 36% losses. **Apply-pending:** 160300 · 160400 · 160600 · 160800 · 160900. **HANDOFF (operator 2026-09-11 "split the mapping tools into a separate chat"):** the last 54 unmapped GoTickets purchases + the mapper work move to a separate session. Starting point authored, NOT applied: `20260911161100_our_purchases_map_evo.sql` — orchestrates the EXISTING tools only (`tevo_venue_search` queue → `tevo_venue_search_enqueue/harvest` + `tevo_venue_alias_adopt` for the 3 unresolved venue strings; `tevo_venue_events_enqueue(60, since)`/`_harvest` for venues with no mirror event on the purchase day; `gotickets_attempt_event_xref` matcher-v3 pass; twin tie-breaks rescheduled/Session-N). Diagnosis of the 54: 17 World Cup (MetLife/Hard Rock/Levi's/Gillette/Arrowhead), Broadway ×8, college FB ×3, rescheduled twins ×4, BNP sessions ×2, Hammerstein ×7 / Pacha ×3 / Winter Garden-NY ×7 (venue strings), Vikings TBD, WNBA Finals TBD. **APPLIED 2026-09-11 (operator "as recommended"):** 160300 · 160400 (+ `20260911161200` fix: 42702 ambiguous `zone_n` in the grader's `chosen` CTE, `written` counted groups) · 160600 · 160800 (+ `20260911161400` fix: `_liq` was one row per gt_event_id → "ON CONFLICT … a second time" on multi-GT-id events) · 160900 · `20260911161300` (SeatGeek `/purchases` contract from the operator: ≤72h windows, per_page ≤10, 200-pull watermark walked in 72h chunks, 1 probe/run while 401). **Grader:** 104 deals labelled (WIN 35 / FLAT 11 / LOSS 58; zone 77 · event 24 · section 3). **Backfill:** all 5,163 feed rows snapshotted (`deal_signal_snapshot`), ≤500/batch. **v0 vs feed on the 80 zone/section labels:** v0 ≥0.70 bucket wins 45.8% (n=48) vs feed ≥0.70 40.0% (n=50); both still overconfident (pred ~0.8–0.9). **First fit (numpy, 80 rows, NOT loaded):** in-sample AUC 0.999 = overfit; 5-fold CV AUC 0.95 full / 0.88 without the now-fetched team-social-market blocks (backfilled rows carry TODAY's values for those, not flag-time) / 0.80 price-vol-dte only; feed `pred_win_prob` alone 0.67. Dominant signal: flagged ≤1 day out wins 89% (n=9) vs 22% (n=71). `y_flip` has 0 labelled rows (no purchase↔feed-deal match yet). Coefficients stay out of `deal_model_coef` until n≥~300 flag-time rows and operator OK. **+ operator 2026-09-11 "poll sg and go tix purchase apis when you poll goticket events":** `20260911161500` (applied) moves both purchase pollers onto the GoTickets 15-min event cadence (sync 4,19,34,50 beside `gt_sales_sync_15min`; drains 9,24,39,55) with rate-limit caps — SeatGeek ≤2 windows/run + ≤2 next-pages/drain (shared token burst ≈10 req/8s), GoTickets ≤2 chunks/run; both skip a run while windows are in flight. **v1 MARKET-ONLY SCORE + GATES (operator 2026-09-11 "ignore news feed, use other metrics"):** `20260911161600` `deal_score_v1(jsonb)` (days-to-event, weekend, 7d/14d MA trend, moneyness, regime, lot size, cost, vs-zone depth, cost/amalgam — shrunk bucket log-odds from the 80-row label; no news/social/team) + `deal_gate_v1` (SUPPRESS at-market moneyness≥0.9 = 0/18 wins · SUPPRESS dumping/softening+falling MA · VERIFY too-cheap <0.5× anchor or <-50% vs zone) stored on `deal_signal_snapshot` via trigger + `v_deals_feed_ranked` + `get_deal_signals` re-ranked · `20260911161700` `get_deals_feed` carries score_v1/gate/timing and hides SUPPRESS rows · terminal `deals.{html,js}` shows a Score v1 column, LAST-DAY / VERIFY chips and the hidden count. **On the label:** v1 ≥0.50 won 94% (n16), 0.30–0.50 50%, 0.15–0.30 11%, <0.15 0% (in-sample; all MLB). **On today's feed (1,238 active):** 193 suppressed (64 at-market, 129 falling), 132 VERIFY, 913 OK — of which 220 score ≥0.30 and none ≥0.50 because everything active is 15+ days out (NHL 762 / NBA 243 / NFL 36). Both applied. Next: re-check the buckets when NHL/NBA deals start grading (label is MLB-only today). **SCANNER REBUILT (operator 2026-09-11 "Add evo scanner, turn on goticket polling, remove 14 day. Scan in real time as poller gets listings for both sources."):** `20260911161800` `scan_listing_deals(source, …)` — the live scanner body captured into the tree for the first time and parametrised: **EVO** (TEvo `listings_snapshots`, cost = wholesale, own/ancillary/parking excluded) joins GoTickets in ONE feed (`source` column; EVO `gt_listing_id` = −ticket_group_id, `evo_ticket_group_id` real id); the **14-day floor is gone** in the scanner AND in `gt_deals_retire_tick` (was retiring every row inside 14 days); `deal_scan_tick_1min` scans, every minute, the events whose newest capture is newer than their last scan — **nearest event first**, ≤25/source — so a listing is scored ~1 min after the poller lands it; `gt_listings_poll_2min` (dedicated GT poller, 300 events/2 min, paused since 08-31) back on; `gt_deals_scan_odd_min` now only refreshes the GT median. `20260911161900` `get_deals_feed` / `v_deals_feed_ranked` / `get_deal_signals` carry `source`; terminal page badges GT/EVO and shows per-source counts. First EVO scan: 15 events, nearest tonight, 10 outliers flagged (1 anchored). **Watch:** scan cost ~1.5 s/event — if the 1-min tick backs up, raise the per-source cap or split the two sources into separate jobs. **7-DAY FLOOR + SCORE v1.1 (operator 2026-09-11 "7 days and more … less is too short a window to convert"):** `20260911162000` — scanner admits events ≥7 days out (both sources), retire tick drops rows inside 7 days; `deal_score_v1` re-tuned on the 72 graded deals flagged ≥7d (base 21%): **resale velocity dominates** (≥60 sales in the prior 7d → 58% vs 8%), weekend 39% vs 7%, MA rising 38% / falling 0%, cost/amalgam ≥0.8 → 4%, moneyness ≥0.9 → 5%, DUMPING 0/8; gates: SUPPRESS at-market · SUPPRESS falling-market (MA<0.95 or DUMPING) · VERIFY too-cheap · VERIFY thin-market (<20 sales/7d). In-sample on 62 rows ≥7d: AUC 0.93, top-10 won 90%, bottom-43 won 2%. Timing labels now 7-14D / 15-30D / 31D+. **PRICE MODEL + RESULTS LEDGER (operator 2026-09-11 "do it and track all results we get, so we can see wins"):** `20260911162100` — the scanner's resale estimate (anchor × clearing-curve degradation) missed the actual event-day clearing price by a median 32% and ran 16% high; the miss tracks prior-week resale velocity (corr 0.66) and the 7d/14d trend (0.36). New: `deal_price_train` (one row per played curated-zone event × zone × T∈{7,14,21,30}: 30d realized anchor before T, 7d velocity, 7d/14d MA trend, weekend, event-day actual n≥8, log_ratio) built by `deal_price_train_build()`; `deal_price_adjust` bucket model (dte × velocity × trend × weekend with roll-ups) refit by `deal_price_adjust_refresh('bkt_v1')`; `deal_price_predict()` → pred_final / pred_roi / **P15** (share of comparable played event-zones that cleared a 15% net flip at this cost). Scanner writes `pred_final_price / pred_roi_pct / pred_p15 / price_bucket / price_model_version` on every deal beside the legacy estimate. **Results:** `v_deal_results` (every graded deal: predictions vs actual) + `get_deal_results(p_days)` RPC + a RESULTS strip on the deals page (W/F/L by source, calibration of P15 / predicted ROI / Score v1, model-vs-legacy price error, latest rows). Nightly `deal_price_train_nightly` 06:35Z builds new played events and refits. Training set built from 60 days of PLAYED events (375 events / 1,996 event-zones / 5,945 rows; nightly thereafter). **Operator 2026-09-11: no prediction backfill** — live feed rows receive pred_* only when the scanner re-evaluates them on a fresh listing capture (the one-shot fill of 142 rows was reverted). Retro-check on 72 graded ≥7d deals (in-sample — those events are in the training set): model median price error 21% (bias +6%) vs legacy 36% (bias +28%); win rate by predicted net ROI 15–30% → 67%, 0–15% → 38%, <0% → 5%, **≥30% → 10%** (a too-good prediction = a mispriced/obstructed seat — candidate for a VERIFY gate once more rows grade). **VIEW / WC EXCLUSIONS (operator 2026-09-11 "check evo and gotix listings for notes and exclude any obstructed/limited/partial views etc … exclude rows that say WC also"):** `20260911162200` `deal_listing_excluded(notes,row,section,wheelchair,view_type)` — one rule for both legs: notes/view_type matching obstructed · limited view · partial view · restricted · side view · rear stage/view · behind stage · no view · overhang · pole · pillar · standing room / SRO · not an event ticket · wheelchair / ADA / accessible / handicap; row or section carrying a WC token (WC, WC1, WC25, WCC, WCH, ADA seen live) or the same words; EVO `wheelchair` flag. GoTickets `notes` already carried this ("limited or obstructed view" = the most common note, 5,778 of 34,975 noted listings in 2h; 1.8% of all listings excluded); EVO carried NO notes → `collect-listings` edge fn v11 now writes TEvo `public_notes` + `view_type` into `listings_snapshots` (nullable, no backfill) and the scanner reads them. Before this the scanner only downgraded such rows to confidence=low; 20 live rows retired on apply. |
| **P1** | **A1-OPS-SOCIAL-DEAD** | A1 [ESPN FIXED 2026-09-11 (3 edge fns redeployed, operator-approved); reddit/X await operator] | **Every social/news ingest has written ZERO rows in 7 days while its cron shows active** (found building the deal scorer; `v_performer_reddit_pulse` empty for every deal performer). **Root causes (2026-09-11):** (1) **ESPN** — `site.api.espn.com` answers every Supabase-egress request with an Akamai `Access Denied` 403 (IP-level; browser UA + Referer + Accept-Language still 403). `espn-news-5min` / `espn-transactions-5min` / `espn-gameday-10min` / `espn-roster-10min` all "succeed" (the HTTP POST to the edge fn completes) while `espn_runs.errors` = every league. `site.web.api.espn.com` serves the identical paths — verified 200 for news, transactions, teams/{id}/schedule, summary, standings?level=3, injuries (pg_net 2026-09-11) and already proven live 2026-09-01 (bot_chat #3531) before the 2026-09-01 full rollback (#3533) redeployed the old host. (2) **Reddit** — `reddit_news_queue_1min` / `_process_1min` (and `watchlist_social_poll_1min`) were paused by the 2026-08-31 pause-all-crons directive and not among the 31 resumed; only `reddit_news_sweep_30min` + `broadway_social_poll_2min` run. Consequence: `broadway_social_poll` keeps inserting `reddit_news_pending` rows nobody drains (2,206 unresolved; newest ones are old.reddit 404s for `bway:*` search queries) and `reddit_news` is empty. (3) **X** — same pause (crons `active=false`), plus TwitterAPI.io credits exhausted 2026-07-07 (KANBAN backlog row); paid API, opt-in. | **Repo (this branch):** `ESPN_HOST = "site.web.api.espn.com"` in `supabase/functions/{espn-collect,espn-league-collect,espn-rosters}/index.ts` (source only). **Done 2026-09-11 (operator "approved for espn"):** espn-collect v14 · espn-league-collect v17 · espn-rosters v7 redeployed on the web host. **Operator calls left:** (b) decide whether `reddit_news_queue_1min` + `_process_1min` come back (`cron.alter_job(active := true)`) — or pause `broadway_social_poll_2min` so the pending table stops growing; (c) X stays off until credits + intent. |
...
| ID | Lane | Task | Action |
|---|---|---|---|
| A1-MAP-1 | A1 / D0 / Op [LIVE 2026-09-11 on purchases + s4kcs + N2S + GoTickets catalogue + SG canonical; tickpick/vivid stay shadow] | **One event mapper for every surface** (operator: "merge all event mappers into one so we can utilize the same resources instead of reinventing every time"; scope = inter-marketplace mapping of the SAME event; "utilize evo performer and venue search where necessary"). Twelve functions re-implement name+venue+date → `tevo_event_id` with their own guards (`our_purchases_map`, `s4kcs_map_events` 8 rules, `n2s_map_events` 0–5, GoTickets matcher v3, SG matcher v3, `axs_aq_match`, `match_to_aq_event_id`, `link_aq_tevo_from_events`, sibling fill, `n2s_gt_map_by_name`, `refresh_td_event_links`, paciolan). **Authored (this branch):** `20260911200000` — `event_mapper_resolve()` (identity → venue+local-day+overlap with twin tie-breaks → venue±24h performer → exact name+day → AQ 4-tier; pure, unique-or-decline, parking/TBD guarded) · `v_event_mapper_needs` (every unmapped surface in one union) · `event_mapper_deep_enqueue()`/`_harvest()` (the shared cold path over the EXISTING `tevo_venue_search` queue: TEvo venue search for unresolved strings, venue-events pull for known venues with no mirror event, hub seed so `aq-to-tevo-search-bridge` runs TEvo name/performer search for future needs; promotes queue resolutions into `cross_source_venue_map`, which only the CRM harvest did before) · `event_mapper_parity(surface, n)` · crons `45 8` / `45 9` UTC. `20260911200100` — `our_purchases_map()` v2 = one resolver call per row + hourly cron; needs view gains both purchase books. **Verified** on a scratch Postgres 16 seeded with prod rows: the 54 leftover GoTickets purchases → 7 map by rule 1 (Mariners / Reds / Pirates twins, BNP Sessions 9+10, WWE), parity over the 48 already-mapped purchases = 48 agree / 0 disagree, cold path seeds 3 venue strings + 12 venue pulls and re-maps once the stubbed harvest lands (World Cup ×11, Hammerstein alias). **Live replay 2026-09-11 (read-only, rule 1 inline):** s4kcs 208/212 agree, N2S 247/248, SG canonical 230/249 — **0 disagreements**, the rest declined (venue-string variants / hospitality names / SG-side rescheduled flags → rules 2–4 + cold path). GoTickets catalogue (rule 2): 50 agree and **4 corrections** — `gt_map_events` (instant + performer similarity, no venue, `row_number` tie) had bound Titans home games to the STALE re-listed twin (3381900 etc., last seen May/June, 0 listings) where the live id (3286193, seen daily, 21k listing rows) exists; the resolver's new liveness tie-break picks the live one. EVO needs no mapper (native id IS tevo; 3,312/3,440 orders carry it). Supersedes unapplied `20260911161100` (other branch); found while reviewing it: pg_net TTL is 6 h (its 24 h enqueue→harvest gap lost every response) and alias promotion was missing from its harvest. **+ 2026-09-11 later — SWITCH-OVER AUTHORED (operator: "merge all these mappers into one function and strengthen so they map to evo and cross map, then have all current users call them after a dry run" · "also map venue ids and performer ids cross sources"):** `20260911210000` — `event_mapper_switch` (per-surface mode legacy / **shadow = dry run** / live), `event_mapper_shadow_log` + `event_mapper_shadow_report()`, `event_mapper_run(surface)` = the ONE entry point (six mapper crons re-pointed, jobnames + schedules unchanged; N2S identity rules 0–0e carried verbatim), `event_mapper_map_surface` (dry-run by default), `event_mapper_apply` (cross-map: hub source id, venue alias → `cross_source_venue_map`, performer alias → `aq_performer_map` + `seatgeek_performer_xref`, hub short ids, catalogue writebacks — all fill-only), `event_mapper_performer_id()`. Resolver gained the `aq_name_consistent` matchup guard. Scratch-verified: shadow flags a wrong legacy pick as `disagree` naming the right game; live N2S keeps order identity ahead of inference; a catalogue hit cross-maps and the purchase for it then resolves by identity. **APPLIED 2026-09-11 ~18:05 UTC (operator: "Apply all three now")** + `20260911213000` (rule-4 name guard, applied 18:15). First live purchases run: 15 mapped (7 twins by rule 1, 7 Sienna Spiro rows by exact name+day → Hammerstein 3411567, 1 by the hub) — **and one WRONG**: the Federer Fan-Week purchase bound to the 11:00 Mixed Doubles session through the hub's venue+time tier (no name check). Reverted by hand (purchase + `gotickets_event` writeback); rule 4 now requires name overlap ≥ 0.5 + `aq_name_consistent` against the hub row's TEvo event — Federer declines, 40 purchases remain for the cold path. N2S cron running the new command every minute (shadow, succeeding). **+ 18:16 UTC — first s4kcs cron run FAILED**: `event_mapper_run` replayed the whole surface inside the legacy transaction, hit the 170 s timeout and rolled the legacy writes back (a resolve cost 321 ms, 163 ms × 2 of it `match_to_aq_event_id` scanning the 18,453-row hub — the bible's "~7,271" was stale). Mitigation: the four cron surfaces set to `legacy`. Fix, applied 2026-09-11 ~18:30 UTC: `20260911215000` (indexes `events_local_day_idx` / `events_venue_id_idx`; rule 2 day-prefix prefilter; `event_mapper_run` live cap 400 and shadow = legacy only; NEW `event_mapper_shadow_tick(surface, max=300)` runs as the cron's SECOND transaction — fresh legacy picks first, 24 h rotation, 30-min throttle — so a slow replay can never roll a legacy write back; six cron commands now `BEGIN…COMMIT; BEGIN…tick…COMMIT`) + `20260911216000` (rule 4 asks the hub through a new expression index for a same-venue row within ±1 day and calls the matcher only then; identity tiers always run). Measured after: ~99 ms per resolve (was 388). Surfaces flipped back to `shadow`; first `event_mapper_shadow_tick('s4kcs_orders')` by hand = 300 replayed · 296 agree · 0 disagree · 4 legacy_only (recall, not correctness: three s4kcs rows carry NO venue — "US Open Tennis - Session 24", "J Cole", "NBA Cup - Pistons at Nets" — and one venue variant "Moda Center - Complex"; the legacy s4kcs rules map those by name alone). Side finding, NOT this card's: `n2s_map_events_5min` (schedule is `* * * * *`) went from ~6 s to ~46 s per run at 01:00 UTC 2026-09-11, hours before any mapper migration was applied. **+ ~18:50–19:05 UTC — LIVE (operator: "begin mapping and crons")**: shadow readouts first — s4kcs 296/300 agree · N2S 74 agree / 1 legacy_only (a "… Parking" row the legacy maps to the main event; TEvo lists parking inside it) · GoTickets catalogue 32 agree / **3 disagree = stale re-listed twins the resolver corrects** / 5 legacy_only of which **3 are WRONG legacy picks** ("Silly Goose @ Union Stage" → Goose/MO; Monster Jam UBS + Prudential → Capital One Arena) · SG canonical 36 agree / 0 disagree. `20260911217000`: live = resolver first (cross-map), then the legacy mapper fill-only for the residue when `live_fallback` (on for s4kcs / N2S / SG; **off for gotickets_event** — its legacy is what binds stale twins). Those 6 catalogue rows fixed by hand (3 → live twin, 3 → NULL). All four surfaces flipped `live`. First live runs: s4kcs cron 18:57 = 61 s, 113 mapped (60 resolver: venue_day_name 45 / name_day_exact 12 / hub 3; 53 legacy fallback); SG by hand 15 mapped (identity_hub 5, name_day_exact 8, legacy 2) + 17 hub rows cross-mapped; N2S cron running live every minute. **gotickets_event mapped 0 of 400 twice**: (a) `218000` — the 216000 gate let every source-id row into the 163 ms hub scan (500 ms/row → 170 s overrun); rule 0 already covers identity, so the gate is the venue probe alone; (b) `219000` — the catalogue has 138k future rows all refreshed 08:45–08:50, so "newest 400" was an arbitrary slice of bars TEvo never lists (0/400 with any mirror event at the venue): the select keeps only venue-known rows (~10k), ties break soonest-first, `live_cap` per surface (1500 for the catalogue). Cold path fired by hand: 42 venue strings pending, 134 venues resolved for event pulls, 12 future needs hub-seeded for the bridge, 20 venue searches + 60 venue-events pulls in flight (harvest 09:45 UTC or by hand). **+ ~19:30 UTC — VENUE IDS ACROSS SOURCES (`20260911220000`)**: the venue xref already existed — `cross_source_venue_map` (1,342 rows, PK `tevo_venue_id`, sg/tickpick ids by event agreement daily since 0908) — but had no GoTickets ids (the purchase + sales books DO carry `venue_id`; the catalogue does not), no AQ hub short id, and never wrote ids back onto `aq_venue_map`. Now: `gotickets_venue_id` by event majority (≥ 90 %, one GT id per TEvo venue — tennis-complex splits stay unmapped) → **154 filled**, `venue_short_id` from `aq_venue_map` → **343**, `aq_venue_map.sg_venue_id/tickpick_venue_id` filled back → **288** (its UNIQUE ids respected), 0 conflicts; `venue_xref_derive_from_events` keeps running daily 06:23 UTC. **Mapper strengthened:** `event_mapper_resolve` gained `p_source_venue_id` (12th arg, defaulted — every 11-arg caller unchanged) and looks the TEvo venue up BY ID through the xref before the name; rules 1/2 accept a known id with no venue string. `event_mapper_surface_sql` now hands the source's venue id over for GoTickets purchases, SG canonical (`sg_venue_id` or the raw payload) and TickPick orders (raw); `event_mapper_apply` learns the id onto the xref on every strong hit (score ≥ 0.8, fill-only, never when the id already names another TEvo venue). Vivid publishes no venue id; `seatdata_venue_xref` is empty. | **Next:** watch `event_mapper_shadow_report` no longer (surfaces are live) — instead read `SELECT surface, mode, live_fallback, live_cap FROM event_mapper_switch` + per-surface mapped-by-method counts after a day; run `event_mapper_deep_harvest()` an hour after the enqueue; tickpick / vivid have no cron caller yet (AQ sweep :22 / backfill :40 own them) — wire `event_mapper_run` for them once the cold path has fed venues; then purchases are live; every other surface runs in SHADOW: read `SELECT * FROM event_mapper_shadow_report('<surface>')` after a few cron cycles and flip with `UPDATE event_mapper_switch SET mode='live' WHERE surface='<surface>'` when disagree ≈ 0 (order: s4kcs → n2s → gotickets_event → sg_events_canonical; tickpick/vivid stay shadow until the cold path has fed them). Then run `event_mapper_deep_enqueue()` once by hand, `_harvest()` an hour later, read `our_purchases_map()` (learns whether TEvo still lists the past World Cup / Broadway dates). Then the phased switch-over, one PR each, gated on `SELECT * FROM event_mapper_parity('<surface>', 500)` ≈ 0 disagree: (1) `s4kcs_map_events` (10-min cron; keep its rule 0 identity + rule 8 mirror as resolver rule 0 inputs), (2) `n2s_map_events` (identity rules 0–0e stay; rules 1–5 → resolver), (3) `gt_map_events` + `match_gotickets_us_events` (rule 2 IS matcher v3; the instant+performer rule gains venue anchoring + the liveness tie-break for free — 4 stale-twin rows get corrected), (4) `sg_attempt_event_xref_v3` (add its `aq_name_consistent`/`aq_league_consistent` accept-guards to rule 1 first — the only guard family the resolver does not yet carry), (5) `axs_aq_match` (±48 h → widen rule 2 window by source). Retire each old body only after its parity run. Not in scope: ESPN (`event_xref`), performer/venue mappers. |
| A1-OPS-32 | A1 / Op | **✅ DONE 2026-09-10 (mig 20260910540000)** — crons 532/533 deactivated (not unscheduled; schema, client and extension all retained, so re-igniting is a collector run). Was: **Paciolan/eVenue is an engine without ignition, and its crons still fire (found 2026-09-10, D6 lane fold).** The whole surface — `paciolan_client.py`, `paciolan_extension/*` (Chrome collector), `routers/paciolan.py`, 16 `paciolan_*` DB objects — carries **zero data**: `paciolan_seat_snapshots` 0 · `paciolan_pull_queue` 0 · raw captures 0. The collector was never run. Meanwhile crons **532 `paciolan_enqueue_due`** (`*/5`, 288 runs/24h) and **533 `paciolan_pull_sweep`** (`20 * * * *`, 24 runs/24h) keep firing against the empty queue. Surfaced by retiring **D6 into A1** (`PROJECT_BIBLE §2.3`) — the fold moved ownership, deliberately **not** the runtime. | **Operator call, two ways and they are opposite.** (a) *Ignite it* — run the Chrome collector once against a real eVenue event and confirm the pipe fills, in which case the crons were right all along; or (b) *stop it* — `cron.alter_job(532, active := false)` + same for 533 (an **Applier** action, Rule 1 — needs explicit permission) so we stop paying for 312 no-op runs/day. Do **not** drop the schema either way: the code is complete and reviewed, so re-igniting later is a collector run, not a rebuild. |
...
| D7-PROD-1 | D7 | **N2S obligation-covering pipeline — SHIPPED, operating.** The whole lane, built 2026-09-09/10 under direct operator direction with no board card until now (PRs #949·#950·#952–#957). Flow: S4K CRM ingest (`n2s_items_queue`/`_drain`, every minute) → event mapping (`n2s_map_events`, 5 rules incl. CRM order-identity and the GoTickets name alias) → four-source listing poll (`n2s_pull_events`: TEvo·GoTickets·SeatGeek·TicketsData) chained to run in the SAME tick as mapping → two-tier matching (`n2s_cover_candidates`: exact/splits, then one-seat over-delivery) → FIFO-exclusive allocation (`n2s_covers`) → `n2s_cover_queue` → `v_n2s_orders` → the subs panel + `n2s_profitable_cover` Realtime feed. **Covers 7 → 48**, all 48 with working buy links. | ✅ Operating. Cron chain green (8 jobs, every minute, 0 failures, max 9.4s/60s tick). |
| D7-OPS-1 | Op / A1 | **⏸ MOOT FOR D7 2026-09-10** — TicketsData contract not renewed (mig 20260910560000); no quota left to make additive. Still open for A1s TD surface if the contract returns. Was: **TicketsData N2S account never seeded — every N2S pull silently uses the exhausted SHARED account.** `n2s_td_drain()` reads `TICKETSDATA_N2S_USERNAME`/`PASSWORD` first and falls back to `TICKETSDATA_USERNAME`/`PASSWORD` when they are empty. Both N2S names are **whitelisted in `get_app_secret()` and fully coded for, but absent from `vault.secrets`** — so the fallback has always been in force. Result: 128 of 128 recent `n2s_ondemand` pulls returned HTTP 403 `quota_exhausted`, 0 rows inserted, no TicketsData snapshot in ~18h. The migration even logs it (`N2S account unseeded; using the SHARED TicketsData account`) but that NOTICE reaches nobody. | **Seed two vault secrets** (`TICKETSDATA_N2S_USERNAME`/`PASSWORD`) if a second account exists. Not a code change and not a purchase decision until that is established. |
| D7-OPS-2 | A1 | **⏸ MOOT FOR D7 2026-09-10** — same: TD contract lapsed, so there is no budget for `td_budget_ok()` to misreport on this lane. Still open for A1s TD surface. Was: **`td_budget_ok()` reports healthy while the vendor 403s.** It returned TRUE throughout the outage above, so our internal budget counter is not tracking the limit that actually binds — which is *why* an 18-hour TicketsData outage was invisible. Fixing D7-OPS-1 without this leaves the next exhaustion just as silent. | Make the budget check reflect observed 403s (e.g. read `td_pull_queue.error_msg='quota_exhausted'`), or state plainly what it does measure. |
| D7-SEC-1 | Op | **Rotate the N2S CRM key.** `crm.s4kcs.com/n2s` was pasted in plaintext during the 2026-09-09 build session and is therefore in the Postgres query log. Separately, the SeatGeek token travels in the query string (`?token=`) so it sits in `net.http_request_queue.url` — the header form is refused by that API (400 "Token is required"), so this is a property of the upstream, not our choice. | Rotate the N2S key. SG token exposure is upstream-imposed; note it rather than chase it. |
| D7-DATA-1 | D7→A1 | **Sightlines are unknowable for most covers — TEvo `public_notes` is never stored.** `sub_view`/`sub_notes` now ship end to end (mig 20260910630000), but only SeatGeek (`has_limited_view` + `seller_notes`) and GoTickets (`notes`) publish a signal. TEvo's API returns `public_notes` and `core/helpers.py` reads it, yet `core/store_events.py` records `"public_notes": None, # not mirrored to listings_snapshots` — the column does not exist on that table. TEvo was 10 of 11 rows on the feed, so the majority reads `unknown`, and `unknown` deliberately does NOT mean clear. Consequence: an obstructed TEvo seat can ship as gate 1 "actionable directly" with no warning. | **A1 call** — `listings_snapshots` + the TEvo collector are A1's data plane, so D7 did not make the change. Add `public_notes text` to `listings_snapshots` and populate it in the collector; `n2s_view_of()` then classifies TEvo rows with no further D7 work. |
| D7-PROD-2 | D7 | **✅ FIXED 2026-09-10 (mig 20260910580000)** — replaced with a real assignment pass; measured worse than filed (19 of 72 obligations lost their cover ENTIRELY, not merely a worse one, because the rule required a candidate to be both the listings best claimant AND its obligations best choice). Live result: covered 55→69, profitable 8→11, 0 listings over-allocated. Was: **Greedy-FIFO allocation leaves covers on the table.** Two findings from the #953 review, both conservative (they cost covers, never over-allocate): (a) a split take claims the WHOLE `(sub_source, sub_listing_id)`, so the untouched remainder can cover nobody; (b) the tier tiebreak is per-order but `_claimed` is global and FIFO, so an earlier order holding only tier-2 candidates can take a `quantity+1` listing that was a later order's EXACT match — paying for a spare seat *and* leaving the other order reported `no_match`. | Replace greedy FIFO with an assignment pass. Own change; do not widen a matcher PR into it. |
| A1-OPS-33 | A1 / Op | **~22 TicketsData crons are still firing at a dead vendor (found 2026-09-10).** The TD contract was not renewed, so every call now fails. D7 disabled its own two (599/601, mig 20260910560000) and made TD opt-in in the N2S matcher, but the A1 data-plane jobs are untouched and still on schedule: `td_pull_drain` (every minute), `td_normalize_drain` (*/2), `td_tier_enqueue_{sh,vd,tm,gt,tp}` (*/5), `td_enqueue_peak_{sh,vd,gt,tm}` (*/10), plus the daily `td_*_discover` sweeps and `td_watchlist_refresh`. Not disabled by D7 because they feed surfaces beyond N2S and switching off another lanes ingest is not D7s call. | **Operator decision.** Either deactivate them (`cron.alter_job(<id>, active := false)` — an Applier action) or confirm they should keep failing while the contract is renegotiated. Deactivate rather than unschedule, and do NOT drop `td_*` schema: the historical listings remain the only record of that vendors inventory. |
| D7-OPS-3 | B1 | **`seam-review` cannot see security controls in migrations.** PR #957 modified `get_app_secret()` — the vault-access whitelist every lane depends on — and the bot returned "✅ No §2.6 shared-seam paths touched". Its path map keys on `routers/*`, `aq_event_map` and similar, so a shared control living in a migration is invisible to it. Earlier PRs were flagged for far less (an additive route param; a read of the mapper). | Add `get_app_secret()` / vault-whitelist edits to the §2.6 seam map so the next one is caught mechanically rather than by the author remembering to flag it. |
| D4-OPS-1 | D4 | **✅ DEPLOYED (PR #321 source → PR #322 rebuild, 2026-05-24; row refreshed 2026-09-11).** Was: `/bridge` SW scope broken — `registerSW.ts` registered `/sw.js` (verified `GET /sw.js`→404 vs `GET /bridge/sw.js`→200 under the prefix) and `sw.js`/`manifest.json` used root-scoped paths = the hub doc, not the Bridge SPA. Fix: register `${BASE_URL}sw.js` with `scope: BASE_URL`; SW derives `BASE` from `self.location` for `SHELL` precache + offline nav fallback; manifest `id/start_url/scope/icons/shortcuts` → `/bridge/`; cache `VERSION` v4→v5. No `Service-Worker-Allowed` header needed (scope = script dir). tsc + `vite build` green. | **Deploy step (owner: key-holder):** rebuild `npm --prefix d4_bridge run build` with `.env.local` Supabase keys (PR #316 landmine) → copy `dist/ → static/bridge/`. Source-only change does NOT reach prod /bridge/ until this rebuild. |
| D4-OPS-3 | D4 | **✅ DEPLOYED (PR #321 → #322, 2026-05-24; refreshed 2026-09-11).** Was: SW `NEVER_CACHE_HOSTS` listed removed-Firebase hosts + omitted Supabase. Fix: dropped Firebase entries; never-cache any `*.supabase.co` host (dynamic suffix match, covers auth/REST/storage) + kept Stripe; refreshed comments. | Same rebuild → `static/bridge/` deploy step as D4-OPS-1. |
| D4-OPS-4 | D4 | **✅ DEPLOYED (PR #321 → #322, 2026-05-24; refreshed 2026-09-11).** Was: offline CSV export labeled voided tickets `active` (`OrganizerCheckIn.tsx` ignored the `voided` flag). Fix: `e.voided ? 'voided' : e.used ? 'used' : 'active'`. tsc green. | Same rebuild → `static/bridge/` deploy step as D4-OPS-1. |
| D4-OPS-5 | D4 ✅ applied | **✅ APPLIED to prod 2026-05-25 (`20260523150000` in `schema_migrations`; refreshed 2026-09-11).** Mig `20260523150000` CREATE OR REPLACEs `exos_mint_tickets` to enforce `exos_events.total_tickets` atomically in the same UPDATE that bumps `tickets_sold` (`total_tickets = 0` = uncapped, mirrors tier `capacity = 0`); a NULL-tier mint is now also bounded. RAISE rolls back the tier increment (single txn). Same signature → no overload. B1 cleared (2026-05-24). | **A1:** apply mig `20260523150000` (free-first batch per handoff doc). |
| D4-OPS-6 | D4 ✅ applied · B1 ✅ cleared | **✅ APPLIED to prod 2026-05-25 (`20260523160000`; superseded by the 5-arg event-scoped RPC, see D4-OPS-30; refreshed 2026-09-11).** Server-side barcode verification added. Mig `20260523160000` DROPs the 3-arg `exos_check_in_ticket` + CREATEs a 4-arg with `p_barcode_payload`: for a signed payload the server re-checks ticket id + owner + ±2-bucket window + HMAC (`extensions.hmac`, base64url — same scheme as `lib/barcode.ts`) and rejects forgeries (`barcode-rejected` / `barcode-expired`). NULL payload = manual/attested (allowed, recorded as such); legacy 3-seg skipped (rollout); a non-3/4-segment `T-` payload is now rejected as malformed. B1 cleared (2026-05-24, PR comment). SEC-LOW follow-up: sunset 3-seg bypass once all clients emit 4-seg. | **A1:** apply mig `20260523160000` (it DROP+CREATEs — no other callers on the 3-arg, B1-verified). |
...
| D4-OPS-14 | D4 ✅ applied | **✅ APPLIED to prod 2026-05-25 (`20260523200000`; refreshed 2026-09-11) — venue/promoter marketing, LIVE non-gated parts.** Mig `20260523200000`: (a) `exos_redeem_discount_code` — promo/access-code validation (presale/unlock via `unlocks_tier_ids`); (b) `exos_announce_to_followers` — owned-audience email blast reusing `exos_org_follows` + the mail queue (`event-announce` template; recipients server-derived → no open relay); (c) `exos_orgs.marketing` jsonb (socials + pixel-ID config); (d) Bandsintown/Google added to the distribution channel model. Branch-verified 7/7. | **A1:** apply mig `20260523200000`. **Deferred/gated (Growth workstream):** publishing to Bandsintown/Google/social = upstream WRITE → Hard-Rule-2 + creds (scaffold via `exos-distribute`); rendering 3rd-party **pixels + JSON-LD** on public pages = CSP + consent → **B1**; per-event OG/SEO on the React SPA needs meta-injection/SSR. |
| D4-OPS-15 | D4 ✅ applied | **✅ APPLIED to prod 2026-05-25 (`20260523210000`; refreshed 2026-09-11) — ticket-issuance / purchase mail.** Mail audit found transfers (`transfer-initiated`/`-claimed`) + org-invite wired, but **no issuance/purchase confirmation**. Mig `20260523210000` adds the `ticket-issued` template + `exos_queue_ticket_issued(ticket_id)` (server-derives recipient from the ticket owner — no open relay; caller = owner or org staff). Wired into the claim flow (`ClaimTicket.tsx` → new owner gets a "ticket ready" email) + `lib/mail.ts queueTicketIssued`. Same shape as the branch-verified announce RPC. | **A1:** apply mig `20260523210000`. **Wiring TODO:** the deferred Stripe `exos_fulfill_checkout` should queue `ticket-issued` to the buyer when payments go live. |
| D4-OPS-16 | D4 ✅ applied | **✅ APPLIED to prod 2026-05-25 (`20260523220000`; refreshed 2026-09-11) — holder notifications wired.** `event-cancelled`/`event-updated` templates existed but nothing triggered them (cancelEvent only flipped status; the EditEvent "emails holders" comment was aspirational). Mig `20260523220000`: `exos_notify_event_holders(event_id, template)` — one mail per DISTINCT non-voided holder, server-derived (no open relay). `cancelEvent` now **auto-notifies on cancel**; `notifyEventHolders()` helper available for the update case. Pattern-identical to the branch-verified announce RPC. | **A1:** apply mig `20260523220000`. **Follow-up:** ✅ "notify attendees" button wired in EditEvent for `event-updated` (published-only, explicit; `notifyEventHolders` returns the recipient count for the toast). Pending A1 apply of the RPC. |
...
| D4-OPS-30 | ✅ CLOSED 2026-09-11 | **✅ APPLIED 2026-09-11 (operator-directed, bot_chat #3650): five of the six landed via MCP** — `0605131000`, `0605131500`, `0605132000` (schema subset: legacy 4-arg RPC dropped; the 5-arg body was already the newer 0702144537 version), `0622210000`, `0702120000` (schema subset: `checkin_test_mode` column — the live check-in RPC referenced it while it did not exist, so **every door scan would have errored**), `0703150000` with `waitlist-open` restored. **Skipped on purpose:** `0702144637` reconcile cron (payments; edge fn not deployed). Was: 🔴 Prod drift — six `exos_*` migrations in the repo are NOT applied (verified against `schema_migrations` + the artifacts each creates, 2026-09-11):** `20260605131000` media anon-listing fix (storage policy still `bucket_id='exos-media'` only — this is D4-OPS-29), `20260605131500` `scanned_by_email`, `20260605132000` event-scoped check-in (5-arg RPC exists via 0702 wiring, but column/policy set incomplete), `20260622210000` public-views `security_invoker` (only events/tiers views carry it; the three `*_public_read` policies are absent — landing it partially would blank the anon storefront), `20260702144637` reconcile cron (moot until payments), `20260703150000` mail-template reconcile (**and as committed it silently dropped `waitlist-open`** — fixed in source 2026-09-11 alongside D4-OPS-31). | Apply in timestamp order as one Applier batch after review; re-run the D4-OPS-29 check afterwards. |
| D4-OPS-31 | D4 ✅ built · ✅ applied 2026-09-11 · **deploy pending** (rebuild `static/bridge/`) | **✅ AUTO PRE-EVENT REMINDERS BUILT + APPLIED (2026-09-11, bot_chat #3650; = D4-OPS-21). Cron `exos_send_event_reminders` live @ :11/:26/:41/:56; dry-run 0/0/0 on the empty schema.** Mig `20260911051000`: `event-reminder` template (allowlist rebuilt as the full 11-value union), `exos_send_event_reminders()` cron RPC (T-24h + T-2h, once per event, catch-up safe, postpone resets), `exos_send_event_reminder_now(uuid)` staff RPC (6h cooldown), `RemindersPanel` on the organizer event report. Mig `20260911050000` enables RLS on `exos_invoice_counters` (the one exos table without it). Harness: `tests/exos` Part C (8 scenarios) + 3 new vitest files (49 FE tests). | Apply both migrations (needs pg_cron for the schedule; the DO-block is guarded) → rebuild `static/bridge/` → confirm cron `exos_send_event_reminders` in `cron.job`. Mail still needs D4-OPS-19 secrets to leave the queue. |
| D4-OPS-32 | D4 ✅ built · **apply pending** (4 migs) · deploy pending | **✅ STAGE 3 (ORGANIZER SIDE) BUILT IN SOURCE 2026-09-11 — PR #976 draft, branch `claude/d4-organizer-stage3` stacked on #975 (retarget to `main` after #975 merges).** One commit per item: (1) D4-OPS-24 `exos_event_analytics` + `EventAnalyticsPanel` + shared `lib/csv.ts` (Summary/Attendees CSV) — mig `20260911130000`; (2) D4-OPS-22 `exos_release_ticket` + `exos_tickets.released_at` + `exos_events.allow_holder_release`/`release_cutoff_hours` + `ReleasePolicyPanel` + staff Release seat — mig `20260911131000` (**only capacity-returning path**; `exos_void_ticket` never frees seats — design note, not changed); (3) comp batch `exos_issue_comp_batch` + `exos_orgs.comp_budget` + `CompIssuancePanel` + OrgSettings budget — mig `20260911132000`; (4) `ScanRejectAudit` on OrganizerCheckIn (FE only); (5) D4-OPS-27 `exos_event_series` + `exos_create_event_series` + `exos_occurs_at_local` + `CreateSeries` page — mig `20260911133000` (also re-creates `exos_public_events` with `series_id`/`series_index`). Validation: vitest 73/73, tsc clean, `vite build` ok, SQL harness Parts A–G green on local PG16, lifecycle harness green. Customer-side asks logged: bot_chat #3652 (release button + RELEASED wording), #3653 (attendee_name follow-up once their mig lands; **timestamp collision** `20260911060000` with D7 PR #973 flagged). | **Operator/Applier:** apply `20260911130000` → `131000` → `132000` → `133000` → `134000` in order (all additive; each header carries the ROLLBACK; `134000` = door roster shows `attendee_name`, needs the customer session's `20260911060000` first). **Decision:** should `exos_void_ticket` also return capacity for FREE tickets (today only `exos_release_ticket` does)? **Then:** merge #975 → retarget #976 to main → rebuild `static/bridge/` → click-through. |
...
| D4-OPS-29 | ✅ CLOSED 2026-09-11 | **✅ APPLIED 2026-09-11 (mig `20260605131000` via MCP, operator-directed; bot_chat #3650).** Was: `exos-media` owner-scoped read policy missing (62h+). Storage DDL not yet applied: `CREATE POLICY … ON storage.objects WHERE bucket_id='exos-media' AND owner=auth.uid()`. Flagged by D4 in bot_chat #562. | A1 apply the storage policy; B1 review. (migrated from PROJECT_BIBLE §10) |
...
### 🧱 D4 staged build (filed 2026-09-11 — operator: "ignoring payments, build the rest in stages")

Readiness read (2026-09-11): backend is feature-complete for free ticketing (35 tables / 68 RPCs / 59 policies, all 0 rows — never used); frontend has every organizer + attendee + door view but thin tests and no live click-through ever run. Stages below are ordered so each one is a shippable increment; **payments (D4-OPS-7/13/17) stay out of every stage.** **Split 2026-09-11 (operator):** two parallel sessions — this one (customer-facing: Stage 2 + the attendee half of later stages) and `session_015Gu8oLENqxXX7dJbCVC2p6` (organizer-facing: Stage 3 on branch `claude/d4-organizer-stage3`). Cross-surface asks go through `bot_chat` `flag`s.

- **Stage 1 — go-live hardening (✅ in source, PR #975; ✅ all 8 migrations applied to prod 2026-09-11):** invoice-counter RLS · auto pre-event reminders (D4-OPS-31) · `waitlist-open` allowlist regression fixed · vitest coverage for barcode/datetime/utils · SQL harness Part C. **To close the stage (operator):** merge #975 + rebuild `static/bridge/` · set `RESEND_API_KEY`/`EXOS_MAIL_FROM` (D4-OPS-19) · custom Auth SMTP (D4-OPS-9) · run `docs/d4_live_fe_test_plan.md` once in a real browser (D4-OPS-11 rehearsal).
- **Stage 2 — attendee side (customer session; 2a ✅ alerts feed · 2b ✅ attendee names · 2c ✅ Spanish across My Tickets / pass / alerts / claim — all 2026-09-11, PR #975):** D4-OPS-23 attendee info capture (per-ticket name + organizer questions → guest list/CSV) · in-app notification backend for reminders/price-steps (`lib/notifications.ts` is FE-only today) · D4-OPS-20 wallet passes (PR #870 carries the pass builders; needs signing certs — land the optical-pass part first) · second i18n locale.
- **Stage 3 — organizer side (✅ in source 2026-09-11, PR #976 draft on `claude/d4-organizer-stage3`, stacked on #975; ⏳ migrations `20260911130000`/`131000`/`132000`/`133000` NOT applied — see D4-OPS-32):** D4-OPS-24 analytics + CSV export (scan-in / no-show / promoter) ✅ · D4-OPS-22 self-serve RSVP release ✅ (RPC + organizer policy; **attendee button = customer session**, bot_chat #3652) · group / comp allocations ✅ (batch + org budget) · D4-OPS-27 recurring + timed-entry series ✅ (schema + create flow; series-wide edit/cancel + storefront grouping later) · scanner scan-reject audit UI ✅. **To close the stage (operator):** apply the 4 migrations in order · merge #975 then retarget/merge #976 · rebuild `static/bridge/` · click-through on the deployed app.
- **Stage 4 — venue-grade:** D4-OPS-25 reserved seating + seat map (only if a seated venue signs) · D4-OPS-28 custom storefront domain · Redis-backed rate limit if the standalone server is ever run multi-replica.
...
- **D4-OPS-21 — event reminders (S-M).** ✅ **BUILT IN SOURCE 2026-09-11 (= D4-OPS-31, apply pending).** T-24h/T-2h cron + manual send-now; `.ics` add-to-calendar already existed.
- **D4-OPS-22 — waitlist + self-serve RSVP release (M).** ✅ **RELEASE BUILT IN SOURCE 2026-09-11 (PR #976, mig `20260911131000`, apply pending).** `exos_release_ticket` frees tier + house capacity and the existing waitlist auto-offer trigger hands the seat on; organizer policy (`allow_holder_release`, `release_cutoff_hours`) + staff "Release seat" on the report. Waitlist itself shipped earlier (mig 20260616180000/220000). **Open:** the attendee-facing release button (customer session, bot_chat #3652).
- **D4-OPS-23 — attendee info capture at registration (M).** 🟡 **Attendee half SHIPPED 2026-09-11 (customer session, PR #975):** per-ticket `attendee_name` + `exos_set_ticket_attendee` RPC, captured on the free-claim card ("who's going"), editable on the pass, shown on TicketDetail/WalletPass, auto-cleared on transfer (mig `20260911060000`, apply pending; bot_chat #3651 → organizer session). **Organizer half → organizer session:** roster/scanner/CSV coalesce `attendee_name`; organizer-defined custom questions still open. Per-ticket name/email + organizer-defined custom questions, surfaced on the guest list / check-in + CSV. Schema (per-ticket attendee fields / answers jsonb) + CreateEvent question builder + claim/checkout capture.
- **D4-OPS-24 — organizer analytics/reporting (M-L).** ✅ **BUILT IN SOURCE 2026-09-11 (PR #976, mig `20260911130000`, apply pending).** `exos_event_analytics` one-call document (sold/scanned/no-show, sales-by-day in event tz, tier/promoter/channel with scan-in, checkin + reject rollups) + `EventAnalyticsPanel` with Summary/Attendees CSV. Scan-reject audit panel on the check-in page ships alongside.
...
- **D4-OPS-27 — add-ons + multi-day/recurring/series (L).** 🟡 **SERIES SCHEMA + CREATE FLOW BUILT IN SOURCE 2026-09-11 (PR #976, mig `20260911133000`, apply pending).** `exos_event_series` + `exos_create_event_series` (template cloned per occurrence, tiers included; recurring and timed-entry slots are the same mechanism) + `CreateSeries` page with tz-correct generator. **Still open:** series-wide edit/cancel fan-out, storefront grouping (view exposes `series_id`; customer session), add-ons already shipped separately (mig 20260616190000).

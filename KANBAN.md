...
| ID | Lane | Task | Action |
|---|---|---|---|
| A1-OPS-26 | A1 / Op | **⏸️ Hourly auto-checks PAUSED per operator (2026-06-17).** The `CLAUDE.md §5` mandatory aging-sweep scheduled tasks (`bot-chat-aging-sweep` :00 → `#terminal-2-alerts`, `b1-security-autocheck` :15/:50, `d0-frontend-autocheck` :25, `d1-bot-chat-monitor` :30, `d2-bot-chat-monitor` :45, `c1-daily-checkpoint`) are being paused for now. **A missing heartbeat is now EXPECTED — do NOT treat sweep silence as a dead-task / P0 incident (this overrides the §5 "silent = dead" contract for the duration of the pause).** These are Claude Code scheduled tasks (`~/.claude/scheduled-tasks/`), reachable only from the operator's local machine — the web/remote session has no `scheduled-tasks` MCP, so the actual disable is operator-side, not done from this branch. | **Operator:** pause each task in local Claude Code (`/tasks` → disable, or the scheduled-tasks UI). **Re-enable when resumed** and DELETE this row (closure protocol). Until then, every lane's aging-sweep silence is intentional. (DB-side `cron.job` health/ingest jobs were left running — not part of this pause.) |

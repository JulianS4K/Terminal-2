...
> **Tip — orient before you build.** To get a fast map of `static/terminal/*` and how it wires to the backend, generate the **code knowledge graph** (`/understand` → `/understand-dashboard`): architectural layers, file/function relationships, and a guided tour. Recipe + caveats (outputs gitignored/regenerable) → `PROJECT_BIBLE.md §8` *("I want to understand the codebase fast")*.

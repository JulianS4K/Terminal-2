...
//                 -> process events whose occurs_at falls in that window
//   ?watchlist_id=X
//                 -> process only that watchlist row's events, ignoring window
//   (no params)   -> process the entire watchlist (small/manual testing only)
//
// Env vars:
//   TEVO_TOKEN / TEVO_SECRET   (from Supabase settings table, or env)
//   CRON_SECRET                (shared secret for the X-Cron-Secret header)
//   SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY  (auto-provided)
//
// Deploy: `supabase functions deploy collect-listings --no-verify-jwt`

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

// ============================================================
// Ticket Evolution client
// ============================================================
...
class EvoClient {
  ...
  /** Paginate /v9/listings for one event. Returns all ticket groups. */
  async getAllListings(eventId: number): Promise<any[]> {
    ...
    for (let page = 1; page <= 20; page++) {
      ...
      const groups: any[] = resp.ticket_groups ?? [];
...
// ------------------------------------------------------------
// Ancillary detection (parking, hospitality, VIP lounges, etc.)
// ------------------------------------------------------------
...
function isAncillarySection(section: string | null | undefined): boolean {
  if (!section) return false;
  return ANCILLARY_PATTERNS.some((re) => re.test(section));
}

// ------------------------------------------------------------
// Metrics computation (pure functions, work on in-memory listings)
// ------------------------------------------------------------
...
function buildListingRows(eventId: number, capturedAt: string, groups: any[]): ListingRow[] {
  return groups.map((g: any) => ({
    ...
    eticket: !!g.eticket,
    is_ancillary: isAncillarySection(g.section),
  }));
}
...
function computeEventMetrics(rows: ListingRow[], eventId: number, capturedAt: string) {
  ...
  const medianGroupSize = groupSizes.length
    ? percentile(groupSizes, 0.5)
    : null;

  // Get-in: minimum retail price for a group with quantity >= 2
  const pairs = seats
  ...
  // Bid-ask proxy from p25s (robust to outliers at tails)
...
function computeSectionMetrics(rows: ListingRow[], eventId: number, capturedAt: string) {
  // Group by section (keep ancillary separate as own rows)
...
// ------------------------------------------------------------
// TEvo cred resolver: prefer DB settings table, fall back to env
// ------------------------------------------------------------
...
async function resolveTevoCreds(db: any): Promise<{ token: string; secret: string }> {
  try {
    ...
    if (byKey.tevo_token && byKey.tevo_secret) {
      return { token: byKey.tevo_token, secret: byKey.tevo_secret };
    }
...
Deno.serve(async (req) => {
  ...
  // --- Phase 3: per-event listings + metrics (parallel, rate-limit-aware) ---
  // TEvo ~5 req/sec sustained; listings endpoint often paginates (1-5 calls/event).
  // Concurrency 3 × 200ms pacing keeps us well under the ceiling.
  ...
  const { errors } = await mapPool(
    ...
    async (eid) => {
      ...
      // Insert raw listings (chunked to avoid payload size limits)
  ...
  for (const err of errors.slice(0, 5)) {
    log.push(`FAIL event ${err.item}: ${err.err.message}`);
  }

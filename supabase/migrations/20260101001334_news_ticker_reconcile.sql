-- Reconcile get_terminal_news_ticker: internal deep-links + Broadway cast.
--
-- Three migrations rewrote this RPC in parallel branches: 20260704020000
-- (transactions), main's 20260704160000 (Broadway cast source), and mine
-- 20260704230000 (internal performer/player deep-links). Because CREATE OR
-- REPLACE fully replaces the body, the highest-timestamp one won on a from-zero
-- replay and dropped the others' sources (230000 kept transactions+internal
-- links but lost Broadway). This migration defines the UNION of all sources —
-- espn / transactions / injuries / scores / standings / reddit (all with the
-- internal deep-link urls) + broadway / bway_flags (already internal
-- performer.html urls) — so every source coexists. Idempotent CREATE OR REPLACE.

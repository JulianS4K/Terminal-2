-- ============================================================================
-- Migration 20260911218000 — event_mapper_resolve(): the rule-4 hub gate drops its identity bypass
-- Migration 20260911218000 · level:data-collection · lane:A1 · writes:none (function REPLACE)
--
-- Lane:     A1 (data plane)
-- Touches:  event_mapper_resolve() (REPLACE — identical to mig 20260911216000 except the gate's first disjunct)
-- Pre-reqs: 20260911216000
--
-- Already applied to prod · via MCP 2026-09-11 under operator direction (same session).
--
-- Measured on prod 2026-09-11 (first live run of gotickets_event): event_mapper_map_surface cost
-- ~500 ms/row and the 400-row live run overran the 170 s cap. Every catalogue row carries a source
-- id, so the 216000 gate's "identity ⇒ ask the hub" branch sent every row into the 163 ms
-- match_to_aq_event_id scan. That branch is redundant by construction: rule 0 already looked the
-- source id up in the same aq_event_map columns the matcher's tier0/tier1 read (and the matcher
-- has no gotickets tier at all). The gate is now the venue probe alone — ~30 ms/row on the same rows.
--
-- ROLLBACK: re-apply event_mapper_resolve from 20260911216000.
-- ============================================================================

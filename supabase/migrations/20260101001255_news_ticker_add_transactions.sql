-- Add ESPN transactions as a source in the terminal NEWS WIRE.
--
-- Per operator (2026-07-04): "add all transactions pages to the news in terminal
-- and poll every 5 mins". This adds a `transactions` CTE to get_terminal_news_ticker
-- (source='Transactions', item_type='transaction') pulling from espn_transactions,
-- unioned alongside espn news / injuries / scores / standings / reddit. The 5-min
-- polling is handled by the cron change in 20260704000200.
--
-- Only the new `transactions` CTE + its UNION ALL are added; the rest is
-- reproduced verbatim from the live definition (plpgsql has no partial replace).

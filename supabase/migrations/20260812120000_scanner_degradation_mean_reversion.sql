-- Migration 20260812120000 · lane:D0 (author) → applier (apply)
--   writes: scan_gotickets_deals(...) [fn — DROP 13-arg + CREATE, mean-reversion form]
--   pre: mig 20260811290000 (linear degradation scanner — the model this refines)
--   auth: scanner service_role-guarded SECDEF
--
-- WHY (refinement of mig 20260811290000, operator-directed 2026-08-12). The event
-- degradation factor projected the 14d-MA excess-over-curve LINEARLY across the whole
-- remaining horizon, damped by a flat 0.5 and bounded by a hard clamp:
--       forward_excess = clamp( (excess/14) · dte · 0.5,  floor, ceil )
-- Two weaknesses, both borrowed straight from options / rates practice where a basis
-- and its momentum are modelled as MEAN-REVERTING, never linearly extrapolated:
--   1. A 14-day trend was trusted equally 15 days out and 47 days out — but far-out
--      trends revert; a stale momentum should decay with horizon, not compound.
--   2. Far-out events therefore slammed into the arbitrary clamp instead of a
--      principled cap.
--
-- FIX — Ornstein-Uhlenbeck (exponential) decay of the excess toward the curve. The
-- excess RATE (excess/14, %/day) reverts to 0 with speed κ = ln2 / halflife, so the
-- forward cumulative excess over the remaining horizon is the OU integral:
--       forward_excess = (excess/14) · (1 − e^(−κ·dte)) / κ
-- Behaviour: → (excess/14)·dte near-term (trend persists short-horizon, matches the
-- old linear form); → (excess/14)/κ as dte→∞ (SATURATES — a soft, principled cap that
-- removes the clamp's arbitrariness). p_degr_halflife (default 7d) REPLACES p_degr_damp;
-- p_degr_floor / p_degr_ceil are kept as rarely-binding safety rails (extreme-data guard).
--
-- Read-only backtest on the live feed (2026-08-12): zero survive/drop flips vs the linear
-- form; divergence is concentrated far-out where the linear form was clamp-slamming
-- (e.g. dte 42, excess +15%: 1.150 → 1.109; dte 43, excess +1%: 1.016 → 1.007). The
-- feed columns (regime, dte_now, degr_excess_pct, degr_factor, *_raw) are unchanged —
-- only degr_factor's forward projection changes. Signature arg-types are identical to
-- mig 290000 (p_degr_halflife is numeric like p_degr_damp), but a param NAME + DEFAULT
-- change requires DROP-then-CREATE (CREATE OR REPLACE cannot rename an input parameter).
-- The cascade's positional `scan_gotickets_deals(30)` binding is unaffected.
--
-- ROLLBACK: DROP FUNCTION the 13-arg scan_gotickets_deals and re-apply mig 20260811290000
--   (restores the linear damp form). No schema change to revert.

DROP FUNCTION IF EXISTS public.scan_gotickets_deals(integer,numeric,integer,numeric,numeric,numeric,integer,numeric,integer,numeric,numeric,numeric,boolean);
...
DECLARE
  ...
  v_kappa numeric := ln(2.0) / GREATEST(coalesce(p_degr_halflife,7.0), 0.5);  -- OU mean-reversion speed (per day)
...
BEGIN
  ...
  evfac AS (   -- OU (mean-reverting) forward degradation multiplier + regime label per event
...
COMMENT ON FUNCTION public.scan_gotickets_deals(integer,numeric,integer,numeric,numeric,numeric,integer,numeric,integer,numeric,numeric,numeric,boolean) IS
  'GoTickets deal scanner + EVENT-SPECIFIC DEGRADATION (mean-reversion form). Refines mig 20260811290000: the 14d-MA excess-over-curve is projected forward to event day via an Ornstein-Uhlenbeck (exponential) decay — forward_excess = (excess/14)*(1-e^(-kappa*dte))/kappa, kappa=ln2/halflife — so a stale trend saturates far-out instead of compounding linearly into a clamp. p_degr_halflife (default 7d) replaces p_degr_damp; floor/ceil kept as safety rails. Adjusted-win refilter (Option A) live; p_dumping_gate hard-suppresses DUMPING (Option C, default off). *_raw columns retain pre-degradation values. D0 mig 20260812120000.';

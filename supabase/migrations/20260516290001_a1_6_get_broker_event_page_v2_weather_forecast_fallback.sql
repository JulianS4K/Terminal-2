-- A1.6 (D0 audit, bot_chat 251 / 262): get_broker_event_page_v2.freshness.weather_latest
-- currently reads max(observed_at) WHERE is_forecast=false. For upcoming events
-- with no observations yet, this is NULL — but forecasts ARE present in the
-- weather block (forecast CTE elsewhere in this RPC). Add fallback: when obs
-- is NULL, use max forecast observed_at. All other v2 RPC logic preserved.
--
-- Note: full body re-stated here (CREATE OR REPLACE on SECDEF function).
-- The only logic change is the weather_latest block at the end.
...
BEGIN
  ...
  IF v_sg_event_id IS NOT NULL THEN
    WITH em_latest AS (
      SELECT row_number() OVER (ORDER BY captured_at DESC) AS rn,
             captured_at, listings_count, tickets_count, cost_min, cost_p25, cost_median, cost_mean, cost_p75, cost_p90, cost_max, cost_sum,
             sold_quantity, sold_gross, sold_price_min, sold_price_median, sold_price_max, unique_sections, unique_rows
      FROM public.seatgeek_event_metrics WHERE sg_event_id = v_sg_event_id ORDER BY captured_at DESC LIMIT 25
    )
    SELECT jsonb_build_object(
  ...
  -- A1.6 fix: weather_latest falls back to most recent forecast when no observations exist

-- ============================================================================
-- Migration 20260911213000 — event_mapper_resolve(): rule 4 (AQ hub tiers) gains the name guard
-- Migration 20260911213000 · level:data-collection · lane:A1 · writes:(none — function body only)
--
-- Lane:     A1 (data plane)
-- Touches:  event_mapper_resolve() (REPLACE — identical to mig 20260911200000 except rule 4)
-- Pre-reqs: 20260911200000, 20260608194500 (aq_name_consistent)
--
-- Already applied to prod · via MCP 2026-09-11 under operator direction (the operator had just
-- authorised applying 200000/200100/210000; this corrects a wrong-write path found on the first
-- live run, minutes later).
--
-- FIRST LIVE RUN FINDING (our_purchases_map, 2026-09-11 18:10 UTC): 15 purchases mapped, 14 right,
-- ONE wrong — "US Open Fan Week Stars of the Open - Roger Federer" (Arthur Ashe, 2026-08-25 19:00)
-- was bound to 3411282 "2026 US Open Championship - Mixed Doubles - Rounds 1 & 2" (11:00 same
-- venue) by rule 4: match_to_aq_event_id's venue+time tiers carry NO name check, and the noon
-- anchor's ±6 h window reached the 11:00 session first. The write (purchase row + gotickets_event
-- writeback, both fill-only) was reverted by hand. Rules 1–3 already require name overlap; rule 4
-- now does too: the hub row's TEvo event name (or the hub row's own name) must overlap the source
-- name at p_min_overlap AND pass aq_name_consistent; identity tiers (tier0/tier1) are exempt.
-- With the guard the Federer purchase DECLINES (0.43 against the correct 19:00 event too — the
-- cold path / a curated alias is the honest route), and the Lion King → hub case still maps.
--
-- ROLLBACK: re-apply event_mapper_resolve from mig 20260911200000.
-- ============================================================================
...
COMMENT ON FUNCTION public.event_mapper_resolve(text, bigint, text, text, text, text, text, date, timestamptz, boolean, numeric) IS
  'THE event mapper (any marketplace event → tevo_event_id): identity (hub/canonical) → venue+local-day+overlap with twin tie-breaks → venue±24h performer (matcher v3) → exact name+day → AQ 4-tier. Pure/STABLE; unique-or-decline; parking + TBD guarded. A1 mig 20260911200000; rule-4 name guard mig 20260911213000.';

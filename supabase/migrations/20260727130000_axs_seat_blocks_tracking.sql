-- ============================================================================
-- Migration 20260727130000 — persist AXS seat blocks + sold-gap tracking (AXS-only)
--
-- Lane:     A1 (data plane — AXS ingest pipeline)
-- Touches:  axs_seat_blocks (W, new table), axs_persist_blocks() (W, new),
--           axs_ingest_snapshot() (W, adds block persistence), v_axs_sold_tracking (W, new view)
-- Pre-reqs: 20260727120000 (v2 seat-level ingest -> axs_seat_snapshots + v_axs_seat_groups),
--           roles coworker_readonly / analyst_ro (existing read roles)
--
-- WHY: the section/row/consecutive-block breakdown + inferred sold gaps were only
-- DERIVED at query time (v_axs_seat_groups). To TRACK inventory over time cheaply
-- we persist that rollup per snapshot into a dedicated table, with gap_before =
-- the seats sold between one available run and the next in the same section+row.
--
-- SCOPE: AXS only, by construction — this reads v_axs_seat_groups (axs_seat_snapshots)
-- and is called only from axs_ingest_snapshot. No other source is touched.
-- ============================================================================

create table if not exists public.axs_seat_blocks (
  ...
  is_ga          boolean,
  neighborhood   text,
  gap_before     integer not null default 0,   -- seats sold between the previous run and this one (same section+row)
  captured_at    timestamptz,
  created_at     timestamptz not null default now()
);

alter table public.axs_seat_blocks enable row level security;  -- service_role-only, mirrors sibling axs_* tables
grant select, insert, update, delete, truncate, references, trigger on public.axs_seat_blocks to service_role;
-- read roles may not exist in a from-zero replay; grant only if present (mirrors sibling axs_* grants)
do $$ begin
  if exists (select 1 from pg_roles where rolname='coworker_readonly') then
    grant select on public.axs_seat_blocks to coworker_readonly; end if;
  if exists (select 1 from pg_roles where rolname='analyst_ro') then
    grant select on public.axs_seat_blocks to analyst_ro; end if;
end $$;

create index if not exists idx_axs_seat_blocks_snapshot  on public.axs_seat_blocks(snapshot_id);
create index if not exists idx_axs_seat_blocks_event     on public.axs_seat_blocks(tevo_event_id, section_label, row_label);
create index if not exists idx_axs_seat_blocks_captured  on public.axs_seat_blocks(captured_at);

-- ---------------------------------------------------------------------------
-- axs_persist_blocks — materialize v_axs_seat_groups for one snapshot + gap_before
-- ---------------------------------------------------------------------------
...
begin
  ...
  delete from public.axs_seat_blocks where snapshot_id = p_snapshot_id;   -- idempotent re-run
  insert into public.axs_seat_blocks (
    snapshot_id, tevo_event_id, axs_event_id, tevo_venue_id, section_label, row_label,
    seat_type, src, price, qty, seat_from, seat_to, seats, is_ga, neighborhood, gap_before, captured_at)
  select
...
-- ---------------------------------------------------------------------------
-- Tracking view: per snapshot × section × row — available vs inferred-sold, over time
-- ---------------------------------------------------------------------------
create or replace view public.v_axs_sold_tracking
with (security_invoker = true) as
select
  b.snapshot_id, b.tevo_event_id, b.axs_event_id, b.captured_at,
  b.section_label, b.row_label,
  count(*)          as blocks,
  sum(b.qty)        as seats_avail,
  sum(b.gap_before) as seats_sold_gap,
  min(b.price)      as price_min,
  max(b.price)      as price_max
from public.axs_seat_blocks b
group by b.snapshot_id, b.tevo_event_id, b.axs_event_id, b.captured_at, b.section_label, b.row_label;

-- ---------------------------------------------------------------------------
-- axs_ingest_snapshot — same as 20260727120000, plus a call to persist blocks.
-- (Full CREATE OR REPLACE so the added persistence step ships atomically.)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Persist the section/row/block rollup + sold gaps for time-series tracking (AXS-only).

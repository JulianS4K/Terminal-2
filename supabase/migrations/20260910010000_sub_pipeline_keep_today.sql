-- ============================================================================
-- Migration 20260910010000 — the sub pipeline keeps TODAY's events
--
...
-- Pre-reqs: 20260910004000
--
-- READ-ONLY: pure SELECT / rebuild. No upstream call.
--
-- Operator direction 2026-09-09: "keep today."
--
-- Reverses the horizon cut made one migration earlier. 20260910004000 moved
-- both functions from `event_date >= current_date` to `> current_date` on the
-- argument that a same-day event has probably already started, so a cover you
-- cannot buy in time is noise at the top of a money-sorted list. The operator's
-- call is that the reverse error is worse: that cut also dropped tonight's
-- genuinely unplayed events, and dropping a real same-day cover costs an actual
-- fill, where showing a stale one costs a glance.
--
-- Back to `>= current_date` in all four places, and the refresh's eviction
-- narrows from `<= current_date` to `< current_date` so it still sweeps rows
-- whose event has passed but no longer evicts today's on sight.
--
-- ⚠ SAME-DAY ROWS CAN BE FOR AN EVENT ALREADY IN PROGRESS, and the pipeline
-- cannot currently tell. `event_date` is a bare date and the only start-time
-- source, `events.occurs_at_local`, is local WALL CLOCK with no zone attached
-- (§3 mixed-timezone landmine), so "has this started?" cannot be answered
-- without picking a timezone to pretend in. Consumers of the queue should read
-- a same-day row as "verify before acting", not as "known live". The real fix
-- is a start timestamptz per event, which would let the horizon be an hour
-- boundary rather than a day boundary; until then a whole-day window is the
-- widest claim this data actually supports.
-- ============================================================================

-- ── 1. matcher: today + future ──────────────────────────────────────────────
...
AS $$
  WITH o AS (
    SELECT s.source, s.order_status, v.sub_signal, s.order_id AS s4k_order_id,
     ...
     WHERE s.event_date >= current_date         -- TODAY + FUTURE (see header)
...
-- ── 2. queue refresh: same horizon, and evict only PAST rows ───────────────

-- Plug the Fed into the news wire: surface FRED macro releases as news items
-- when they update.
--
-- Two parts:
--   (1) Seed FEDFUNDS (Federal Funds Effective Rate) into macro_series_config so
--       the literal Fed policy rate flows through the existing FRED pipeline
--       (fred_queue_all → macro_indicators). UMCSENT was the only FRED series;
--       FEDFUNDS makes "the Fed" actually present.
--   (2) Add a 9th `macro` CTE to get_terminal_news_ticker (source='Fed',
--       item_type='macro') that emits one row per enabled FRED series for its
--       latest reading — gated on WHEN THAT READING FIRST LANDED in our store,
--       inside the news window. That is the "show when the Fed updates" signal.
--
-- Why first-landed (min ingested_at) and not realtime_start: FRED's observations
-- endpoint, queried without vintage params, returns realtime_start = the query
-- date, so it re-stamps every daily poll (verified: UMCSENT's 2026-05 value 44.8
-- carries realtime_start = each poll date). ingested_at likewise re-stamps. The
-- ONE stable "when did this reading first arrive" signal is min(ingested_at) over
-- the rows carrying the current (series, observation_date, value). When a new
-- monthly print (or a revision) lands, that min jumps to ~now and the row enters
-- the wire for the window duration, then ages out — exactly a release ticker.
--
-- Monthly series (FEDFUNDS, UMCSENT) behave cleanly here: a new observation_date
-- appears only when a new number is published, so first-landed ≈ release time
-- (within our daily poll cadence). Daily-constant policy series (e.g. target-rate
-- DFEDTARU, which advances observation_date every business day at an unchanged
-- value) are intentionally NOT seeded — they would false-trigger every day under
-- this model; add them later only with proper value-change detection.
--
-- CREATE OR REPLACE replaces the whole body, so this restates all 8 prior sources
-- from 20260704300000 (reconcile) verbatim and appends `macro`. Idempotent.

INSERT INTO macro_series_config (series_id, display_name, units, frequency, source, notes) VALUES
  ('FEDFUNDS', 'Fed Funds Rate', 'Percent', 'M', 'FRED',
   'Federal Funds Effective Rate — the Fed policy rate (monthly average, FRED). '
   'Surfaces in the terminal news wire (Fed source) when a new print lands.')

-- ============================================================================
-- Migration 20260702231000 — NEWS WIRE: add ESPN scores + standings sources
--
...
--
-- CHANGES vs the injuries version (20260702230040):
--   * NEW `scores` CTE over espn_event_snapshots — one row per game (DISTINCT ON
--     event, latest snapshot), live + final only (state IN ('in','post'); 'pre'
--     has no score). Unlike injuries, espn_event_id is STABLE so there is no
--     provisional-id churn — a plain latest-per-event is correct. published_at =
--     captured_at (the score's observation time), so live games float to the top
--     and finals settle by when they ended.
...
AS $function$
  ...
  scores AS (
    -- Latest snapshot per game, live + final only. espn_event_id is stable →
    -- plain latest-per-event (no churn grouping needed, unlike injuries).
...
COMMENT ON FUNCTION public.get_terminal_news_ticker(integer, text[], text[], text, integer) IS
  ...
  '(Scores: live+final, latest per game) + espn_team_snapshots (Standings: latest '

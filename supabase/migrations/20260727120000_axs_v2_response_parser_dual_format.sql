...
--           axs_section_snapshots (W, runtime), axs_events (W, runtime, via ingest)
...
-- body.startflow) is gone; the new envelope is body.event{} + body.price_points[]
-- + body.sections[] + body.offers[] + body.onsale{}, with body.status/exit_code.
-- The old distiller keyed on body.meta.api == 'veritix', so since ~2026-07-02
-- every pull returned HTTP 200 but ingested to NULL (216 events stranded at
-- snapshot_id = -1), and a lost 'pending' probe wedged the whole drain via the
-- in-flight guard.
--
-- WHAT: keep BOTH formats — new (v2) is the default, legacy veritix is the
-- fallback — so a mixed backlog and any upstream rollback both ingest cleanly.
-- Plus harden axs_probe_step: (1) success classifier accepts either envelope;
-- (2) self-heal probes stuck 'pending' whose response aged out of net's TTL, and
-- only treat sub-10-minute pendings as genuinely in-flight, so one lost response
-- can never deadlock the drain again.
--
-- NOTE: the v2 payload is section + price-point granularity — it carries NO
-- per-seat manifest (the old body.offers.offers[].items[] seat rows). So v2
-- ingest populates axs_event_snapshots + axs_section_snapshots only;
-- axs_seat_snapshots stays empty for v2 pulls (documented drift, not a bug).
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Helper: is this /fetch response a successful AXS document? (either envelope)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Helper: classify the response reason (mirrors the old inline CASE + 'axs_v2')
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- axs_ingest_snapshot — dual-format ingest (v2 default, veritix fallback)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Format detection: v2 (new) is default, veritix (old) is fallback.
  ...
  if v_is_new then
    ...
    -- price_point.section is a useless constant in v2; the real section<->price
    -- link is price_level_id. Index each price level's face + availability here,
    -- then attribute to the sections that list that level (below).
    drop table if exists _axs_pp;
    create temp table _axs_pp on commit drop as
    select
      pp->>'price_level_id'                                   as price_level_id,
      coalesce((pp->'pricing'->>'face')::numeric,
               nullif(pp->>'amount','')::numeric / 100)       as face,
      coalesce(nullif(pp->>'availability','')::int, 0)        as avail,
      coalesce((pp->>'sold_out')::boolean, false)             as sold_out
    from jsonb_array_elements(coalesce(v_body->'price_points','[]'::jsonb)) pp;
    ...
    select
      ...
      (select min(face) from _axs_pp),                                 -- event price band + get-in from
      (select max(face) from _axs_pp),                                 -- the price_points directly (clean,
      (select min(face) from _axs_pp where not sold_out and avail > 0),-- no cross-section double count)
      jsonb_array_length(coalesce(v_body->'price_points','[]'::jsonb)),
      jsonb_array_length(coalesce(v_body->'sections','[]'::jsonb)),
      jsonb_array_length(coalesce(v_body->'offers','[]'::jsonb)),
      (select coalesce(sum(avail),0) from _axs_pp),                    -- all primary; v2 has no resale split
      0,
    ...
    -- One row per real seating section (body.sections), prices joined via its
    -- price_levels[]. avail_qty is price-level-scoped and may overlap across
    -- sections that share a level (the event seats_primary above is the clean total).
    ...
    select
      v_snap, v_tevo_event, v_tevo_venue, p_axs_event_id,
      s->>'label',
      s->>'neighborhood',
      coalesce((s->>'general_admission')::boolean, false),
      coalesce((s->>'sold_out')::boolean, false),
      coalesce(pl.avail, 0), pl.price_min, pl.price_max,
      case when jsonb_typeof(s->'seat_types') = 'array'
           then array(select jsonb_array_elements_text(s->'seat_types')) end,
      false
    from jsonb_array_elements(coalesce(v_body->'sections','[]'::jsonb)) s
    left join lateral (
      select coalesce(sum(pp.avail),0) as avail, min(pp.face) as price_min, max(pp.face) as price_max
      from _axs_pp pp
      where pp.price_level_id in (
        select jsonb_array_elements_text(coalesce(s->'price_levels','[]'::jsonb)))
    ) pl on true
    where (s->>'label') is not null;

    drop table if exists _axs_pp;
    -- axs_seat_snapshots intentionally NOT populated (v2 = section granularity, no seat manifest).
  else
    -- ===================== LEGACY veritix envelope (fallback) =====================
    if v_tevo_venue is null then
      select tevo_venue_id into v_tevo_venue from public.axs_venues
...
-- ---------------------------------------------------------------------------
-- axs_probe_step — dual-format classifier + stuck-pending self-heal
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Re-queue it as 'retry' so it gets pulled again.
  ...
  -- burning TD credits on dead events. (v2 status='expired'; exit_code 4.)
  ...
  loop
    ...
    if v_snap is not null then
      ...
      -- log AXS credit consumption into the unified TD ledger (counter value at ingest).
  ...
  -- Only a FRESH pending (fired < 10 min ago) counts as genuinely in-flight; a
  -- lost/stale one was already re-queued above, so it can't deadlock the drain.

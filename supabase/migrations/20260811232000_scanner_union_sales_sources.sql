-- Migration 20260811232000 · lane:D0 (author) → applier (apply) · writes:scan_gotickets_deals(int,numeric,int,numeric,numeric,numeric,int,numeric) [fn, replace] · reads:+seatdata_sales_snapshots · pre:mig 20260811231000 (v5 scanner) · auth:service_role-guarded SECDEF
--
-- WHY (operator-directed "look at past sales where available"). v5's current-event
-- realized anchor read only SeatGeek sales. This unions in SeatData realized sales
-- (seatdata_sales_snapshots) — verified same per-ticket price basis (on high-sample
-- shared events SG/SD medians match within ~2%: $165/$169, $1094/$1057; divergences
-- only on n<8). SeatData covers ~110 events/45d that SeatGeek doesn't, and adds
-- depth elsewhere. So a deal's win-probability is now computed over ALL available
-- realized market sales for the event × section number. Historic fallback
-- (section_sale_baseline) unchanged. Only the current-event LATERAL differs from v5.
--
-- ROLLBACK: re-apply mig 20260811231000 (SeatGeek-only current-event anchor).
...
BEGIN
  ...
  scored AS (
    ...
    LEFT JOIN LATERAL (
      -- current-event realized sales: ALL available market sources (SeatGeek + SeatData)
...
COMMENT ON FUNCTION public.scan_gotickets_deals(int,numeric,int,numeric,numeric,numeric,int,numeric) IS
  'Live-feed scanner v6: GoTickets section outlier -> realized win-probability over ALL available market sales (SeatGeek + SeatData) for the event x section-number, historic section_sale_baseline fallback -> win_prob >= p_min_win_prob (0.70) net of fee. service_role only. D0 mig 20260811232000.';

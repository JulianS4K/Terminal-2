-- Migration 20260620220000 · level:2 · lane:A1 · operator-directed (2026-06-20)
-- Lane: A1 (data plane — de-noise the competing-event alert)
-- Touches W: CREATE OR REPLACE FUNCTION public.compute_alerts_tick()
-- Touches R: public.event_competitors_snapshot + existing v2 sources
-- Pre-reqs: alerts v2 (mig 20260620200000/210000)
--
-- competing_event_added fired 2,190 alerts on enable — it triggered for EVERY event with
-- any neighbor (avg 2.1 competitors), and re-burst every 24h via the cooldown. That is the
-- exact noise the v2 redesign targets. Fix: route it through the SAME hysteresis framework
-- as the volatility rules (fire once on the edge, clear at a lower band) AND raise the bar to
-- a genuinely CROWDED market (>=5 competing events; clear <3). 2,190 -> ~162 one-time, then
-- only new crossings. Message reworded to "Crowded market". Everything else is the applied v2
-- function verbatim (arb rule still parked; vol read from the mv_event_price_vol matview).
-- ============================================================
...
BEGIN
  ...
  CREATE TEMP TABLE _sig ON COMMIT DROP AS
    ...
    FROM _vol v
    UNION ALL
    -- de-noised: crowded-market edge (>=5 competitors), hysteresis-gated (clear <3)

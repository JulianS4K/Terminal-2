-- Fix: AXS drain persist-loop scanned the whole 104MB pg_net buffer.
--
-- The loop's WHERE called safe_jsonb(r.content) joined to net._http_response,
-- which the planner evaluated across all ~9k buffered bodies (every source's
-- HTTP traffic), parsing ~100MB per run → 2-min runtimes that piled up behind
-- the 1-min cron. Fix: the loop now reads ONLY axs_probe (small); the api check
-- happens inside axs_ingest_snapshot, which fetches the single response by id
-- (index scan) and returns NULL for non-veritix/aged-out bodies. A LIMIT bounds
-- per-run work so the backlog drains over several runs instead of one long one.
...
begin
  ...
  -- persist confirmed pulls. Read ONLY axs_probe here; ingest looks up the one
  -- response by id and self-skips non-veritix. Bounded so a backlog can't make
  -- one run exceed the cron interval.
  for v_ev, v_rid in
  ...
  loop
    ...
    if v_snap is not null then
      ...
      -- response aged out / not veritix → mark so we don't retry ingest forever
...
-- drop the prior 2-arg overload so the cron's no-arg call stays unambiguous
drop function if exists public.axs_probe_step(integer, integer);

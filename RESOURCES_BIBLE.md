...
**AXS RULE — consecutive seats group into one listing.** Individual `axs_seat_snapshots` rows that are CONSECUTIVE within the same `section_label` + `row_label`, at the same `price`, `seat_type`, and `src` (primary/resale), are one sellable block — present them grouped, never as N separate rows. **`v_axs_seat_groups`** applies this (gaps-and-islands: `seat_no - row_number()` is constant across a run) → one row per block with `qty`, `seat_from`/`seat_to`, `seats`. A run splits on any gap in seat number, price change, or type change. Non-numeric labels (GA/lettered) stay qty-1. Caveat: odd/even-by-aisle venues break strict numeric adjacency (physically-adjacent seats differ by 2) → they surface as qty-1 blocks. (Validated: 1,280 seats → 231 blocks, avg 5.5, max 15.)

**`v_axs_listings`** re-shapes those blocks into the **EVO listing standard** (`event_id`=tevo_event_id, `section`, `row`, `quantity`, `retail_price`, `type`, `format`, `splits`, `wheelchair`, `is_owned`, `source='axs'`) so AXS inventory reads like every other source — plus AXS-only `seat_from`/`seat_to`/`seat_numbers` (TEvo ticket groups don't carry exact seats).

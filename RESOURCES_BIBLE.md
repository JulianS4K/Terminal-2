...
- **Enrollment:** `td_seed_mapped_xref(limit,apply,owned_only)` enrolls AQ-mapped events on SH/VD (URL templated from sh/vivid id, owned-first); GT via discovery; TM seeded for MSG. **AXS:** own probe `axs-probe-drain` (1m, `is_peak`+`td_budget_ok` gated; off-peak ~1/10m at `:08`); charges the TD ledger at ingest.

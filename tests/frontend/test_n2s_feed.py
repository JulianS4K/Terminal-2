...
_PULL_RE = re.compile(r"n2s-covers/pull")


def test_pull_button_requests_a_marketplace_pull(feed_page, live_server):
    """The pull button must POST. Refresh only re-reads what the matcher already
    computed; this is the control that goes back out to the marketplaces, and a
    dropped method would make it silently re-read instead."""
    seen = []

    def _pull(route):
        seen.append(route.request.method)
        _json_route(route, json.dumps({
            "orders": 107, "events": 47,
            "dispatched": {"tevo": 2, "gotickets": 1, "seatgeek": 10, "ticketsdata": 0},
            "skipped_fresh": {"tevo": 45, "gotickets": 30}}))

    feed_page.route(_COVERS_RE, lambda r: _json_route(r, _payload([_cover(1)])))
    feed_page.route(_PULL_RE, _pull)
    feed_page.goto(f"{live_server}/terminal/subs.html", wait_until="domcontentloaded")
    feed_page.wait_for_selector("#n2sTable tr[data-n2s]")
    feed_page.click("#n2sPull")
    feed_page.wait_for_function(
        "() => document.getElementById('n2sMeta').textContent.includes('13 request')")
    assert seen == ["POST"]


def test_pull_button_says_so_when_everything_is_already_fresh(feed_page, live_server):
    """Dispatching nothing is the five-minute guard working, not a failure. If
    the button looked inert the operator would press it repeatedly."""
    feed_page.route(_COVERS_RE, lambda r: _json_route(r, _payload([_cover(1)])))
    feed_page.route(_PULL_RE, lambda r: _json_route(r, json.dumps({
        "orders": 107, "events": 47,
        "dispatched": {"tevo": 0, "gotickets": 0, "seatgeek": 0, "ticketsdata": 0},
        "skipped_fresh": {"tevo": 47, "gotickets": 31}})))
    feed_page.goto(f"{live_server}/terminal/subs.html", wait_until="domcontentloaded")
    feed_page.wait_for_selector("#n2sTable tr[data-n2s]")
    feed_page.click("#n2sPull")
    feed_page.wait_for_function(
        "() => document.getElementById('n2sMeta').textContent.includes('already current')")

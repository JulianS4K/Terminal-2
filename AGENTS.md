...
- migrations applied through 20260507000020_drop_team_xref
...
- espn-collect edge fn = **v3 (code, 2026-05-07)** — change-only ingest: reads from performer_external_ids (217 teams), uses upsert_espn_*_snapshot RPCs to skip-if-unchanged. Daily cron still UNSCHEDULED — needs CRON_SECRET set + frequency decision (suggested: 6h for standings/injuries, 1h for news, 5min during live games). Manual trigger via `POST /functions/v1/espn-collect` with `x-cron-secret: <value>`.

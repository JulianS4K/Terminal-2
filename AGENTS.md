...
- WAIT cowork — **espn-gameday post-state pickup is broken**. Discovered 2026-05-08 in Knicks deep-dive. Cron `espn-gameday-10min` has been running every 10 min since 2026-05-07 23:25 UTC, 99 successful runs, 0 failures. But: only **3 new rows** in `espn_event_snapshots` total since cron started, vs ~140+ expected (every game in NBA/NHL playoffs that flipped pre→in→post in those 16h). The 1 distinct post-state event we have is an MLB game from before cron baselined. Knicks G2 was played 5/7 evening, never captured. Same staleness pattern in `espn_team_snapshots` (last 23:08 UTC), `espn_news` (last 23:10), `espn_injuries_snapshots` (last 23:23) — everything stopped capturing within minutes of cron firing. Likely root cause: `content_hash` on espn_event_snapshots is computed over a payload that doesn't differentiate state (or status_short) so pre→in→post transitions hash identically and the change-only RPC says "no change, just bump last_seen_at." Verify by reading the espn fn's `upsert_espn_event_snapshot` RPC to see what fields go into the hash. Probable fix: include `state` + `status_short` + `home_score` + `away_score` in the content_hash. Until fixed, the chart's "last 5" strip is empty for any team because we have no post-state rows.

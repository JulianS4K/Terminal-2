...
- DONE <pending-sha> — **chat fn v27** (cowork lane, user directive): listings cap raised from 6/12 to a hard 10, schema default 10. New system-prompt rule: when `total_matching_filters > count` the bot must ask ONE filtering question (qty / row / budget / section) before showing 10 raw listings. `find_better_seats` left at 6/12 (different UX, showing alternatives not match-dump). Source committed; v27 deploy via supabase MCP pending in next turn (60KB source borderline for single tool-call context budget).

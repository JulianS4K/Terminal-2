...
// ============================================================
// Ticket Evolution client (TS port of evo_client.py)
// ============================================================
...
/** Retry on 429 (TEvo rate limit) with exponential backoff. */
 ...
 *  Adds `perItemDelayMs` pacing after each item to stay under rate limits.
 *  Returns results + count of errors. Never throws on individual failures. */
...
Deno.serve(async (req) => {
  ...
  // Resolve TEvo creds — DB first, env fallback.
  let token: string | undefined, secret: string | undefined;
  try {
    const { data } = await db
      .from("settings")
      .select("key,value")
    ...
    token = byKey.tevo_token;
    secret = byKey.tevo_secret;
  } catch (_) {
    // settings table doesn't exist or unreachable — fall through
  }
  token = token || Deno.env.get("TEVO_TOKEN");
  secret = secret || Deno.env.get("TEVO_SECRET");
  if (!token || !secret) return json({ error: "no TEvo creds in settings or env" }, 500);
  ...
  // Load watchlist (either one row or all rows)
  ...
  // --- phase 1: gather events (sequential — usually just 1-2 calls per watchlist row)
  ...
  // --- phase 2: upsert events + watch_sources
  ...
  // --- phase 3: stats, parallelized with concurrency cap + rate-limit handling
  // TEvo allows ~5 requests/sec. Concurrency 3 × 200ms pacing ≈ 15 req/sec peak,
  // which gives headroom for the withRetry backoff to absorb any spikes.
  ...
  const { results, errors } = await mapPool(
    ...
    200, // ms pacing between calls per worker
  );

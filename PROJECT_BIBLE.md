...
| **SG 429 rate CRITICAL (peak hours)** | ✅ **CLOSED 2026-05-28** — mig `350000`: peak burst 25→8. Combined with mig `010000` (60d crons 8→4). pct_429 recovering at 21:00 UTC. Monitor `v_sg_broker_429_health`; threshold >15% → investigate further. | — |
...
| **sweep_old_listings "not unique" (100% failure)** | ✅ **CLOSED 2026-05-28** — mig `360000`: DROP 0-arg overload; batched loop; cron explicit arg. | — |
| **sweep_old_sg_listings timeout (100% failure)** | ✅ **CLOSED 2026-05-28** — mig `360000`: 10K-chunk/90s batched loop replaces unbounded DELETE. | — |
| **td_sh movers ON CONFLICT (all 5 windows)** | ✅ **CLOSED 2026-05-28** — mig `370000`: DISTINCT ON horizon CTE; all windows re-seeded. | — |
| **SG null-listings vicious cycle (141+ events dark)** | ✅ **CLOSED 2026-05-28** — mig `300000`: 30d ownership fallback + COLD→COOL floor + 48h sweep TTL + min_interval 1440→1200. | — |
| `listings_aq_backfill_overnight` 120s timeout despite `SET LOCAL` | **Open** — `SET LOCAL statement_timeout='3min'` inside a pg_cron DO block is overridden by pg_cron system-level 120s hard limit. `match_unmatched_listings_chunked(25, true)` itself may need restructuring, or the cron needs to call a standalone SECDEF fn to escape the system timeout. | A1 |
| D4 `exos-media` storage: owner-scoped read policy missing | **Open (62h+)** — storage DDL not yet applied: `CREATE POLICY … ON storage.objects WHERE bucket_id='exos-media' AND owner=auth.uid()`. Flagged by D4 in bot_chat #562. | A1/B1 |
| D1 JSON-LD CSP sha256 mismatch | **Open (~48h)** — sha256-hash in `app.py` CSP header `script-src` doesn't match inline `<script type="application/ld+json">` in store pages. bot_chat #558. | D1/B1 |

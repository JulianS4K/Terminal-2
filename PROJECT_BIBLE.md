...
| A per-event cron loop bounded ONLY by `SET LOCAL statement_timeout` inside the loop | `LOOP … SET LOCAL statement_timeout='90s'; PERFORM heavy(event); … END LOOP;` | **`SET LOCAL statement_timeout` inside a running function is a NO-OP on the in-flight statement** (empirically verified 2026-07-01: a nested `pg_sleep(4)` under `SET LOCAL '2s'` ran the full 4s, uncancelled). Only a **top-level** `SET statement_timeout='…'` as its own statement *before* the call caps a nested call (verified: cancelled at 2s) — which is what the cron command carries. So a per-event loop with no other bound runs unbounded. **With `cron.use_background_workers=off` this is the scheduler-wedge cause**: one runaway job holds an open txn that blocks `cron_should_fire()`/the launcher → the WHOLE scheduler freezes (3× on 2026-07-01, ~13h degraded). **RULE — any cron whose function loops with per-iteration heavy work MUST carry a loop-level wall-clock budget:** `v_start timestamptz := clock_timestamp();` in DECLARE + `EXIT WHEN clock_timestamp() - v_start > interval 'Ns';` at the top of the loop. Defense-in-depth: (1) that loop budget, (2) the cron command's `SET statement_timeout='Nmin'; SELECT fn() …` hard cap, (3) the durable FastAPI scheduler watchdog (`scheduler_watchdog_kill_stale_backends`, mig 20260701182336; `server.py _scheduler_watchdog_loop`) that terminates any backend active >5min once the scheduler goes stale >6min. Exemplars (mig 20260701190000): `backfill_stale_zone_metrics` (90s), `evo_event_backfill_queue` / `sg_event_backfill_queue` (60s). (A1 2026-07-01) |

...
> **Doc version:** v2.16.2 (2026-07-03; §2.5 drift — removed the stale "still-inline broker/store" claim (decomposition complete; all route handlers in `routers/*`), corrected the RULE-2 guard client count 9→10, added `open_notebook` to the router list + `bandsintown` to the A1 client list) · v2.16.1 (2026-07-03; accuracy sweep — synced the §nav "what already exists" pointer counts to the `RESOURCES_BIBLE` owner: edge fns 29→33 (catalogued `broadway-cast-collect`+`notebook-llm`), tables/views/crons 212/178/164→217/203/178). Full prior doc-version history → [`docs/archive/2026-07-02-doc-version-history.md`](docs/archive/2026-07-02-doc-version-history.md).

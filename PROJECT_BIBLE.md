...
## 3. Column-name landmines (catch SQL bugs before you author) *(v1.1 · A1 · 2026-05-31)*
...
- `sg_to_tevo_search_bridge_30min` cron → fills `aq_event_map.tevo_event_id` (SG-only events)
- `aq-to-tevo-search-bridge` edge fn + `aq_to_tevo_search_bridge_13h` cron → **non-SG analogue** (mig 20260601120000, PR #392): actively searches TEvo `/v9/events` for **TM / Vivid / ScoreBig(SH)** hub rows that have no `sg_event_id`, and fills `aq_event_map.tevo_event_id`. Selector `aq_tevo_search_candidates(p_limit, p_backoff_hours)` = never-tried-first anti-join (service-role); venue-signal guard (venue sub-score ≥ 25) blocks same-name/diff-venue false positives.
  - **Read-only mapping-health surface for any bot**: **`aq_tevo_search_attempts`** (PK `aq_id` → `aq_event_map.id`; `result ∈ matched / no_results / low_score / errored`; `meta jsonb`). Query it to gauge **non-SG mapping coverage** — e.g. `SELECT result, count(*) FROM aq_tevo_search_attempts GROUP BY 1`. `matched` rows are terminal (the hub row leaves the candidate pool once `tevo_event_id` is set); `no_results`/`low_score` retry after the 24h backoff. Pairs with the SG-side ledger `sg_tevo_search_attempts`.

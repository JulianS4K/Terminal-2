...
4. **Render writes are an Applier action** (§2.1) — reads are open to every session; a write to any service requires the Applier principal (any session, operator-directed — not lane-gated). Surface *ownership* (§2.7) ≠ standing write authority; the former "D0 workspace-wide parity" was retired 2026-07-02.
5. **Push to `main` is per-task** — maintained per-task by whichever session does the work, after green CI (reorg 2026-06-17, supersedes "A1 sole pusher"). **Prod-DB mutation is an Applier action** — any session may apply its own reviewed migration under operator direction; the A1/B1/D0 codes are naming conventions (surface ownership), not a chain of command. See §2.1.
...
| **Builder** | Author-only, per-task: edit any surface it **owns** in §2.5 (html/js/css/routes/migration *files*) · open a PR · push its own work to `main` after green CI. A DB change is authored as a migration *file*, then applied under the Applier action (which any session may take, operator-directed — §2.1 Applier row) | every session building on a surface |
...
*Owner* = which lane owns the service **surface**; a *write* to any of them is an **Applier** action (§2.1), performed by any session under operator direction (not lane-gated; the former "D0 workspace-wide Render parity" was retired 2026-07-02 — surface ownership ≠ standing prod write authority).
...
| Supabase mutation / `apply_migration`; cron `schedule`/`unschedule`; Vault; Render **write**; data-mutating edge-fn deploy | **Applier** (§2.1) — any session, operator-directed |

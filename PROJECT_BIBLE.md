...
> **Doc version:** v2.40.0 (2026-09-09; §4 — `aq_link_gotickets_by_event_match()` widened and **bug-fixed**: uniqueness must be **bidirectional** (TEvo lists a multi-performance run as N events while GT lists one, so three `Dolly` performances all claimed one GT event); candidates widened from CRM-sold to **owned-inventory** events; exact-name equality added for single-word acts. Records that the GT-coverage ceiling is **~86%, not 100%** — 77% of the residue simply is not in GoTickets' catalogue.) · v2.39.0 (2026-09-09; §4 — catalogued `aq_link_gotickets_by_event_match()`, the **derive** counterpart to the **trust**-the-catalogue linker, reaching the 97.4% of GT events GT never mapped itself. Records the **6 false matches** its first under-guarded dry run produced (a concert → a CAMPING PASS; two Big Ten sessions → the same wrong session; three Miami Open sessions → Grandstand) and the three guard families that stop them. Both linkers now run in the daily sweep.) · v2.38.0 (2026-09-09; §4 — the venue+event resolver is now a **daily self-running job with a bot_chat ping** (crons 588/589). Records the two defects operating it exposed: the sweep was **future-only** so it could never reach an event the mapper's `-7 days` window still wanted, and 5 of 8 "stuck" venues were just noisier spellings of resolved ones — fixed by a prefix-adopt rule that must still clear the §4 Memorial-Stadium generic-token trap.) · v2.37.0 (2026-09-09; §4 — catalogued the **TEvo venue+event resolver**: `/v9/events?name=` needs a near-exact name while `/v9/searches` is fuzzy, which is why 297 college hub rows looked absent from TEvo; it needs **no edge function** (pg_net + `tevo_sign_get`); records the two 401-causing signing landmines, the nested-venue guard split, the `Spartan Stadium (Michigan)` false match, and that venue ids are worthless without the per-venue event sweep. Coverage 90.4%→93.8%.) · v2.36.0 (2026-09-08; §4 — catalogued `aq_link_gotickets_from_catalogue()`: the GoTickets catalogue already held a `tevo_event_id` the hub never read (471 rows filled, GT reach 55.4%->77.4%). Records that the catalogue's mapping is **wrong at score 1.000** (season-ticket packages -> single games) so `map_score` is no gate, and that **"GT" means GoTickets, never Gametime**.) · v2.35.0 (2026-09-08; §4 — recorded that `aq_tevo_search_candidates()`'s `sg_event_id IS NULL` exclusion was **guarding a race that was not happening**: the SG side had resolved 0 of the 295 SG-owned unmapped future hub rows, so ~1,084 CRM orders sat in a pool neither bridge drained. Widened with a live SG-resolution race guard.) · v2.34.0 (2026-09-08; §4 — added the **state-hint venue-resolution guard** (the state is in the string; the ambiguity guard cannot see world-ambiguity the map does not hold) and twelve more seeded CFB venue ids.) · v2.33.0 (2026-09-08; §4 — added the **mixed-timezone `aq_event_map.event_date` landmine** (UTC rows shift a day against `occurs_at_local` and bind the wrong opponent; the two-token floor does not catch it) and the DKR/Texas venue seed.) · v2.32.0 (2026-09-08; §4 — catalogued `link_aq_tevo_from_events()`: the no-API hub fill that was scheduled NOWHERE, and the between-two-pipelines gap it closes for rows carrying an `sg_event_id`.) · v2.31.0 (2026-09-08; §4 — added the **parking-guard landmine** (start-anchored guard + `split_part` stripping the venue marker bound 188 parking orders to their events) and the ambiguous-prefix/unaccent resolver fixes.) · v2.30.0 (2026-09-08; §4 — catalogued `s4kcs_venue_alias_derive()` (CRM venue aliases learned from mapped orders; the ≥2-event floor; the `UPDATE … FROM` one-source-row-per-target trap) and `s4kcs_map_events()` rules 6-7 (venue-identity matching, +917 orders; why rule 7 must never prefix-match; the cartesian-product plan that times out in a session but not under cron).) · v2.29.0 (2026-09-08; §4 — catalogued `cross_source_venue_resolve()` with the **bare-"Memorial Stadium" prefix landmine** (resolves to OKLAHOMA; ≥10 same-named stadiums; a wrong venue id scores 50 in the bridge), the two alias columns it was not reading, and six hand-seeded CFB venue ids.) · v2.28.0 (2026-09-08; §4 — catalogued `venue_xref_derive_from_events()` + `v_marketplace_venue_xref`: cross-marketplace venue ids derived by event agreement into the EXISTING `cross_source_venue_map` (six partial venue-xref surfaces already exist — populate, don't add a seventh); records that Vivid/GoTickets publish no venue id, and the generic-word stop-list the GT alias guard needs.) · v2.27.0 (2026-09-08; §4 — catalogued `aq_link_tevo_from_sibling_rows()`: hub-to-hub sibling fill so the AQ→TEvo bridge only ever searches genuinely unmatched events, with the two guards that make it safe and the Monster Jam false match that motivates the second.) · v2.26.0 (2026-09-01; §3 — added the S4K CRM batch-dedupe landmine: the feed can repeat a `(source, id)` pair inside one payload, aborting the whole upsert batch; intermittent, so a clean manual run proves nothing.) · v2.25.0 (2026-09-01; §3 — two landmines from the S4K CRM ingest: `order_status_xref`'s unique key is **3 columns** (`source, source_status, source_status_kind`) so a 2-column ON CONFLICT fails 42P10; and the CRM feed ships `"None"`/empty strings in typed date and price fields, which abort a whole batch on a bare cast.) · v2.24.0 (2026-09-01; §6b — the substitution checker now emits the buy-side link itself as `buy_url`; read it from the response instead of hand-building one.) · v2.23.0 (2026-09-01; §6b — added the **buy-side GoTickets link format** (`pro.gotickets.com/tickets/<gt_event_id>/…?sections=<section_id>`): the terminal is the analysis surface, but an answer naming specific GT inventory now links where it can be bought. Both ids are columns on `gotickets_listings_snapshots` — `section_id` IS GT's URL section id, so it is a lookup, never hand-mapped from the section label.) · v2.22.0 (2026-07-27; added the **D6 · Paciolan/eVenue** product lane (browser-extension-sourced primary box-office ingest — `paciolan_client.py`·`paciolan_extension/*`·`routers/paciolan.py`·`paciolan_*` schema; consecutive-seat blocks modeled on AXS `v_axs_seat_groups`) across §2 lane map / §2.3 products; new upstream `evenue.net` locked read-only in `check_readonly.py` (`paciolan_client.py` GET-only); companion labeler `area:d6` + CODEOWNERS + title-lint lane entry. Lane count seven→eight.) · v2.21.0 (2026-07-08; full drift sweep — added the **D5 · Fantasy** product lane (sports FE `static/fantasy/*` + the `/fantasy/*` page-route block spun off from the terminal, served by the shared `vibepass-storefront-test` shell; data plane stays A1's ESPN/fantasy engine) across §2 lane map / §2.3 products / §2.5 code map / §2.7 deploy — closing the no-orphan gap; §2.3 `*_client.py` clarified 9 A1-owned of 10 total; §2.5 router list +`eventbrite`; §2.4 A1 writer set +athlete-registry / fantasy-engine / news-wire (Reddit·X·FRED) / zone-metric / Eventbrite tables; §nav counts re-synced to a fresh live `RESOURCES_BIBLE` §0 snapshot (283 tables · 213 views · 210 crons · 38 edge fns).) · v2.20.0 (2026-07-08; §3 — pg_cron row gained **(e) GATE CLAMP**: `cron_should_fire()`'s work-check wrapper "restored" a hardcoded txn-local 120s `statement_timeout` that outlives the gate call inside the job's single implicit transaction and clamps every later statement in the command, overriding in-string budgets — the real source of the movers exact-120s failures (mig 20260708042000 capture-and-restore fix; caught by the check-poll-mapping-health deadman sweep).) · v2.19.0 (2026-07-04; §2 consistency — dropped the stale `C1` from the §2 ownership-label list (now `A1/B1/D0–D4`, matching the hierarchy-dissolved statement; C1/D2 are retired historical tokens); companion peripheral-doc sweep retired the last A1-centralization / four-domain / "supervisor" language (README, release-discipline, open_notebook, bot_chat_conventions, b1_operating_constraints, KANBAN routing note).) · v2.18.0 (2026-07-03; merged #778 drift fixes — §2.5: removed stale inline-broker/store claim (decomposition complete), RULE-2 guard client count 9→10, added `open_notebook` router + `bandsintown` A1 client; §nav "what already exists" counts synced to RESOURCES_BIBLE owner: edge fns 29→33, tables/views/crons 217/203/178) · v2.17.0 (2026-07-03; §1.5/§2.1 — retired "today only A1 sessions" gatekeeper language: the **Applier** action (prod-DB apply, Render writes, cron/Vault, edge-fn deploys) is taken **per-task by any session under operator direction**, not centralized on A1. The A1/B1/D0 codes are naming conventions for surface ownership, not a chain of command — hierarchy stays dissolved.) · v2.16.0 (2026-07-02; §2.5 code map: added the `bandsintown` router (#693) + shifted the shared-client counts (9 GET-only · 7/9 http_retry · 5 vault) for the new Bands in Town client) · v2.15.0 (2026-07-02; accuracy sweep — reconciled storefront-service ownership (§2.3/§2.5 ⇄ §2.7 deploy table), added `d0_sales` router + `core/concurrency`·`core/seo` to the §2.5 code map, corrected the server.py inline-routes claim, marked `auto_match_sg_canonical_v3` + `sg_listings_poll_owned_1min` inactive, repointed the `_compute_movers` exemplar to `routers/broker.py`/`core/movers`) · v2.14.0 (2026-07-02; §3 landmines refreshed from the 2026-07-01/02 cron incident — pg_cron in-string `SET` DOES arm per-statement but LEAKS across jobs via persistent pooled connections (own-prefix or `BEGIN/SET LOCAL/COMMIT`), migs 20260702075500/20260701224500; + per-row trigger index-probes stall firehose ingest, mig 20260702000500) · v2.13.0 (2026-07-02) — **tier-2 lane consolidation (KANBAN C1-OPS-3):** retired two vestigial first-class lanes — **C1 folded into B1** (docs governance *is* the Auditor principal's work, so its surface — bibles, registry, `bot_chat`, seam register — belongs with the Auditor's home lane; B1 = build + guards + governance) and **D2 folded into D0** (both broker-facing; D2 wrote no tables). C1's former DB sidecars (`*_canonical`/`*_resolved`/`seatgeek_event_xref`) moved to A1, reconciling the cross-source-xref contradiction. Net: **6 lanes** — substrate A1·B1 + products D0·D1·D3·D4. `c1`/`d2` survive only as historical branch/KANBAN/`bot_chat`-tier tokens (the `p_level` DB constraint is untouched). · v2.12.0 (2026-07-02) — **§2 restructured to finish the ownership/permission/priority convergence:** permission collapsed to **3 principals** (Applier/Auditor/Builder, §2.1) with a dated **exceptions ledger** (§2.2) and an **ownership-label legend** (§2.3); priority/status (ACTIVE/PAUSED/★) evicted to `KANBAN.md`; §2.4 order tables reconciled to A1 (single-writer); §2.6 seam review mechanized as the non-blocking `seam-review` CI job; §6 MCP scope stated by principal; hard rules §1.4/§1.5 aligned. **The actor hierarchy is dissolved** — A1/B1/C1/D0–D4 are now flat *lane identifiers* (regions of the ownership map), with no ranks or chain of command (`A1→B1→C1` / "under" / "sub of" removed; `docs/bot-hierarchy.mermaid` reframed to a flat map). §2.3 recut from a flat 8-peer list into a **platform substrate (A1/B1/C1) × products (D0–D4)** grid with the `(product block) × (substrate layer)` ownership rule — the `server.py` route-block carve-out is now the stated rule, not an exception. §2.2 exceptions ledger made **CI-enforced** — `.github/permission-exceptions.yml` + the blocking `permission-exceptions` job (`scripts/check_permission_exceptions.py`): an expired/malformed entry fails the build, so a grant can't silently become permanent. Ownership paths (§2.5 code map + §2.6 seam register) unchanged. Full prior doc-version history → [`docs/archive/2026-07-02-doc-version-history.md`](docs/archive/2026-07-02-doc-version-history.md).
...
| **What already exists** (before creating a table/view/RPC/cron/edge-fn — reuse, don't rebuild) | `RESOURCES_BIBLE.md` (283 tables · 213 views · 210 crons · 38 edge fns) |
...
LANE MAP (codes name cells in a layer × product grid — flat, no ranks; §2.1 sets permission, §2.3 the grid):
  ...
  PRODUCTS (each a distinct surface that sits on the substrate; some share a deploy service):
    D0 terminal + orders-dashboard   D1 store   D3 broadway   D4 Bridge/Exos (full app)   D5 fantasy (sports)   D6 paciolan/eVenue (primary box-office ingest)
...
**The codes are not a flat peer list.** They name cells in a **layer × product** grid: a two-area **platform substrate** (the shared foundation) and the **products** that sit on it — **eight lanes** (substrate A1·B1 + products D0·D1·D3·D4·D5·D6). Two codes were retired as first-class lanes on 2026-07-02 (see the note below the tables).
...
|---|---|---|
| **A1** | data + framework | Supabase tables/migrations/crons; the **AQ mapper** + cross-source xref (incl. `*_canonical`/`*_resolved`/`seatgeek_event_xref`); the 9 A1-owned read-only `*_client.py` (of 10 total; `broadway_client.py` is D3's) + edge functions + **order ingestion**; `server.py`/`routers/*`/`core/*` as a package; DB-layer security (RLS/SECDEF/RULE-2); data-freshness + 429/cron monitoring |
...
| **D5** | Fantasy (sports) | `static/fantasy/*` + the `/fantasy/*` page-route block (in `routers/pages.py`, spun off from the terminal 2026-07); the fantasy FE over A1's ESPN/fantasy data plane (leagues, athlete drafts by trailing form, lineups, weekly scoring) — hosted on the D0-owned `vibepass-storefront-test` shell (surface ≠ service, like D1 — see §2.7); UX + speed testing |
| **D6** | Paciolan/eVenue (primary box-office) | `paciolan_client.py` (the seat-map normalizer + gaps-and-islands grouper), `paciolan_extension/*` (the Chrome collector), the `/api/paciolan/*` route block (`routers/paciolan.py`), and `paciolan_*` schema (`paciolan_event/section/seat_snapshots`, `v_paciolan_seat_groups`, `v_paciolan_listings`, `paciolan_ingest*`). Browser-extension-sourced primary box-office inventory (Paciolan/eVenue), categorized as platform `paciolan`; consecutive-seat "N together" blocks modeled on AXS (`v_axs_seat_groups`). RULE-2 lockdown of `evenue.net` is A1/B1 policy (§2.1 Auditor) over D6's surface. Migration application is an Applier action (A1). |
...
|---|---|
| **A1** | `event_xref`, `*_xref`, `event_listing_snapshot_daily`, sweep fns, `latest_event_metrics` matview, ESPN tables + the **athlete registry** (`espn_athletes` + Wikipedia enrichment) + the **fantasy-engine** tables, the **news-wire** tables (Reddit / X / FRED macro), **zone-metric** rollups, `td_discovered_events` + the **Eventbrite** xref, `event_movers_index(_history)`, `discovery_gap_alerts`, **the order + ingest tables** (`evo_orders`, `seatgeek_orders`, `tickpick_orders`, `vivid_orders`, `seatdata_sales_snapshots` — order *ingestion* is A1's data plane, §2.3), **`*_canonical`/`*_resolved` sidecars + `seatgeek_event_xref`** (cross-source data, moved from C1 2026-07-02), **`pg_policies`/RLS** |
...
| `static/fantasy/*` + the `/fantasy/*` page-route block (in `routers/pages.py`) | **D5** | fantasy (sports) FE over A1's ESPN/fantasy data plane; the route block is D5 behavior inside A1's `pages.py` structure (product block × substrate layer, §2.3); served by the shared `vibepass-storefront-test` shell (§2.7) |
...
| `vibepass-storefront-test` | `srv-d8140bnaqgkc73al4asg` | **D0** | D1 · D5 | `server.py`, `static/store/*`, `static/fantasy/*` | `render.yaml` |
...
| `order_status_xref` ON CONFLICT | conflict-target `(source, source_status)` — the pair the `unified_orders` branches join on | **The unique key is `(source, source_status, source_status_kind)`.** `source_status_kind` defaults to `'state'` but is NOT implied by a 2-column conflict target: `ON CONFLICT (source, source_status)` fails the apply with `42P10: there is no unique or exclusion constraint matching the ON CONFLICT specification`. Name all three. (caught applying mig 20260901180000) |
| S4K CRM `/marketplace/orders` batch | upsert the `rows` array straight into `ON CONFLICT` | **Dedupe first — `SELECT DISTINCT ON (source, id)`.** The feed can repeat a `(source, id)` pair within ONE payload (seen as a byte-identical duplicate), and Postgres then aborts with `ON CONFLICT DO UPDATE command cannot affect row a second time` — failing the entire 22k-row batch, not the offending row. Same class as the SG-sales dedupe in §7. It does NOT reproduce every pull, so a clean manual run proves nothing; the scheduled run caught it. (mig 20260901180000) |
| `s4kcs_orders` date/number fields | cast the CRM's `event_date`/`inhand_date`/`payout_date`/`purchase_date`/`price` straight with `NULLIF(x,'')::date` | **The feed ships junk in typed fields** — empty strings in ~20k `payout_date` / ~6k `inhand_date` / ~2.9k `price`, plus the literal string `"None"` (a stringified Python None) in `inhand_date`. A bare cast aborts the WHOLE 22k-row batch. Use `s4kcs_as_date()` / `s4kcs_as_num()`, which accept only a well-formed value and NULL everything else. (mig 20260901180000) |
...
| `aq_tevo_search_candidates(p_limit, p_backoff_hours)` | int, int | **Non-SG AQ→TEvo cold-search selector** (mig 20260605133500). The non-SG analogue of `sg_to_tevo_search_bridge` — selects TM/Vivid/SH hub rows that carry no canonical `tevo_event_id` and were never (or stalest) searched against TEvo `/v9/events`, never-tried-first. Drained by an edge fn; writes `aq_tevo_search_attempts`. ⚠ **The name is now half wrong — it is no longer strictly "non-SG"** (mig 20260909004500). It carried `AND m.sg_event_id IS NULL` since 2026-06-05 so the two bridges could not race for the same TEvo request. Sound instinct, wrong population: measured over the 295 SG-owned unmapped future hub rows, the SG side had resolved **zero** — 0 with a `tevo_event_id` in `sg_events_canonical`, 0 `seatgeek_event_xref` rows, 126 not in `sg_events_canonical` at all. The exclusion was guarding a race that was not happening while parking ~1,084 CRM orders in a pool neither bridge drained (the same between-two-pipelines gap `link_aq_tevo_from_events()` closes from the other side). Now admits an SG-owned row **only while SG shows no resolution** — two `NOT EXISTS` against `seatgeek_event_xref` and `sg_events_canonical`, so a row leaves this pool the moment the SG pipeline actually claims it and the race guard still holds. First run after: 60 attempted, 24 matched, 0 errored — **every match came from a newly-admitted row** (the SG-owned split was 24 matched / 32 no_results / 1 low_score). Future CRM order coverage 87.0% → 87.7%. |
| `tevo_venue_daily_enqueue()` / `tevo_venue_daily_harvest()` / `tevo_venue_alias_adopt()` | — | **Daily self-running venue+event sweep + ping** (mig 20260909180000; crons 588 `5 9 * * *` / 589 `35 9 * * *`). Two crons 30 min apart because pg_net is async and its queue runs ~200 deep — same queue/process shape as the s4kcs ingest. ⚠⚠ **DEFECT IT FIXES #1: the sweep could never reach an event that had already happened.** `tevo_venue_events_enqueue()` hardcoded `occurs_at.gte=current_date`, but `s4kcs_map_events()` rules 6/8 look back to `current_date - 7 days` — so an order whose event was yesterday was structurally unmappable however well its venue resolved. New `p_since`; the daily job passes `current_date - 10` so the sweep **overshoots** the mapper's window instead of trailing it. ⚠⚠ **#2: stuck venues that were never stuck.** The first ping showed 5 of its 8 top blocked venues were noisier spellings of ALREADY-resolved ones (`Husky Stadium Seattle`→699, `Boone Pickens Stadium at Lewis Field`→2356, `The Woodlands of Dover International Speedway`→48005). No API call could fix those. `tevo_venue_alias_adopt()` takes a resolved id when its normalised name is a **whole-word PREFIX** of the stuck one and states agree — but a bare prefix rule **IS the §4 Memorial Stadium landmine**, so the prefix must also carry ≥1 DISTINCTIVE token (stop-list applied): `memorial stadium` is wholly generic and can never adopt, `husky stadium` adopts on *husky*. All 8 first-run adoptions hand-checked. **Every resolved venue is re-swept daily**, not just new ones — schedules move and events get announced, so yesterday's sweep is stale (~80 GETs/day, one per venue not per event). **The ping** is one `bot_chat` status row: resolved/no_match/ambiguous, events upserted, orders mapped, live coverage %, the count of **genuinely** unmapped orders (parking/season-ticket/cancelled excluded, so it is actionable not alarming), and top stuck venues + the remedy. It posts EVERY day, unlike `mapping_health_check()` — a silent progress report would hide a resolver that had quietly stopped resolving. First run: 8 aliases adopted, 1,085 events, **328 orders mapped**, bot_chat 3576. |
| `tevo_venue_search_*` / `tevo_venue_events_*` | int / — | **TEvo venue-id + event resolver** (mig 20260909030000). Closes the last structural mapping gap. ⚠⚠ **`/v9/events?name=` needs a NEAR-EXACT name; `/v9/searches` is FUZZY** — measured live: `name=South+Dakota+State+Jackrabbits+Football` returns **0**, while `/v9/searches?q=Jackrabbits` returns the performer whose venue is 33170 *Dana Dykhouse Stadium*, with upcoming events. That single difference is why **297** college hub rows sat on `no_results` looking like TEvo simply lacked them. ⚠ **NO EDGE FUNCTION NEEDED** — this was long carried as blocked on an edge-fn deploy because TEvo needs HMAC; `tevo_sign_get()` + pg_net do it from plain SQL. **Two signing landmines, each a 401:** query params must be **alphabetical** (the HMAC signs the string as sent), and `tevo_sign_get(p_path,…)` **prepends the host itself**, so `p_path` is `/v9/searches`, never `api.ticketevolution.com/v9/searches`. **`entities=venues` is NOT honoured** (Performers come back too — check `_type`), and many college venues exist ONLY nested in a Performer's `venue`, so the harvest has two paths with different guards: *direct* needs ≥1 distinctive token (generic-word stop-list applied), *nested* makes **state agreement mandatory** plus ≥2 loose tokens — `Memorial Stadium Nebraska`→1093 passes, the same response's `NHL Stadium Series`→MetLife fails. **A false match it actually produced:** `Spartan Stadium (Michigan)`→1002 *Michigan Stadium* (Ann Arbor) — Spartan is Michigan STATE's, both in MI so the state guard could not separate them and the CRM's **parenthetical** supplied the only token; caught by hand-auditing all 80, unbound, and `tevo_venue_search_norm()` now strips `(...)`. **Venue ids alone are nearly worthless** — promoting 80 moved coverage by TWO orders, because the EVENTS were still missing; the sweep (`/v9/events?venue_id=`, **one call per venue, not per event**) pulled 1,114 events and rule 8 then mapped **972** orders. Store `occurs_at_local` as local wall-clock (`left(...,19)`) — converting to UTC would re-create the §3 mixed-timezone landmine inside the mirror. Coverage 90.4% → **93.8%**. |
| `aq_link_gotickets_by_event_match()` | — | **GoTickets link DERIVED, not trusted** (mig 20260909190000). The opposite direction to `aq_link_gotickets_from_catalogue()`, and the distinction matters: that one **trusts** `gotickets_event.tevo_event_id` and so reaches only the 4,641 of 228,102 rows GT mapped itself; this one **derives** the link where the catalogue has no tevo id at all — 160,427 future GT events, **97.4% of the catalogue** — by matching our `events` mirror to GT on resolved venue + date + name, never reading GT's tevo id. Measured 2026-09-09: 368 future events had CRM orders but no GT id (3,297 orders); this matched **22** (8% yield — GoTickets genuinely does not list most of that tail), covering 256 orders. ⚠⚠ **THE FIRST DRY RUN PRODUCED 6 FALSE MATCHES** because it did not reuse `from_catalogue()`'s guards — caught in the dry run and never written, but only because it was read row by row: `Jack Johnson` (the concert) → **`Jack Johnson Camping - 3 Day Pass`** @ *Gorge Amphitheatre Camping*; `Big Ten Volleyball Session 2` **and** `Session 3` → both to `Session 1`; `Miami Open Session 5/18/19` → `Grandstand Session 2/17`. So three guard families are load-bearing: **session-number equality**, **camping/grandstand/grounds-admission/'pass only' exclusion on both names AND the GT venue string** (non-ticket products and sub-venues — same class as the parking guard), and season-ticket/parking. Then `aq_name_consistent()` + ≥2 shared ≥4-char tokens, then exactly ONE GT event may survive per TEvo event. Date tolerance is ±1.5 days deliberately: GT stores UTC, `occurs_at_local` is local (§3). ⚠ **Resolve GT venue strings ONCE into a temp table**, restricted to needed dates — 228k rows through `cross_source_venue_resolve()` per candidate never finishes. Never overwrites an existing id. Runs inside `tevo_venue_daily_harvest()` **after** the mapper (newly-mapped events are new GT candidates); both linkers' counts appear in the daily ping. **WIDENED + BUG-FIXED 2026-09-09 (mig 20260909200000):** ⚠⚠ **uniqueness must be BIDIRECTIONAL** — one-way (one GT per TEvo) let THREE `Dolly - A True Original Musical` performances (TEvo 12-26, 12-26, 12-28 @ St. James) all claim ONE GT event on 12-27, because **TEvo lists a multi-performance run as several events while GT lists one**; 11 GT events were being multiply-claimed. ⚠ Candidates were also the wrong POPULATION: joining `s4kcs_orders` only saw events we had **sold**, missing every event we hold **inventory** on — now `(CRM orders) UNION (owned listings, 30d)`. ⚠ Exact normalised-name equality added as an alternative to the ≥2-token floor, which rejects every **single-word act** (`Juanes` shares exactly one token with `Juanes`). ⚠ Venue strings now come from `cross_source_venue_map` by **plain join** — calling `cross_source_venue_resolve()` per distinct GT venue name blew the 60s ceiling. Result 81.9% → **83.0%** of future events we own listings on. **CEILING IS ~86%, NOT 100%:** of the 741 unlinked, **574 (77%) have NOTHING in the GT catalogue at that venue+date** — GoTickets does not list them, so the residue is an inventory question, not a matching one. The 13 pre-existing many-to-one `gotickets_event_id` values (26 hub rows, all carrying `gotickets_map_score`, i.e. from GT's own feed — Circus Vazquez 16:00/19:00, Disney On Ice 10-30/10-31) are left alone; none came from this linker. |
| `aq_link_gotickets_from_catalogue()` | — | **GoTickets catalogue -> hub fill** (mig 20260909021500). `gotickets_event` already carries a `tevo_event_id` for 4,641 of its 222,434 rows; the hub never read it, so a hub row and the catalogue could hold the SAME tevo id and `aq_event_map.gotickets_event_id` still sat NULL. First run filled **471** rows (431 future, 400 distinct GT events); CRM future orders reaching a GT event id went **55.4% -> 77.4%** (Gametime-sourced 54.0->83.0, StubHub 54.1->76.7). ⚠ **"GT" HERE IS GOTICKETS, NEVER GAMETIME** — both are marketplaces we carry and both invite the abbreviation; the schema keeps them apart by always spelling Gametime out (`gametime_event_id`, `gametime_url`) while every `gt_*`/`gotickets_*` identifier is GoTickets. `evd_event_id_crosswalk` carries both, spelled out. ⚠⚠ **THE CATALOGUE'S OWN `tevo_event_id` IS NOT TRUSTWORTHY AND `map_score` IS NOT A DEFENCE** — joining on it alone imports GT's mapping errors: at score **1.000** it maps season-ticket PACKAGES to single games (`2026-2027 Lakers Season Tickets` -> `NBA Preseason - Nuggets at Lakers`), at 0.486 it matched `Florida Gators at Auburn Tigers` to `North Greenville Crusaders at West Alabama Tigers` on the word "Tigers", and at 0.536 it conflates US Open sub-venues. Meanwhile 0.457 is a CORRECT pair. So all seven guards re-derive agreement from the data. Two are non-obvious: the venue-token guard **must** subtract the mig 20260908203500 generic-word stop-list (else `Arthur Ashe Stadium` and `Louis Armstrong Stadium` share "stadium" and every sub-venue swap passes), and multi-court complexes are excluded **outright** because their sub-venues share the complex's PROPER nouns, which no stop-list can separate (`Billie Jean King National Tennis Center` vs `Louis Armstrong Stadium at the Billie Jean King National Tennis Center`). Never overwrites an existing id. Hourly `aq_link_gotickets_catalogue_hourly` at `:40`, ahead of the `:45`/`:50` hub fills. |
| `aq_link_tevo_from_sibling_rows()` | — | **Hub-to-hub sibling fill** (mig 20260908195605). Fills `aq_event_map.tevo_event_id` from a SIBLING hub row — exact normalized name + same date, where the matched rows at that (name, date) agree on **one** `tevo_event_id` — so the bridge never spends a TEvo request re-discovering an id we already hold. Additive to `link_aq_tevo_from_events()` (propagates by `aq_short_event_id`; 0 overlap) and to it/`resolve_aq_tevo_from_sources()` (both join `events` on EXACT venue equality; 49/52 of these rows differ). Mostly venue aliasing: `The Forum`/`Kia Forum`, `The Rose Bowl`/`Rose Bowl Stadium`. **Guard 1** counts DISTINCT event ids, NOT venue strings — a (name,date) group spanning 3 venue strings but 1 event id is *confirming* an alias. **Guard 2** requires a ≥4-char venue-token overlap: without it, `Monster Jam` 2026-10-17 @ Reliant Stadium (Houston) binds to the sibling @ Numerica Veterans Arena (Yakima) — simultaneous tour legs share a name and date. `city` is NOT usable as that axis (NULL on 50/52). Ungated hourly cron `aq_link_tevo_siblings_hourly` at `:50`, three minutes ahead of the bridge's `:53`. First run filled 237 (69 future, 168 past). |
| `venue_xref_derive_from_events(p_apply)` | boolean (default true) | **Cross-marketplace VENUE ID xref** (migs 20260908202400 + 20260908203500). Derives marketplace venue ids by EVENT AGREEMENT — the venue ids seen on events already mapped to a `tevo_venue_id`, accepted only where `count(DISTINCT)=1` — so it never compares two venue name strings and none of the alias problems apply. **Populates `cross_source_venue_map`, the table `cross_source_venue_resolve()` reads; do NOT create a rival venue xref** — there are already six partial ones (`entity_venue_map` 1,276 · `aq_venue_map` 712 · `seatgeek_venue_xref` 38 · `seatdata_venue_xref` 0 · `v_canonical_venue`), and a seventh is the drift generator. The unlock: `sg_events_canonical.sg_venue_id` was populated on 114 of 12,767 rows while `raw_event_jsonb->'venue'->>'id'` carried one on 8,148 across 1,293 SG venues — the column was never backfilled. SeatGeek 7 → **853** ids, TickPick → 168, 0 conflicts; the derivation independently reproduced 142 of 146 hand-built TickPick pairs and contradicted none. Writes are **fill-only** — a pre-existing id that disagrees is kept and flagged to `bot_chat`, never overwritten. **Vivid and GoTickets publish no venue id to us** (`vivid_orders.raw_xml` has no `<venueId>`; `gotickets_event` has no id column), so they are NAME aliases only — and the GT alias set needs its own guard, since GT's event→TEvo mapping errors otherwise leak in (`Yankee Stadium` ← `Woodbridge Center Lot`): keep a name only on a ≥4-char shared token EXCLUDING generic venue words, or `stadium` alone lets the bad one through. Read via `v_marketplace_venue_xref`. Daily cron `venue_xref_derive_daily` at `23 6 * * *`. |
| `cross_source_venue_resolve(name, city, state)` | text, text, text | **Venue string → `tevo_venue_id`** — the lookup the AQ→TEvo bridge depends on. Three branches in order: exact `canonical_name`, then exact alias containment, then a PREFIX match. ⚠ **The prefix branch is name-family-blind and ignores city/state.** Bare `"Memorial Stadium"` resolves to **31717 (Memorial Stadium OKLAHOMA)** because `'memorialstadiumoklahoma' LIKE 'memorialstadium%'` — and there are ≥10 distinct Memorial Stadiums in this data (Oklahoma · Indiana · Nebraska · Kansas · Illinois · Clemson · Texas/DKR · Missouri/Faurot · Navy · NMSU). A wrong venue id scores **50** in the bridge, enough to pass on a generic shared token like `"Football"`. Exposure was 0 when found (2026-09-08): no hub row uses the bare string, and the 133 CRM orders that do never reach this function — `s4kcs_map_events()` matches venue strings against the hub directly. **Suffixed forms are safe** (`'memorialstadiumin'` neither prefixes nor is prefixed by `'memorialstadiumoklahoma'`), so the fix for a new ambiguous venue is an exact alias, not a prefix. Alias columns read: `sg_`/`tickpick_`/`vivid_`/`gotickets_`/`crm_aliases` (last two added 20260908210000 — both existed as data with nothing reading them). The real fix is city/state on the prefix branch; deliberately not done under a live bridge for a latent bug with zero occurrences. |
| `s4kcs_venue_alias_derive(p_apply)` | boolean (default true) | **CRM venue-name aliases, learned from mapped orders** (migs 20260908214500 + 20260908215500). For each CRM venue string, takes the TEvo venue of every order already mapped from it and accepts only where all observations agree AND **≥2 distinct events** back it. That floor is load-bearing: bare `"Indian Wells Tennis Garden"` had ONE mapped order pointing at Stadium 1, and Indian Wells is a multi-stadium complex — a one-observation sample would have written a coin flip as fact (73 single-event strings are skipped). Writes to `crm_aliases`, which the resolver checks BEFORE its prefix branch, so an alias **corrects** a bad resolution: `"Toyota Center"` 1602 Kennewick → 2786 Houston, `"Orpheum Theatre"` 1161 Minneapolis → 2773 LA. ⚠ **`UPDATE … FROM` applies ONE source row per target** — the first cut lost a second alias on two venues silently (no error; the count was the only tell). Aggregate to one row per venue before updating. Daily cron `s4kcs_venue_alias_derive_daily` at `11 6 * * *`, ahead of the venue xref at `23 6`. |
| `s4kcs_map_events()` rules 6-7 | — | **CRM order → event, matched on venue IDENTITY** (migs 20260908220500 + 20260908222000). Rules 3-5 compare the venue STRING, so the CRM and hub must spell a building identically. **Rule 6** resolves both sides to a `tevo_venue_id` first, so every alias is matched for free (`"Reliant Stadium"` → NRG, `"The Kia Forum"` → 602, `"Memorial Stadium Indiana"` → 2044) — mapped **917 orders**, audited at 0 date and 0 venue mismatches. **Rule 7** is canonical venue-name EQUALITY (punctuation/spacing only: `Ross-ade` == `Ross–ade`) for venues with no TEvo id. ⚠ **Rule 7 must never use a PREFIX** — that is what makes bare `"Memorial Stadium"` resolve to Oklahoma, and inside a mapper the name guard will not save you, because two college football games share the token `"football"`. Both carry rule 5's full guard set (`aq_name_consistent` + the two-token matchup floor + `HAVING count(DISTINCT tevo_event_id)=1`). Rule 6 is windowed to 7 days and materialises venue resolutions into an ANALYZEd temp table — a CTE plus a comma join made it a cartesian product that completed under the cron's budget but timed out at 55s, which reads as "broken in prod" and is not. |
| **Parking guard — anchor it NOWHERE, match ANYWHERE** | — | ⚠ **A parking pass is not the event.** Every venue-keyed CRM rule guarded parking as `event_name !~* '^(parking|parking pass)'`, anchored at the START. The CRM puts it at the END — `"Cincinnati Reds at Los Angeles Dodgers Parking"` — and the venue side cannot help because rules 5-7 key on `split_part(venue_name,' - ',1)`, which turns `"Kia Forum - Los Angeles Parking"` into `"Kia Forum"`, **stripping the marker that identifies it**. A parking product then matches the real event on venue+date and sails through `aq_name_consistent()` — the names are nearly identical, which is the whole problem. Bound **188 orders / 110 events** before it was caught (mig 20260908230000); 20 predated the venue-identity work, 168 came from rule 6, which found more because the parking venue and the real venue resolve to the SAME `tevo_venue_id`. Surfaced only when the accent fix let `"Mana Parking"` @ `"Intuit Dome Parking"` reach the concert `"Maná"`. Guard with `NOT ILIKE '%parking%'` on event AND venue, on BOTH the CRM and hub sides. Verified safe: no real event name in this data contains the word. §3 lists parking as expected-NULL — this is the opposite failure, mapping it to something it is not. |
| `link_aq_tevo_from_events()` | — | **Cheapest hub fill in the system — and it was ORPHANED until 2026-09-08** (mig 20260908233000 finally scheduled it, `45 * * * *`). Joins `aq_event_map` straight to the `events` mirror on exact venue+name+date±1, one distinct TEvo event required, propagating by `aq_short_event_id`. **No API call.** ⚠ It closes a gap NEITHER bridge covers: `aq_tevo_search_candidates()` excludes rows carrying an `sg_event_id` (they are the SG bridge's), but the SG bridge does not always fill them — so a row can be skipped by one pipeline as "not mine" and never reached by the other. That is how the single biggest unmapped CRM event (Oregon@Illinois 2026-10-24, **143 orders**) sat unmapped while its TEvo event 3241216 was already in our own mirror at the right venue and date. `backfill_aq_maps()` (cron 307) does NOT call it — that fills from `sg_events_canonical`, which needs SG to have resolved the event first. Run it BEFORE the sibling fill (`:50`) and the bridge (`:53`). |
| **`aq_event_map.event_date` MIXES TIME ZONES — never join it to `events.occurs_at_local` on date alone** | — | ⚠ Some hub rows store UTC, others local. `aq_event_map` 6200 is `"Los Angeles Dodgers vs. St. Louis Cardinals"` at **2026-09-04 02:10** — UTC for the **09-03 19:10 local** game (tevo 3100237). Join that to the mirror on `event_date::date = left(occurs_at_local,10)::date` and it lands on 09-04's game, **`Washington Nationals at Los Angeles Dodgers`** — wrong opponent. **The two-token matchup floor does NOT catch it**: `angeles` + `dodgers` are shared, so it passes. 44 hub rows currently carry a UTC-shaped time (00:00–06:00, non-midnight). A venue-id + date hub fill was designed, measured at 45 rows, and **abandoned on inspection** for exactly this — plus US Open `"Session 5 (Grandstand Only)"` binding to `"Grounds Admission Only"`, a different product. `link_aq_tevo_from_events()` is safe from it only because it demands EXACT event-name equality, which the flipped/renamed forms fail. Any new hub↔mirror join needs a time-of-day check or exact-name equality, not a date window. |
| **Venue resolution: the state is usually IN the string — use it** | — | `cross_source_venue_resolve()` accepts `p_city`/`p_state` but every caller passed NULL, so it answered with the wrong STATE: `"XFINITY Center - MD"` → 1618 Xfinity Center **- MA** (Mansfield, Massachusetts) while the events were Maryland Terrapins at College Park; `"Memorial Stadium - NE"` → Oklahoma. ⚠ **The ambiguity guard (mig 20260908224500) does not catch this** — it only sees ambiguity present in the MAP, and when the map holds one of several same-named venues the prefix is unique in the table and the wrong answer passes. The resolver now derives a state hint from a trailing `- XX` / `, XX` (validated against a state/province list) and **filters candidates** on it, so a same-state candidate wins outright: `"Pantages Theatre - CA"` went from 1187 Tacoma **WA** to 1188 Hollywood Pantages **CA**. Rows whose stored `state` is malformed (38 of 1,267 hold `"New York"`, even `"shown"`) are skipped, not filtered out. **Callers must not strip the tail before resolving** — the mapper's rule 6 did exactly that and had to pass the hint explicitly (mig 20260908235500). |
...
**Buy-side (GoTickets) — cite the live GT page, not the terminal.** The terminal is the *analysis* surface; when an answer names specific GoTickets inventory (covering a sold order, a substitution, a deal), link where it can actually be bought:

```
https://pro.gotickets.com/tickets/<gt_event_id>/<slug>?sortBy=price&sortDirection=asc&sections=<section_id>
```

`<slug>` is cosmetic — `/tickets/<gt_event_id>/` alone resolves. **Both ids come straight from `gotickets_listings_snapshots`** (`gt_event_id` + `section_id`): `section_id` IS GoTickets' own URL section id, so `sections=` is a lookup, **never hand-mapped from the section label** (`Promenade  311` → `3262193`, operator-confirmed 2026-09-01). There is no per-listing deep link — filter to the section, sort price-ascending so the qualifying rows land on top, and cite `gt_listing_id` per row so the reader can identify them on the page. The substitution checker emits this link per GoTickets sub (`buy_url`, built by `core.substitutions.gotickets_buy_url`) — read it from the response rather than hand-building one.

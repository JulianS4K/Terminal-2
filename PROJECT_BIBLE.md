...
| `server.py` + `routers/*` + `core/*` | **A1** (file/package) | decomposition (BR-CODE-1): `routers/*` = `APIRouter` modules `include_router`'d back (`shares`/`seatmap`/`retail_chat`/`catalog`/`lists`/`broker`/`seatdata`/`axs`/`seatgeek`/`misc`/`site_essentials`); `core/*` = shared runtime both import (`config`·`auth`·`helpers`·`broker_helpers`·`movers`·`search`·`substitutions`·`store_events`·`observability`·`db`·`resilience`·`ratelimit`); the last three are the production-readiness infra layer — `db` (bounded-timeout Supabase client), `resilience` (DB circuit breaker + retry + storefront stale-read), `ratelimit` (Redis-or-in-process limiter); env knobs → `RESOURCES_BIBLE §7`; **one-directional import** (`server.py`/`routers` → `core`, never reverse); per-surface route blocks lane-owned — D0 `/api/broker/*`, D1 `/api/store/*` (§2.6). Full per-module route map → `RESOURCES_BIBLE §9`. |

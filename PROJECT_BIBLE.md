...
| TicketsData platforms | SH/VD/GT/TM only | **SG + TickPick also work** via TD `/events` (search by `performer_url`) + `/fetch` (listings). `td_api_platform` maps `TP→tickpick`; the `ticketsdata_event_xref.platform` CHECK now allows `TP`. **A new TD platform needs its own `td_enqueue_peak_<plat>` cron** — otherwise its active xref rows never enqueue→pull. Discovery mirrors GT: `td_<plat>_performers` allowlist + `td_<plat>_discover/_drain`, matched by venue+datetime. (A1 2026-05-29, `td_tickpick_platform`) |

...
| 5 | Cross-source bridge topology | the 5 ID rules + what NOT to rebuild (→ `D0_BIBLE.md §3`) |
...
## 3. Column-name landmines (catch SQL bugs before you author) *(v1.3 · A1 · 2026-06-08)*
...
## 8. Workflow recipes *(v1.1 · C1 · 2026-06-09)*

### "I want to understand the codebase fast (code knowledge graph)" *(v1.0 · C1 · 2026-06-09)*
Cold-start orientation aid. Generate an interactive **code knowledge graph** of the whole repo with the [Understand-Anything](https://github.com/Egonex-AI/Understand-Anything) Claude-Code plugin (tree-sitter + semantic analysis):
1. `/understand` — scans the repo and writes `.understand-anything/knowledge-graph.json`: **nodes** = files/functions/classes/SQL-migration tables/docs/configs/CI-pipelines; **edges** = imports/contains/exports/tested_by/depends_on; grouped into **architectural layers** + an ordered **guided tour**.
2. `/understand-dashboard` — opens the interactive force-graph to explore layers, search, click any node for its summary/tags, and walk the tour.
- **Outputs are gitignored, NOT versioned** (large + regenerable; the generated graph also trips `secret-scan` on test-name false positives like `…fastly…`). Regenerate per session as needed — nothing to commit.
- First full chart: 2026-06-09 — ≈1,855 nodes / 2,067 edges across **17 layers** (Database Migrations, Backend API & Source Clients, D4 Bridge, D0 Terminal, D1 Storefront, Edge Functions, CI/CD, Docs & Governance, Trip Planner, …). Fastest way to see "what talks to what" before touching a lane.

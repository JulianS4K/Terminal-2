...
| Mapping a S4K CRM event | `match_to_aq_event_id()`, or a trigram-similarity threshold on the event name | **Neither works.** The CRM payload has NO source event id (only name / a date-only `event_date` / venue), which kills 4 of that function's 5 tiers, and its surviving venue+date tier uses a ±6h window a date-only feed can't satisfy. A similarity threshold is WORSE than useless here: measured against four independently-verified orders, the correct event scored **1.000** but the wrong adjacent session scored **0.907** (others 0.884/0.857/0.821) — no cutoff separates them, and a wrong event silently prices a cover against another event's listings. Use `s4kcs_map_events()`: normalized-EXACT name + date, candidates must agree on one tevo id. (mig 20260901183000) |
| S4K CRM feed completeness | a HTTP 200 from `/marketplace/orders` means the full six-market book | **200 can be a PARTIAL book.** Markets are fetched in parallel and failures are reported in the body's `errors` array while the others still return. TickPick vanished for hours behind a 401 with the cron reporting success and 2,288 rows silently going stale. Check `v_s4kcs_ingest_health` / `s4kcs_orders_pending.errors`, never row count alone. (mig 20260901181500) |

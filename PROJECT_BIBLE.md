...
| 9 read-only `*_client.py` (evo/seatgeek/seatdata/ticketsdata/axs/tickpick/vivid/gotickets/bandsintown) | **A1** | 9 of the **11** clients; GET-only by construction (`broadway_client.py` = D3-owned, `paciolan_client.py` = D6-owned) |
| `paciolan_client.py`, `paciolan_extension/*`, `routers/paciolan.py`, `paciolan_*` schema | **D6** | primary box-office ingest (§2.3); the route block is D6 behavior inside A1's `routers/` structure. Inventory → `RESOURCES_BIBLE §2.18` |
...
| A pg_cron command with several statements (`SELECT drain(); SELECT scan();`) | assume a later statement failing leaves the earlier ones committed | **The whole command runs as ONE implicit transaction — a timeout in the LAST statement rolls back the FIRST.** Cost a live ingest outage 2026-08-11 (mig `20260811280000`): the deals scanner timed out and rolled back the GoTickets listings drain with it, so snapshots stalled, an ~11.9k-row inflight backlog built, and every later run then failed on that backlog. **Fix:** wrap the risky step in an exception-trapping `DO $b$ … EXCEPTION WHEN OTHERS THEN RAISE WARNING …; END $b$;` subtransaction, or split it into a separate job on offset minutes (prod runs `gt_ingest_drain_1min` + `gt_deals_scan_odd_min` on odd minutes so they can never share a transaction). Pairs with the §3 `EXCEPTION WHEN OTHERS` row — `OTHERS` does NOT trap `query_canceled`. |
| A per-row helper fn that rescans a whole parent set (`gt_curated_zone_id` called once per sale) | call the resolver inside the row loop | **O(rows × entire parent set) → statement timeout**, the other half of the same 2026-08-11 incident: resolving each sale's curated zone rescanned ALL of that performer's zones, over a 45d sales window × every outlier. **Fix:** resolve ONCE per listing into a CTE, then match rows against only that zone's rules (`performer_zone_rules` + `section_in_range`), and narrow the window (45d→30d). Same shape as the firehose per-ROW trigger row above. |
| GoTickets DEALS scanner returning nothing for an event | assume the scan is broken | **Deals are manual-curated-zone-ONLY** (realized-median v2, mig `20260811273000`, operator directive "hard default to manual zones"): a listing whose section does not resolve through `gt_curated_zone_id` produces **no deal, by design**. Curated zones are hand-maintained in `performer_zones`/`performer_zone_rules` — an event whose performer has no curated zone is coverage-only, not a bug to fix in the scanner. |
| `gotickets_event.tevo_event_id` · `paciolan_*` snapshots' `tevo_event_id` | filter/join assuming it is populated | **AQ-derived, often NULL on fresh rows** — the same trap as every other source table (§0). Populated by `gotickets_match_us_6h` (`match_gotickets_us_events`, matcher-v3 reuse) and `paciolan_aq_match()`→`paciolan_apply_aq()`. Unmapped ⇒ invisible to the poll cadence AND to the deals scanner. The GT firehose price column is **`all_in_price`** (not `price`/`broadcast_price`). |
...
| `get_deals_feed(p_limit, p_since, p_min_roi, p_min_confidence, p_include_gone)` | int, timestamptz, numeric, text, bool | **D0 DEALS page primary** (`static/terminal/deals.js`). The live GoTickets deal feed — newest first, `gone_at` rows excluded unless `p_include_gone`. |
| `scan_gotickets_deals(p_max_events, p_z_threshold, p_min_section_n, p_min_section_median, p_min_roi, p_seller_fee, p_min_realized_n, p_min_win_prob, p_degr_*, p_dumping_gate)` | 13 params, all defaulted | The scanner behind the feed (section-outlier gate + ≥15%-profit gate, realized-median v2). Cron-driven via `gt_deals_scan_odd_min`; call with a small `p_max_events` by hand — it is the heaviest fn in the GT stack (see the §3 incident rows). |
| `get_event_listing_outliers(...)` / `get_zone_baseline(...)` / `get_deal_backtest(...)` | — | Per-event outlier detail · zone comparable anchor · backtest of the deal gates. |
| `get_gotickets_event_listings(...)` / `get_gotickets_event_series(...)` | — | Read the GT book for one event (current listings · time series) — the terminal's GoTickets panels. |
| `match_gotickets_us_events(...)` / `gotickets_attempt_event_xref(...)` | — | GT→TEvo mapper (reuses matcher v3 + `cross_source_venue_resolve`). Cron `gotickets_match_us_6h`; writes only currently-unmapped rows, never overwrites an existing map. |
| `gt_listings_poll_tick(p_max)` / `gt_listings_drain(p_max)` | int | GT firehose poll + drain (EVO-firehose mirror). Poll is tiered: curated-zone teams on the deal cadence, all other US non-parking events on the EVO DTE curve. |
| `paciolan_ingest_secure(...)` / `paciolan_aq_match()` / `paciolan_apply_aq()` | — | **D6** extension-capture entry point + the AQ mapping pair that fills `tevo_event_id` (NULL until they run). Inventory → `RESOURCES_BIBLE §2.18`. |

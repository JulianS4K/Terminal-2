...
| 2026-06-03 | 20260603180000 | **SG `/listings` owned/non-owned poller split** (operator directive; token-spend/429 fix). `sg_listings_poll_tick` gains `p_owned_kind ('owned'|'non'|NULL)`; old 2-arg signature dropped. RETIRES combined `sg_listings_poll_2min`; ADDS `sg_listings_poll_owned_1min` (`*/1`, `p_max=5`, **enabled**) + `sg_listings_poll_nonowned_5min` (`*/5`, `p_max=8`, **gate-disabled**). Non-owned `collector_cadence` (owned_kind='non') all bands → **1440 (once-daily)** regardless of horizon. "owned" = `owned_count_last_7d>0` (cadence flag, not the seller feed). Both crons share the one rate-aware/strand-safe fn. Resume non-owned: `UPDATE cron_policy SET enabled=true WHERE jobname='sg_listings_poll_nonowned_5min'`. |
| 2026-06-03 | 20260603190000 | **Halve raw listings retention** (operator directive: "cut listings data retention by half, per pull data, but metrics retain normal rates"). `sweep_old_listings` default **30d→15d**; cron `sweep-old-listings` now passes `15` explicitly. `event_metrics` **unchanged at 120d** + all generated rollups untouched → aggregated history/charts preserved. `listings_snapshots` was 18 GB / ~50.5M rows; 80s-bounded daily sweep drains the 15–30d backlog (~half the table) over ~1–3 weeks, then holds. No bulk delete run. |
| 2026-06-03 | 20260603200000 | **Data-health audit fixes (1/2)** — three prod failures found in the all-fronts freshness sweep. (a) **SG seller-listings ingest** (`sg_seller_process_30min` 24/24 fail, 169 backlog): began dropping the enforced FKs on `seatgeek_seller_listings`. (b) **`sweep_duplicate_listings`** 12/12 timeout: target recently-polled events (`evo_listings_poll_state`) + time-box; cron p_max 50→200. (c) **`evo_discover_new_events`** daily error: fixed the positional `bot_chat_log(…)` call (`'status'`→p_level, `'D0'`→int p_related_pr) to named args. Reddit/SeatData/TickPick left dormant per operator. |
| 2026-06-03 | 20260603210000 | **Audit fix — dedup per-event bound.** `…200000`'s dedup still timed out (ORDER BY last_polled DESC picks the hottest = biggest events; one event's full-history window-sort exceeds the cap). Bound the per-event dedup to a **2h captured_at window** (dups form at write time; older ones age out via 15d retention). Verified: completes fast, 0 leftover dups. |
| 2026-06-03 | 20260603220000 | **Audit fix — seller ingest (2/2).** Dropping the 2 VALID FKs wasn't enough: the live insert then failed on the *NOT VALID* `seatgeek_seller_listings_sg_event_id_fkey` (a NOT VALID FK STILL enforces inserts — PROJECT_BIBLE §3). Dropped the remaining 2 FKs (`sg_event_id`, `tevo_event_id`). Verified: `sg_seller_process()` ingests cleanly (282 fresh rows, backlog draining). |
| 2026-06-08 | 20260608030000 | **SG owned-focus + listings full-count fix** (operator-directed). **(A) full-count fix:** `sg_broker_listings_process` now writes true per-pull metrics from the WHOLE broker response into `seatgeek_event_metrics` — the `ON CONFLICT(content_hash)` raw dedup made the hourly refresh under-count (`listings_all_count`=411 when the live feed had 3,560; a 06-08 head-to-head vs **TD-seatgeek** proved broker depth = TD depth, so it was a counting bug, NOT a depth gap → don't route SG via TD, keep PR #449 kill-switch). Verified live: 5,225/2,353/977. Retired `sg_broker_listings_metrics_refresh_hourly`. **(B)** listings poll OWNED-only @ ½ band rate (`sg_listings_poll_tick` +`p_rate_mult`, owned cron passes 2); DROPPED league + nonowned pollers + league rescue. **(C)** sales flat 24h, all non-SKIP ≤90d (broker `/sales` shares the ONE 10-req/10s bucket w/ listings — no daily cap), post-event flush kept. Net SG broker ~6,500→~3,360/day; 429 burst clears. Old fn overloads dropped. |
| 2026-06-08 | 20260608031000 | **Fix movers index frozen since 06-02** — `refresh_movers_agg` ran all 5 windows in ONE txn under a short timeout; 180/365 re-percentile 1.7–3.25M raw TD rows → timeout → whole block aborts → `event_movers_agg` froze → compute's 18h guard returned 0 everywhere. FIX: per-window savepoint isolation + 8-min per-window budget. 7/15/30d un-frozen; 180/365 isolated. Follow-up: source long-window TD trends from `event_listing_snapshot_daily` (pre-agg) not raw. |
| 2026-06-08 | 20260608032000 | **bot_chat backlog: status-reply auto-resolve + noise sweep** (lands B1's bot_chat-233 drift fix; C1-owned view). `v_bot_chat_unresolved` now excludes rows with a `status` reply via `in_reply_to` (implements CLAUDE.md §1, previously uncoded). One-time bulk-resolve of auto-noise (all `status`, all `SG_BROKER_429%` flags, stale change_log/sync/broadcast): unresolved **1,321→104** (remainder = real old flags/questions). |
| **2026-06-30** | 20260630160000 | **TD sourcing Phase 1 — AXS credit-ledger unify.** AXS `/fetch` charges `td_charge_credit` at ingest; +3 accounting views (`v_td_credit_usage_by_platform`/`_mtd`/`v_td_credit_cost_per_call`); true AXS cost ≈1/fetch (the "~46/fetch" was counter contention). Full model: RESOURCES_BIBLE §5.1. |
| 2026-06-30 | 20260630170000 | **Peak/off-peak window** — `is_peak()` ET 09:00–01:59 (data-derived from 135k SG realized sales); both `collector_band` overloads use it; AXS probe off-peak throttle + even leapfrog (SH`:0`·VD`:2`·GT`:4`·TM`:6`·AXS`:8`). |
| 2026-06-30 | 20260630180000 | **SH/VD enrollment seed** — `td_seed_mapped_xref()` generalizes the owned seed to ALL AQ-mapped events (owned-first, templated URLs); active enrollment 602→~2,100. |
| 2026-06-30 | 20260630190000 / 200000 | **Budget caps → 250k pool** (reverted an erroneous drift to 50k/600k) + **cycle-aligned to the billing 9th** (`td_credits_this_cycle()`, not calendar month). |
| 2026-06-30 | 20260630210000 | **Phased caps** — 300k plan now → **8k REAL/day from 7/9** (gate compares recorded ×1.276 real:recorded ratio); AXS probe now also `td_budget_ok()`-gated. |
| 2026-06-30 | 20260630230000 | **Phase-A TD cadence ×0.6 boost** (TD-only, date-gated until 7/9) to burn the otherwise-expiring surplus. |
| 2026-06-30 | 20260630220000 | **TD-SeatGeek surplus burn** (TEMPORARY — auto-reverts 7/9 via `td_sg_burn_teardown`): kill-switch overridden, ~1,948 SG events enrolled, TD `/fetch` confirmed to serve SG listings. |
| 2026-06-30 | 20260630240000 | **Breadth ladder + non-owned policy + consolidation** — `td_should_poll` (GT ≤30d · TP ≤3d · non-owned 1/day + SH-only >30d); `td_enqueue_peak` made date-robust; **`td_tier_enqueue_*` RETIRED** (dup of same xref). |
| 2026-06-30 | 20260630250000 | **Ticketmaster → MSG-only** (venue 896): deactivate non-MSG TM, seed the 22 MSG events with a tm_event_id. |
| **2026-07-05** | 20260705153000 | **Unified retention engine** — NEW `retention_policy` (per-table keep_days/enabled = THE kill switch) + `retention_run_log` + `retention_tick()` (hourly cron `retention_tick_hourly`, ctid-batched, dual wall-clock budgets, per-policy error capture). Retires the `sweep-old-*` cron family (all 6 unscheduled WITH a migration trail — they'd been toggled off out-of-band 2026-07-01 and listings_snapshots hit 75 GB/227M rows unswept; audit AUD-16). Ladder re-armed: listings 15d · TD 30d · section_metrics 60d · section_rows 30d · event_metrics 120d · espn_injuries 90d; **SG row seeded DISABLED** (pollers dark since 06-26 — operator decision). Deadman watch +2 rows. |
| 2026-07-05 | 20260705153500 | **Venue-anchored section medians** — NEW `venue_section_price_daily` (one row per event × venue-section × day = day's last `section_metrics` capture; INDEFINITE retention; venue-keyed via `venue_section_map` evo tokens) + performer/AWAY-TEAM dims (name-parse → `performer_metadata`, else 2-performer pair rule; 12/12 on live sample) + `rollup_venue_section_price_daily()` / `venue_section_rollup_catchup()` (hourly cron `venue_section_rollup_hourly`: today refresh + trailing-70d backfill). **Read layer**: `venue_section_{performer,opponent}_agg` (kind `team`\|`other` keeps concert performers separate from teams; exact percentile_cont medians from each event's latest daily row) rebuilt by `refresh_venue_section_aggs()` after each rollup; views `v_venue_section_medians_by_{performer,opponent}` read the aggs; RPC `get_venue_section_medians` (email-gated) feeds the NEW D0 venue-page SECTIONS tab (`static/terminal/venue.{html,js}`). Preserves per-section median history across events before the 60d raw TTL deletes it. |

---

Last updated: 2026-07-05 — **retention redesign + venue-section medians** (migs `20260705153000`/`153500`: retention_policy engine replaces the sweep-old-* cron family; venue_section_price_daily indefinite rollup + by-performer/by-opponent median views; authored on PR branch, prod apply operator-gated). | 2026-06-30 — **TD sourcing redesign** (migs `20260630160000`–`250000`): AXS credit ledger + accounting views, peak/off-peak `is_peak()` + even leapfrog, SH/VD + MSG-TM enrollment, 250k-REAL phased budget (cycle-aligned to the 9th, 8k/day from 7/9), breadth ladder + non-owned 1/day policy, `td_tier_enqueue_*` retired, SG surplus burn (auto-reverts 7/9); all applied to prod + merged to main (PR #699). | 2026-06-08 — SG owned-focus + listings full-count fix (`20260608030000`) + movers agg-timeout fix (`…031000`) + bot_chat auto-resolve & noise sweep (`…032000`), all applied to prod & file-synced to main. | 2026-06-08 — cross-source amalgam value landed to main (mig `20260531214306`: amalgam_getin/median/source_count generated cols + `get_event_amalgam` RPC; applied to prod 05-31 via MCP, file now synced). | 2026-06-03 — added SG owned/non poller split (`20260603180000`), raw-listings retention halve 30d→15d (`20260603190000`), + 3 data-health audit fixes (`20260603200000`–`220000`: seller-listings FK ingest, dedup per-event bound, evo_discover bot_chat_log), all applied to prod (branch `claude/sg-listing-token-spend-Et52v`, PR #423). | 2026-05-31 — added 6 collector-cadence + retention migs (collector_cadence `20260531140000`, EVO poller …150000, SG poller+bands …160000, cron cutover …170000, retention Wave 1 …180000, P0 fixes …200000). | 2026-05-30 — terminal event-page chain timeout audit (mig 20260530120000; branch `a1/terminal-sql-audit-fixes`, applied to prod, not yet pushed to main). | 2026-05-29 — added 7 landmark migs (TD focus 20260529120000, SG floor-sweep …170000, TickPick platform …180000, TP+TM metrics …190000, Ticketmaster platform …200000, TM fixes …210000, TM resale split …220000); all pushed to main. | prior: 2026-05-28 — added §14 landmark migration log (migrated from `PROJECT_BIBLE.md §9`); added roster note reconciling "audit lane" → A1.
